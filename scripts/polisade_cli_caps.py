...
Runtime contract (skills):
    ...
    caps=$(python3 scripts/polisade_cli_caps.py detect)
...
from _polisade_env import env_get  # noqa: E402  (v3.0.0 PDLC_→POLISADE_ fallback)
...
# because this helper runs as `python3 scripts/polisade_cli_caps.py`, which puts
...
# ---------- issue #107 — skills emission allowlist --------------------------
...
def claude_only_skills(root) -> set[str]:
    """Return skill names excluded from EVERY converted build (qwen / opencode /
    gigacode): those whose SKILL.md frontmatter OR cli-capabilities.yaml entry
    sets `claude_only: true`. They ship only in the Claude Code plugin source
    (#187 — convert.py never targets Claude Code, so a claude-only skill must be
    absent from every command / emitted-skill / routing path it produces)."""
    out: set[str] = set()
    plugin_root = Path(root)
    for name, f in _iter_skill_files(plugin_root):
        try:
            fm, _ = _parse_frontmatter(f.read_text(encoding="utf-8"))
        except (IOError, OSError, UnicodeDecodeError):
            continue
        if _truthy(fm.get("claude_only", False)):
            out.add(name)
    manifest = load_manifest(root)
    for name, info in (manifest.get("skills") or {}).items():
        if isinstance(info, dict) and info.get("claude_only") is True:
            out.add(name)
    return out


def get_emit_as_skill_allowlist(root) -> set[str]:
    """Return the set of skill names flagged `emit_as_skill: true` in the
    manifest, MINUS any `claude_only` skill (those are never emitted into a
    converted build — #187). Used by `tools/convert.py` (Qwen/GigaCode skills
    emission + QWEN.md/AGENTS.md intent routing),
    `scripts/polisade_lint_skills.py` (description quality gate), and
    `scripts/regression_tests.sh` (issue #107 regression)."""
    manifest = load_manifest(root)
    if not manifest:
        return set()
    out: set[str] = set()
    for name, info in (manifest.get("skills") or {}).items():
        if not isinstance(info, dict):
            continue
        if info.get("emit_as_skill") is True:
            out.add(name)
    return out - claude_only_skills(root)


def get_intent_triggers(root, skill_name: str) -> list[str]:
    """Return the list of natural-language intent trigger phrases for a
    skill, or [] when the skill has no `intent_triggers` entry. The
    manifest is the behavioural source of truth for QWEN.md routing and
    for the description-consistency lint rule."""
    manifest = load_manifest(root)
    if not manifest:
        return []
    info = (manifest.get("skills") or {}).get(skill_name) or {}
    triggers = info.get("intent_triggers")
    if not isinstance(triggers, list):
        return []
    return [t for t in triggers if isinstance(t, str)]


#: Providers `settings.vcsProvider` accepts. A `vcs_default` naming anything
#: else is a manifest typo that would ship a project state `polisade_doctor.py`
#: reports as `unknown provider`, so it is refused instead of propagated.
VALID_VCS_DEFAULTS = ("github", "bitbucket-server")


def get_vcs_default(root, target: str) -> str | None:
    """Return `targets.<target>.vcs_default` from the manifest (issue #120).

    None when the manifest, the target or the key is absent — the caller then
    leaves the init template's own `github` default alone. A present but
    invalid value raises: silently ignoring it would ship a build whose
    canonical PROJECT_STATE.json fails `polisade_doctor.py::check_vcs_provider`.
    """
    manifest = load_manifest(root)
    if not manifest:
        return None
    tinfo = (manifest.get("targets") or {}).get(target) or {}
    value = tinfo.get("vcs_default")
    if value is None:
        return None
    if value not in VALID_VCS_DEFAULTS:
        raise ValueError(
            f"cli-capabilities.yaml: targets.{target}.vcs_default={value!r} is "
            f"not one of {VALID_VCS_DEFAULTS} — a project initialised from this "
            f"build would fail doctor's vcs_provider check"
        )
    return value


def overlay_path(skill: str, overlay_root, target: str = "qwen") -> Path:
    """Expected overlay file location for a skill, per target layout.

    Qwen/GigaCode use the namespaced `commands/polisade/<skill>.md` layout.
    opencode (issue #170) names commands by file stem and has no `:`-namespace,
    so its overlays are flat `commands/polisade-<skill>.md` — matching what the
    converter emits and what apply_overlay copies for that target.
    """
    if target == "opencode":
        return Path(overlay_root) / "commands" / f"polisade-{skill}.md"
    return Path(overlay_root) / "commands" / "polisade" / f"{skill}.md"
...
def check_target_coverage(plugin_root, target: str, overlay_root) -> list[dict]:
    ...
    # claude_only skills are never built for any converted target, so they
    # cannot need an overlay there (#187).
    excluded = claude_only_skills(plugin_root)
        ...
        for cap in reqs:
            ...
            if markers:
                ...
                if not any(m in body_text for m in markers):
                    continue
            if overlay_root is not None and overlay_path(skill_name, overlay_root, target).exists():
                continue
            overlay_dir = "opencode-overlay" if target == "opencode" else "qwen-overlay"
            overlay_rel = overlay_path(skill_name, Path("tools") / overlay_dir, target)
            issues.append({
                "level": level,
                "skill": skill_name,
                "cap": cap,
...
def lint(plugin_root) -> list[dict]:
    ...
    # (c) — target coverage for every enforced target. Each target resolves
    # its own overlay directory: opencode (issue #170) ships flat-named
    # overlays under tools/opencode-overlay; Qwen/GigaCode share
    # tools/qwen-overlay (GigaCode is a rename-pass over the Qwen build).
    for target in (manifest.get("targets") or {}):
        overlay_dir = "opencode-overlay" if target == "opencode" else "qwen-overlay"
        overlay_root = plugin_root / "tools" / overlay_dir
        issues.extend(
...
def detect_current_cli() -> str:
    ...
    Env override wins: set POLISADE_CLI={claude-code|qwen|gigacode} to force it
    (legacy PDLC_CLI still honoured with a deprecation warning).
    """
    forced = env_get("CLI")
    if forced:
        return forced
    ...
    if os.environ.get("QWEN_CODE_ENV") or os.environ.get("QWEN_CLI"):
        return "qwen"
    # issue #170: opencode exports OPENCODE_* in its tool/run environment.
    if os.environ.get("OPENCODE") or os.environ.get("OPENCODE_BIN"):
        return "opencode"
    # PATH fallback
    if _which("gigacode"):
        return "gigacode"
...
def _discover_plugin_root() -> str:
    ...
    self-locate (this file lives at `<plugin_root>/scripts/polisade_cli_caps.py`).
    """
    env = env_get("PLUGIN_ROOT")
    if env:
        return env
    return str(Path(__file__).resolve().parent.parent)


# OPS-007 / issue #55: a bare `shutil.which("codex")` returns true for any
# binary that happens to be named `codex` (corp envs have legacy utilities
# with that name). We additionally probe `<cli> --version` and match the
# output against a branding regex. Only `codex` is gated here — self-CLIs
# are detected via env vars in detect_current_cli() (collision ≈0); expand
# this map if we start seeing self-CLI impersonators.
#
# Pattern verified on codex-cli 0.123.0 (2026-04): `codex --version` prints
# "codex-cli 0.123.0". Multiline + case-insensitive, anchored at the start
# of a line so a foreign binary can't pass by mentioning "codex" in free
# text. NB: tied to current OpenAI Codex CLI branding — if the package is
# renamed, update this map together with scripts/ops007_smoketest.sh and
# RELEASE_NOTES.md.
_IDENTITY_PATTERNS = {
    "codex": r"(?mi)^\s*codex-cli\b",
}


def _identity_ok(cli: str) -> tuple[bool, str | None]:
    """Run `<cli> --version` and match against _IDENTITY_PATTERNS[cli].

    Returns (ok, reason). `reason` is human-readable and always populated
    on failure; on success it is None. Timeout is controlled by
    POLISADE_IDENTITY_TIMEOUT (seconds, default 5; legacy PDLC_IDENTITY_TIMEOUT
    still honoured with a deprecation warning).
    """
    pattern = _IDENTITY_PATTERNS.get(cli)
    if not pattern:
        return True, None  # no identity gate configured for this cli
    try:
        timeout = int(env_get("IDENTITY_TIMEOUT", "5"))
    except ValueError:
        timeout = 5
    try:
        proc = subprocess.run(
        ...
        return False, f"{cli} binary not found on PATH"
    except subprocess.TimeoutExpired:
        return False, f"{cli} --version timed out after {timeout}s"
    except OSError as exc:
        return False, f"{cli} --version failed: {exc}"
    out = (proc.stdout or "") + (proc.stderr or "")
    if re.search(pattern, out):
        return True, None
    snippet = out.strip().replace("\n", " | ")[:120] or "(empty output)"
    return False, f"{cli} identity check failed; got: {snippet}"


def detect_available() -> dict:
    cli = detect_current_cli()
    # Gate through `_which(...)` (not `shutil.which` directly) so OPS-022
    # fixtures that monkey-patch `_which` to force "all CLIs available"
    # keep working without also needing to patch `shutil`.
    if _which("codex"):
        codex_path = shutil.which("codex")
        codex_ok, codex_reason = _identity_ok("codex")
    else:
        codex_path = None
        codex_ok, codex_reason = False, None
    return {
        "cli": cli,
        "codex": bool(codex_ok),
        "task_tool": cli in ("claude-code", "qwen", "opencode"),
...
def _own_cli_for(cli: str) -> str | None:
    return {
        ...
        "qwen": "qwen-code",
        "gigacode": "gigacode",
        # issue #170: opencode self-review binary is `opencode` itself.
        "opencode": "opencode",
    }.get(cli)


SELF_CLIS = ("claude-code", "qwen", "gigacode", "opencode")
...
# required here. opencode (issue #170) self-reviews via `opencode run`.
REQUIRED_SELF_TARGETS = ("claude-code", "qwen", "gigacode", "opencode")
...
def _codex_unavailable_reason(avail: dict) -> str:
    """OPS-007 — craft a reviewer.reason that distinguishes 'not installed'
    from 'installed but failed identity check'. Consumers use this when
    codex is required but unusable."""
    ident = (avail.get("identity") or {}).get("codex") or {}
    if ident.get("path") and not ident.get("ok"):
        return ident.get("reason") or "codex identity check failed"
    return "codex CLI is not on PATH"
...
def _resolve_auto(avail: dict, env_cli: str, manifest: dict | None = None) -> dict:
    ...
    if own and avail.get(own):
        return {"mode": "self", "cli": env_cli,
                "cmd": [own, *_self_args(manifest, env_cli)]}
    return {"mode": "blocked", "cli": env_cli, "cmd": [],
            "reason": f"no reviewer CLI available ({_codex_unavailable_reason(avail)})"}
...
def resolve_reviewer(prefer: str | None, settings: dict | None = None,
    ...
    avail:    dict | None   — pre-computed result of `detect_available()`.
              Pass this when the caller already ran detect_available()
              itself, to avoid re-running the identity probe (which
              subprocess-calls `codex --version`). OPS-007: on a
              slow/hanging codex binary, an independent recompute
              doubles the POLISADE_IDENTITY_TIMEOUT wall time.
    ...
    # honours POLISADE_PLUGIN_ROOT (OPS-021) so integration fixtures can swap in
    # a custom cli-capabilities.yaml.
    manifest = load_manifest(_discover_plugin_root())

    # OPS-007: if a binary named `codex` is present but failed identity
    # verification, the result carries a warning. Consumer-side skills emit
    # it so the user sees *why* codex was ignored, instead of a silent
    # fallback to self-review (issue #55 criterion 5). This warning is
    # independent of the resolved mode — the impersonator's presence is
    # noteworthy regardless of which reviewer we end up picking.
    ident = (avail.get("identity") or {}).get("codex") or {}
    warning = None
    if ident.get("path") and not ident.get("ok"):
        warning = (f"codex binary at {ident['path']} failed identity check "
                   f"and was ignored: "
                   f"{ident.get('reason') or 'identity mismatch'}")
    ...
    if prefer == "self":
        return _with_warning(_resolve_self(avail, env_cli, manifest))
    ...
    if mode not in VALID_REVIEWER_MODES:
        return _with_warning({"mode": "blocked", "cli": env_cli, "cmd": [],
    ...
    if mode == "off":
        return _with_warning({"mode": "off", "cli": env_cli, "cmd": [],
    ...
    if mode == "external":
        ...
        if avail["codex"]:
            return _with_warning({"mode": "codex", "cli": "codex",
                    "cmd": ["codex", *_codex_args(manifest)]})
        return _with_warning({"mode": "blocked", "cli": env_cli, "cmd": [],
                "reason": f"settings.reviewer.mode=external requires Codex CLI, "
                          f"but it is unavailable: "
                          f"{_codex_unavailable_reason(avail)}"})
    ...
    if forced_cli == "codex":
        if avail["codex"]:
            return _with_warning({"mode": "codex", "cli": "codex",
                    "cmd": ["codex", *_codex_args(manifest)]})
        return _with_warning({"mode": "blocked", "cli": env_cli, "cmd": [],
                "reason": f"settings.reviewer.cli=codex but Codex CLI is "
                          f"unavailable: {_codex_unavailable_reason(avail)}"})
    if forced_cli in SELF_CLIS:
    ...
    return _with_warning(_resolve_auto(avail, env_cli, manifest))
...
def _cmd_detect(args) -> int:
    ...
    # OPS-007: thread the already-computed avail so the identity probe
    # (subprocess `codex --version`) runs exactly once per invocation.
    out["reviewer"] = resolve_reviewer(None, settings=settings, avail=out)

    # OPS-007: `reviewer.warning` is surfaced in the structured output (JSON
    # field and text-format "warning:" line). Presentation is owned by the
    # consumer — skills echo the JSON field once; no stderr dup here.
    ...
    if args.format == "text":
        ...
        if rv.get("warning"):
            print(f"           warning: {rv['warning']}")
        print("available:")
        # Skip keys whose values are dict/list — they'd render as dict.__repr__
        # in the flat loop. Render `identity` separately below.
        for k, v in out.items():
            if k in ("cli", "reviewer") or isinstance(v, (dict, list)):
                continue
            print(f"  {k:<12} {v}")
        ...
        if identity:
            ...
            for cli_name, info in identity.items():
                ...
                print(f"  {cli_name}: ok={ok} path={path}"
...
def main() -> int:
    ap = argparse.ArgumentParser(prog="polisade_cli_caps")
