...
def check_version_consistency(root):
    ...
    - `skills/init-verify/SKILL.md` → `EXPECTED_PDLC_VERSION` literal
    ...
    if verify_path.exists():
        try:
            ...
            m = re.search(
                r'EXPECTED_PDLC_VERSION\s*=\s*"([^"]+)"',
                text,
            )
            if m:
                sources["init-verify/SKILL.md::EXPECTED_PDLC_VERSION"] = m.group(1)
        except (OSError, UnicodeDecodeError):
            pass
...
def check_post_apply_recipe(root):
    """Issue #108 — post-apply commit+PR recipe contract for /pdlc:migrate
    and /pdlc:sync, plus three weak-model anti-patterns in /pdlc:pr.

    Background: corp-session 2026-04-24 (GigaCode CLI / Bitbucket Sigma) —
    PM ran `/pdlc:migrate --apply` + `/pdlc:sync --apply`, then asked to
    commit and open a PR. The agent improvised: bare `git push` (#75/#97),
    ad-hoc Python with direct Bitbucket REST API + .env tokens (PM
    cancelled), `pdlc_pr.py` (does not exist), and
    `--body "$(git log ...)"` (corp shell rejects command substitution).

    Positive markers (must appear in BOTH skills/migrate/SKILL.md and
    skills/sync/SKILL.md):

      - `pdlc_vcs.py git-push` — push helper invoked, not bare `git push`
        (invariant #10 / OPS-028).
      - `pdlc_vcs.py pr-create` — PR via the canonical script, not /pdlc:pr
    ...
    Anti-pattern markers (must appear in skills/pr/SKILL.md):
      ...
      - `pdlc_pr.py` — explicit «no such file» mapping to pdlc_vcs.py.
      - `requests.post` OR `REST API` — ban on ad-hoc Python/curl PR
        creation that bypasses pdlc_vcs.py.
    """
    issues = []
    ...
    if pr_path.exists():
        ...
        anti_patterns = (
            ...
            (("pdlc_pr.py",),
             "explicit `/pdlc:pr → pdlc_vcs.py` name-mapping (no such "
             "file as pdlc_pr.py)"),
    ...
    if "<!-- pdlc:init INLINE TEMPLATES BEGIN -->" not in text:
        issues.append({
            ...
            "message": (
                ...
                "`<!-- pdlc:init INLINE TEMPLATES BEGIN -->` marker. "
                "The converter cannot inline canonical template bytes "
                "without it; under GigaCode Filesystem Guard "
                "/pdlc:init will silently reconstruct templates."
            ),
        })
    if "<!-- pdlc:init INLINE TEMPLATES END -->" not in text:
        issues.append({
            "skill": "init",
            "level": "error",
            "message": (
                "Issue #119: skills/init/SKILL.md missing "
                "`<!-- pdlc:init INLINE TEMPLATES END -->` marker."
            ),
        })
...
def check_migrate_canonical_env_example(root):
    """Issue #119 — `scripts/pdlc_migrate.py._CANONICAL_ENV_EXAMPLE` literal
    ...
    try:
        ...
        issues.append({
            "level": "error",
            "message": (
                f"Issue #119: scripts/pdlc_migrate.py failed AST parse "
    ...
    if literal is None:
        issues.append({
            "level": "error",
            "message": (
                "Issue #119: scripts/pdlc_migrate.py is missing "
    ...
    if literal != expected:
        issues.append({
            "level": "error",
            "message": (
                "Issue #119: scripts/pdlc_migrate.py "
...
def audit_install_dir_refs(root):
    ...
    verbatim into a follow-up issue body discussing whether `.pdlc/bin/`
    ...
    Classification:
      - read   — `Read tool` / `read_text(` / shell `cat` / glob over the
                 install dir.
      - exec   — `python3 ${PDLC_PLUGIN_ROOT...}/scripts/X` / `bash`.
      - other  — anything else (mention, doc reference).

    Additional columns:
      - scripts_used — sorted list of `*.py` filenames referenced after
                       `{plugin_root}/scripts/`.
      - criticality  — high  (PR/state workflow scripts: pdlc_vcs,
                              pdlc_sync, pdlc_migrate),
                       med   (diagnostic/lint: pdlc_doctor, pdlc_cli_caps,
                              pdlc_lint_*, pdlc_check_release_notes),
    ...
    install_re = re.compile(
        r"(\$\{PDLC_PLUGIN_ROOT[^}]*\}|\{plugin_root\}|skills/init/templates/)"
    )
    read_re = re.compile(
    ...
    # follows in the canonical `${PDLC_PLUGIN_ROOT:-...}/scripts/...`
    ...
    exec_re = re.compile(
        r"(?:python3?|bash|sh)\s+"
        r"(?:\$\{PDLC_PLUGIN_ROOT[^}]*\}|\{plugin_root\})"
        r"(?=/|\s|$)"
    )
    ...
    high_crit_scripts = {"pdlc_vcs.py", "pdlc_sync.py", "pdlc_migrate.py"}
    med_crit_scripts = {
        "pdlc_doctor.py", "pdlc_cli_caps.py", "pdlc_lint_skills.py",
        "pdlc_lint_artifacts.py", "pdlc_check_release_notes.py",
    }
    for d in sorted(skills_dir.iterdir()):
        if not (d.is_dir() and (d / "SKILL.md").exists()):
            continue
...
def main():
    ...
    if "--audit-install-refs" in flags:
        ...
        print("# audit: skill references to plugin install dir (issue #119)\n")
        print("| skill | refs | kinds | scripts_used | criticality |")
        print("|---|---|---|---|---|")
        for r in rows:
            scripts = ",".join(r["scripts_used"]) if r["scripts_used"] else "—"
            print(
                f"| {r['skill']} | {r['refs']} | {','.join(r['kinds'])} "
                f"| {scripts} | {r['criticality']} |"
            )
        sys.exit(0)
    ...
    if canonical_env_issues:
        ...
        results.append({
            ...
            "dir": "scripts/pdlc_migrate.py + skills/init/templates/env.example",
            "status": status,
    ...
    post_apply_issues = check_post_apply_recipe(root)
