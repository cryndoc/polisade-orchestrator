...
# ---------------------------------------------------------------------------
# Issue #74 (legacy OPS-027) — shared helpers for the `git add -f` guard.
#
# Module-level (not inside check_git_add_force_guard) because three other
# consumers import these via importlib.util:
#   - scripts/ops027_repro_harness.sh  (preflight guard-check)
#   - scripts/ops027_check_session_log.py  (post-hoc session analysis)
#   - scripts/regression_tests.sh      (post-convert check)
#
# PUBLIC CONTRACT: renaming or moving any of these symbols is a breaking
# change — update all three consumers together.
# ---------------------------------------------------------------------------
...
def check_version_consistency(root):
    """Check that template PROJECT_STATE.json pdlcVersion matches plugin.json version."""
    issues = []
    plugin_path = root / ".claude-plugin" / "plugin.json"
    template_path = root / "skills" / "init" / "templates" / "PROJECT_STATE.json"

    if not plugin_path.exists() or not template_path.exists():
        return issues

    try:
        with open(plugin_path) as f:
            plugin_ver = json.load(f).get("version", "")
        with open(template_path) as f:
            template_ver = json.load(f).get("pdlcVersion", "")
    except (json.JSONDecodeError, IOError):
        return issues

    if plugin_ver and template_ver and plugin_ver != template_ver:
        issues.append({
            "level": "error",
            "message": f"Version mismatch: plugin.json={plugin_ver}, template PROJECT_STATE.json={template_ver}",
        })
    return issues
