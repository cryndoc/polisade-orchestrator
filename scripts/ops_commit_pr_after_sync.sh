...
work_a=$(mktmp)
_mk_project "$work_a" 6 TASK-001  # schema 6 = current, drift only
...
work_a2=$(mktmp)
_mk_project "$work_a2" 6 TASK-001
...
work_b=$(mktmp)
_mk_project "$work_b" 3 ""  # schema 3 → 6 ⇒ migration_needed
...
assert d.get("schemaVersion") == 6, d
PY
    then
        b_ok=1
    fi
