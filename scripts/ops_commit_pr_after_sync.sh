#!/usr/bin/env bash
# Issue #108 smoketest — commit+PR flow после /polisade:migrate|sync,
# anti-patterns в /polisade:pr.
#
...
MIGRATE="$REPO_ROOT/scripts/polisade_migrate.py"
SYNC="$REPO_ROOT/scripts/polisade_sync.py"
LINT="$REPO_ROOT/scripts/polisade_lint_skills.py"
...
_p "scripts: polisade_migrate.py / polisade_sync.py / polisade_lint_skills.py"
...
work_a=$(mktmp)
_mk_project "$work_a" 7 TASK-001  # schema 7 = current, drift only
...
work_a2=$(mktmp)
_mk_project "$work_a2" 7 TASK-001
...
work_b=$(mktmp)
_mk_project "$work_b" 3 ""  # schema 3 → 7 ⇒ migration_needed
...
assert d.get("schemaVersion") == 7, d
PY
    then
        b_ok=1
    fi
...
forbidden = [
    ...
    (re.compile(r"\bgit push origin\b"),
     "bare `git push origin` (must go through polisade_vcs.py git-push)"),
...
#       polisade_pr.py, BITBUCKET_DOMAIN, $(...) command substitution.
...
if [ "$RUN_RUNTIME" = "0" ] || [ "$SKIP_INSTALLED" = "1" ]; then
    ...
    qwen_ext_dir="$HOME/.qwen/extensions/polisade"
    if [ ! -d "$qwen_ext_dir" ]; then
        ...
        # PR через polisade_vcs.py pr-create к bare remote не откроется (нет
        # gh API), но цель — проверить выбор пути агентом, не сам PR.
        d_root=$(mktmp)
        d_remote="$d_root.git"
        _mk_project "$d_root" 7 TASK-001
        ...
        d_prompt="После /polisade:sync --apply в текущем каталоге есть незакоммиченные изменения. Закоммить их в отдельной ветке и открой PR в main. Не используй bare git push — используй polisade_vcs.py. Body PR — через файл, не через command substitution. Покажи команды, которые ты вызываешь."
        ...
        _d_check_neg "polisade_pr\.py"               "polisade_pr.py"
        ...
        _d_check_pos "polisade_vcs\.py git-push"     "polisade_vcs.py git-push"
        _d_check_pos "polisade_vcs\.py.*pr-create"   "polisade_vcs.py pr-create"
