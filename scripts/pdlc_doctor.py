...
def check_codex_cli():
    ...
    with detect_available() in pdlc_cli_caps.py — otherwise doctor would
    disagree with `pdlc_cli_caps.py detect`.
    """
    label = "codex_cli"
    try:
        from pdlc_cli_caps import _identity_ok
...
def _format_ambiguous_text(ambiguous_refs):
    ...
    lines.append("Run /pdlc:migrate --apply to attach scope prefixes automatically.")
    lines.append("-" * 60)
    return lines
...
def run_cli_caps(root, fmt):
    ...
    # a slow/hanging codex binary doubles PDLC_IDENTITY_TIMEOUT wall time.
