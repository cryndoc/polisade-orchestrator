...
#: Признаки того, что путь ведёт в КОРЕНЬ ПРОЕКТА, а не в staging.
#: V3-S3.33: этот скрипт пишет корпус-ОБРАЗНОЕ дерево
#: (`DIR/docs/architecture/<target>`), и до полосы ничто не мешало передать
#: сюда корень репозитория — тогда минимально-валидные заготовки легли бы
#: прямо в живой корпус мимо `polisade_corpus_io.py`, без атомарности,
#: блокировки, журнала и backup. Обещание «в корпус не пишу» перестаёт быть
#: словом и становится проверкой.
_PROJECT_ROOT_MARKERS = (
    Path(".state") / "PROJECT_STATE.json",
    Path(".git"),
)

#: Живой корпус. Ни один выход этого скрипта не имеет права оказаться внутри.
_CORPUS_TAIL = ("docs", "architecture")


def _real_of(path):
    """`realpath` пути, которого может ещё не быть на диске.

    Разрешаем ближайшего СУЩЕСТВУЮЩЕГО предка и приклеиваем остаток: иначе
    симлинк-компонент (`stage/docs -> <project>/docs`) сравнивался бы по имени
    и проходил (находка ревью круга 2).
    """
    target = Path(os.path.abspath(str(path)))
    probe = target
    while not probe.exists() and probe.parent != probe:
        probe = probe.parent
    real = os.path.realpath(str(probe))
    tail = os.path.relpath(str(target), str(probe))
    if tail in (".", ""):
        return real
    return os.path.normpath(os.path.join(real, tail))


def _project_root_of(real_path):
    """Ближайший предок, похожий на корень проекта, или `None`.

    Нужен, чтобы отличить «staging внутри проекта» (норма) от «staging, чей
    путь после разрешения симлинков ведёт в живой корпус этого проекта»
    (дефект). Сравнение по именам этого не различает.
    """
...
def _sink_refusal(path, *, flag, corpus_relative):
    """`None`, если писать сюда можно; иначе текст отказа.

    Единый fail-closed guard на ВСЕ выходы скрипта. Первая редакция проверяла
    только `--content-dir`, и `--emit-plan docs/architecture/manifest.yaml`
    спокойно затирал манифест корпуса мимо примитива — тот же дефект, только
    через соседний флаг (находка ревью круга 2, оба ревьюера).

    `corpus_relative=True` — путь сам является staging-корнем, под которым
    скрипт создаст `docs/architecture/...`. Тогда проверяется не форма пути
    (она у staging и у корпуса одинаковая — на то он и staging), а КУДА он
    ведёт после разрешения симлинков: совпал с живым корпусом своего проекта —
    отказ. Staging внутри проекта (`.polisade/tmp/...`) при этом остаётся
    законным, иначе guard ломал бы штатный вызов.
    """
    # Хвост приклеивается ДО разрешения: иначе симлинк ВНУТРИ пути
    # (`stage/docs -> <project>/docs`) не резолвится вовсе — сам `stage`
    # существует и разрешается в себя, а подмена сидит на следующем
    # компоненте. Это и был рабочий обход, найденный ревью круга 2.
    real = _real_of(Path(path).joinpath(*_CORPUS_TAIL) if corpus_relative
                    else Path(path))
    root = _project_root_of(real)
    if root is not None:
        corpus = os.path.realpath(str(root / Path(*_CORPUS_TAIL))) \
            if (root / Path(*_CORPUS_TAIL)).exists() \
            else os.path.normpath(str(root / Path(*_CORPUS_TAIL)))
        if _inside(real, corpus):
            return (
                "%s %s ведёт в ЖИВОЙ КОРПУС проекта %s (фактический путь: %s). "
                "Выходы этого скрипта — план и заготовки в staging; корпус "
                "пишет только polisade_corpus_io.py, он даёт атомарность, "
                "блокировку, журнал и backup. Дай путь вне %s/."
                % (flag, path, root, real, "/".join(_CORPUS_TAIL)))
    if not corpus_relative:
        parts = Path(real).parts
        for i in range(len(parts) - 1):
            if parts[i] == _CORPUS_TAIL[0] and parts[i + 1] == _CORPUS_TAIL[1]:
                return (
                    "%s %s ведёт внутрь %s/ (фактический путь: %s). План и "
                    "`decided` — не корпусные артефакты; дай путь вне корпуса."
                    % (flag, path, "/".join(_CORPUS_TAIL), real))
    return None
...
def main(argv=None):
    ...
    # V3-S3.33: проверяем ДО разбора спеки — отказ по вызову не должен зависеть
    # от того, окажется ли §5 валидной. Проверяются ВСЕ три выхода: оставить
    # хоть один без guard'а значило бы перенести дыру на соседний флаг.
    for flag, value, corpus_rel in (("--content-dir", args.content_dir, True),
                                    ("--emit-plan", args.emit_plan, False),
                                    ("--decided", args.decided, False)):
        if not value:
            continue
        refusal = _sink_refusal(value, flag=flag, corpus_relative=corpus_rel)
        if refusal:
            print("usage: %s" % refusal, file=sys.stderr)
            return 2
