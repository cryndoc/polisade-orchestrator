...
CURRENT_POLISADE_VERSION = "3.7.5"
...
def compute_pm_questions(state, root, adopt_v2=False):
    ...
    if adopt_v2:
        return questions
    exp = ((state.get("settings") or {}).get("experimental") or {})
...
def compute_migrations(state, root, adopt_v2=False):
    ...
    exp_now = ((state.get("settings") or {}).get("experimental") or {})
    ...
    if not isinstance(arch_now, dict) or not isinstance(arch_now.get("corpus"), dict):
        ...
        migrations.append(Migration(
            "Add architecture.corpus { dir, manifest, mode: %s, pendingRun } "
        ...
        def relocate_adrs(s, moves=adr_moves, manifests=adr_manifest_refs,
            ...
            for container in ("artifactIndex", "artifacts"):
                ...
                if not isinstance(idx, dict):
                    continue
                for k, v in idx.items():
                    if not (k.startswith("ADR-") and isinstance(v, dict)
                            and isinstance(v.get("path", ""), str)
                            and v["path"].startswith("docs/adr/")):
                        continue
                    name = v["path"][len("docs/adr/"):]
