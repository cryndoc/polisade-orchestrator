...
from _polisade_env import env_get  # noqa: E402  (#330)
...
CURRENT_POLISADE_VERSION = "3.8.1"
        ...
        def apply_req_scope(s, plan=req_scope_plan):
            ...
            if plan["summary"]["unresolved"] > 0:
                ...
                sys.stderr.write(
                    ...
                    "automatically. Run `${POLISADE_PYTHON:-python3} "
                    "%s --traceability` to inspect `ambiguous_refs`.\n"
...
def compute_settings_migrations(root):
    ...
    if template is None and current is not None:
        ...
        _note_malformed(
            ...
            "`.claude/settings.json` НЕ рассматривалась (issue #330): "
...
# ДВА, потому что скрипт запускается из двух разных мест:
...
#   * исходник плагина — читается везде, где нет Filesystem Guard
#     (Claude Code, Qwen, opencode и регрессия этого репозитория).
# Оба пути СОБРАНЫ из констант, а не записаны литералом: имя вендоренного
...
def _canonical_docs_templates():
    """Вернуть {имя: байты} канонических шаблонов артефактов или None.
...
_FLAG_EFFECTS = {
    ...
    "onboard": (
        "НИЧЕГО: флаг инертен с 3.6.0 (полоса V3-P2, ADR-0004) — команда "
...
def mode_report(root, state, planned_paths=()):
    ...
    for flag in sorted(V2_FLAG_DEFAULTS):
        effect, needs = _FLAG_EFFECTS.get(flag, ("", ()))
        requires = []
        for rel in needs:
            target = root / rel
            if target.exists():
                status = "ok"
            elif rel in planned or str(target) in planned:
                status = "will-arrive"
            else:
                status = "missing"
...
def compute_mode_switch_migrations(state, enable=(), disable=()):
    ...
    migrations.append(Migration(
        "Set settings.experimental: %s (--enable/--disable, #339)"
...
def compute_docs_templates_migrations(root):
    ...
    * шаблон ОТЛИЧАЕТСЯ — вопрос PM, НИКОГДА не перезапись. Правка шаблона
    ...
    canonical = _canonical_docs_templates()
    if canonical is None:
        _note_malformed(
            ...
            "рассматривались (issue #341): сверить проект не с чем.",
    ...
    if dest_dir.is_symlink() or (dest_dir.exists() and not dest_dir.is_dir()):
        ...
        _note_malformed(
            ...
            "`docs/templates` — не каталог (символическая ссылка или файл), "
            "поэтому шаблоны артефактов не доставлялись (issue #341).",
            "docs/templates", kind="malformed-state-field")
        return migrations
    ...
    for name in sorted(canonical):
        ...
        try:
            ...
            _note_malformed(
                ...
                "каноном не проводилась (issue #341)."
        ...
        if current is None:
            ...
            _note_malformed(
                ...
                "плагина. Он НЕ перезаписан: правка шаблона командой — "
                "источник, а не производная (issue #341)." % name,
                "docs/templates/%s" % name, kind="malformed-state-field")
...
def compute_polisade_tmp_gitignore_migrations(root):
    ...
    wanted = (
        ...
        (r'^\s*\.polisade/\*\*/__pycache__/?\s*$', ".polisade/**/__pycache__/",
         "\n# Bytecode of the vendored runtime scripts (#127/#297): running any\n"
         "# of them writes it, and an untracked entry here stops the post-apply\n"
         "# commit recipe on its own safety-net — issue #332.\n"
         ".polisade/**/__pycache__/\n",
         "Append `.polisade/**/__pycache__/` to .gitignore (issue #332)"),
    )
...
def main():
    ...
    except ModeSwitchRefusal as exc:
        print(json.dumps({
            "status": "refused",
            "detail": str(exc),
            "touched_paths": [],
    ...
    docs_templates_migrations = compute_docs_templates_migrations(root)
    ...
    if not migrations:
        ...
        print(json.dumps({
            ...
            "experimental_modes": mode_report(root, state),
        }, indent=2, ensure_ascii=False))
        sys.exit(0)
    ...
    if not apply:
        ...
        print(json.dumps({
            ...
            "experimental_modes": mode_report(root, state, touched_rel),
        }, indent=2, ensure_ascii=False))
        sys.exit(0)
