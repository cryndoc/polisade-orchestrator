...
CURRENT_POLISADE_VERSION = "3.7.9"
...
def _record_fix_hint(container):
    ...
    repairs a broken record there. It does NOT touch `artifacts` — that block
    is the deprecated pre-schema-3 field which current code never writes — so
    printing the same advice for both would send the reader to a command that
    cannot help. (This is why the two containers are reported separately
    rather than deduplicated by ADR id: they need different remedies.)
    """
    ...
    return ("Блок `artifacts` устарел (до схемы 3) и текущим кодом не пишется, "
            "`/polisade:sync` его не чинит — запись правится вручную или удаляется.")
...
def _note_malformed(note_id, problem, container,
    ...
    STATE_SHAPE_NOTES.append({
        ...
        "question": "%s%s %s" % (
            problem,
            "" if (container == "schemaVersion" or container.endswith(".json"))
            else " Перенос legacy-ADR по ней не проверялся.",
...
def main():
    ...
    # #290 — everything the planners could not read: ADR records, unusable
    # containers, malformed top-level fields, unreadable neighbouring state
    # files. Folded AFTER every planner has run (an earlier fold silently
    # dropped the notes raised by the two below) and BEFORE any output, so the
    # findings surface in dry-run too — the whole point is that they are seen
    # BEFORE --apply.
