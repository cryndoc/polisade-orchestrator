...
CURRENT_POLISADE_VERSION = "3.7.17"
...
def compute_acceptance_baseline_gitignore_migrations(root):
    """Let the ratified acceptance baseline travel with the repo (#299).

    `.state/acceptance-baseline.json` records WHAT WAS RATIFIED: the digest of
    every acceptance pair plus the digests of the instrument files those checks
    run. Both halves matter — editing the test file is the second way to buy
    green — and neither can be recomputed from the current tree, because that is
    exactly what a baseline is compared against.

    While the file stayed local, `--fail-on-changed` still refused rather than
    passing quietly (it needs a baseline), so nothing went silently green. What
    was missing is worse in a team: only the person who last ran acceptance on
    their own machine could see that a check had been weakened. A colleague and
    CI had nothing to compare against at all.

    Placed immediately AFTER the knowledge re-include, and only where that
    position is REACHABLE. Git keeps the last matching rule, so the same two
    shapes #292 refuses to repair defeat this line too: a re-include placed
    before its exclusion, and an interleaved `.state/` further down. Inheriting
    the position without inheriting the test was the first draft of this
    function, and it reproduced the #292 bug one file over — announcing a rule
    it then wrote where git would never reach it. The refusal is silent here on
    purpose: #292 already asks the PM about that file, and the rule order is
    one decision, not two.

    If the exclusion is still the broken bare `.state/` form, the #292
    migration repairs it in the same run — both re-read the file when they
    apply, so they compose in either order.

    Idempotent: the line is added only when it is absent.
    """
    migrations = []
    gitignore_dst = root / ".gitignore"
    ...
    if not reincluded:
        ...
        return migrations
    if any(_BASELINE_REINCLUDE_RE.match(line) for line in lines):
        return migrations
    excluded = [i for i, line in enumerate(lines) if _STATE_EXCLUDE_RE.match(line)]
    if excluded and max(excluded) > max(reincluded):
        # The position we would write to is unreachable: an exclusion after the
        # last re-include wins over anything above it. #292 names this file to
        # the PM; adding a second rule there would only be a second dead line.
        return migrations

    def add_baseline_reinclude(s, path=gitignore_dst):
        current, raw = _read_gitignore(path)
        if any(_BASELINE_REINCLUDE_RE.match(line) for line in current):
            return
        ...
        eol = anchor[len(body):]
        if not eol:
            # The anchor is the last line and carries no terminator. Inserting
            # after it without adding one glues the two rules together —
            # `!.state/knowledge.json!.state/acceptance-baseline.json` — and
            # destroys BOTH. Live in an already-migrated project, where the
            # sibling append that would have fixed the terminator does not run.
            eol = next((current[i][len(current[i].rstrip("\r\n")):]
                        for i in range(at - 1, -1, -1)
                        if current[i].rstrip("\r\n") != current[i]), None) \
                or _dominant_eol(raw)
            current[at] = body + eol
        current.insert(at + 1, "!.state/acceptance-baseline.json" + eol)
        _write_gitignore(path, current)

    migrations.append(Migration(
        "Add .gitignore: `!.state/acceptance-baseline.json` so the ratified "
        "acceptance baseline travels with the repo (#299)",
        add_baseline_reinclude,
        [gitignore_dst],
    ))
    return migrations
