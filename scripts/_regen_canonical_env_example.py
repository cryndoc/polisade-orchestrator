...
def main() -> int:
    ...
    if not pattern.search(src):
        ...
        return 2
    # The replacement is passed as a callable, NOT as a template string:
    # `repr()` emits `\n` escapes and `re.sub` would expand them back into
    # real newlines, exploding the one-line literal into an unterminated
    # multi-line string. A callable's return value is used verbatim.
    new_src = pattern.sub(lambda _m: block, src, count=1)
