...
def main():
    ...
    # at once, and committing the counter does not help — the protocol keeps it
    # monotone above DELETED artefacts, so a fresh clone disagrees with the
    ...
    numbering = {"assigned": [], "pending": [], "failed": []}
    ...
    if plan and numbering["on_trunk"] and apply:
        ...
        numbering["references"] = polisade_id.rewrite_references(
            root, {row["from"]: row["to"] for row in done})
