...
Usage:
    python3 scripts/polisade_lint_artifacts.py [project_root]
...
- SPEC: load/perf NFR (Verification column) is referenced by some TASK (#34)
...
# Frontmatter `kind:` values this linter must NOT validate with ISO-29148 rules
# (#232). Each has its own grammar and its own linter:
#   change-spec → scripts/polisade_spec_lint.py (6 sections, inline EARS
#                 Statement, §3 localization). Pipeline V2 Ф2, #211 / WP2.3.
# An artefact WITHOUT `kind` is legacy ISO-29148 and stays on the ISO path —
# that back-compat is what keeps existing projects working without migration.
_NON_ISO_SPEC_KINDS = frozenset({'change-spec'})
...
# Issue #34 — «эту NFR проверяют нагрузочным тестом». Ищется ТОЛЬКО в колонке
# Verification (последняя ячейка строки §6), никогда в формулировке: «p99
# latency < 200 мс» — это ЧТО требуется, а не ЧЕМ это проверяют, и warn по
# такой строке был бы ложным на каждой второй перформанс-NFR.
#
# Токены — ЦЕЛЫЕ слова, а не подстроки: голое `perf` матчило `perfectly`
# («response is perfectly rounded» в колонке Verification давало ложный WARN и,
# по тому же правилу, лишний 900-секундный гейт). `нагруз` остаётся префиксом
# намеренно — это русский корень с падежными окончаниями («нагрузочный тест»,
# «под нагрузкой»), и \b перед ним уже отсекает `перегруз`.
LOAD_VERIFICATION_RE = re.compile(
    r'\b(?:load|perf|performance|latency)\b|\bнагруз', re.IGNORECASE)
...
def extract_load_nfr_ids(content):
    """Return NFR ids whose **Verification** cell names a load/perf test (#34).

    The Verification column is the LAST non-empty cell of the `| NFR-NNN | … |`
    row — that is the whole rule. Everything before it (category, statement) is
    what the requirement demands, not how it is checked, so scanning it would
    fire on every latency requirement ever written.
    """
    load_ids = []
    for nfr_id, rest in extract_nfr_rows(content):
        cells = [c.strip() for c in rest.split('|')]
        cells = [c for c in cells if c]
        if not cells:
            continue
        if LOAD_VERIFICATION_RE.search(cells[-1]):
            load_ids.append(nfr_id)
    return load_ids


def collect_task_requirement_refs(root, req_index):
    """Return the set of COMPOSITE `DOC.REQ` ids covered by some TASK.

    Both frontmatter forms are legal (see «Requirement ID Scoping»): a TASK may
    write `NFR-002` when one top-level doc declares it, or `SPEC-001.NFR-002`
    otherwise. A bare id is therefore **scoped**, not global — resolved against
    the TASK's own `parent` chain first, and only then, if that leads nowhere,
    against the requirement index when exactly one doc declares the id. Without
    that scoping a single TASK on `SPEC-001.NFR-001` would silence the warning
    for a same-numbered NFR in every other SPEC (adversarial review, round 1).
    An ambiguous bare id covers nothing — the linter already errors on it and
    demands the composite form.

    Резолв не самодельный: это тот же `normalize_ref`, которым `_check_ref`
    проверяет ссылки TASK/ADR. Второй резолвер разошёлся бы с первым, и
    предупреждение стало бы противоречить ошибке на том же файле.
    """
    covered = set()
    ...
    for task_file in sorted(tasks_dir.glob("TASK-*.md")):
        ...
        if isinstance(reqs, str):
            reqs = [r.strip() for r in reqs.strip('[]').split(',') if r.strip()]
        for raw in reqs:
            ref = str(raw).strip().strip('"\'')
            if not ref:
                continue
            info = normalize_ref(ref, fm, root, req_index)
            if info["composite"]:
                covered.add(info["composite"])
    return covered
...
def lint_spec(path, content):
    ...
    # kind-gate (#232): this linter encodes ISO-29148 form (blockquote
    # Statement, Given-When-Then). A `kind: change-spec` artefact has its own
    # 6-section grammar and its own linter — scripts/polisade_spec_lint.py.
    # Без гейта два линтера дают противоположный вердикт на одном файле.
    # Артефакты без `kind` — legacy ISO, обрабатываются как раньше.
    if fm.get('kind') in _NON_ISO_SPEC_KINDS:
        return issues, [], []
...
def _format_collision_error(canon, defined_in):
    ...
    return (
        ...
        f"Use composite {hint_prefix}.{canon} or run /polisade:migrate --apply"
    )
    ...
    if is_legacy_two_digit(raw_ref):
        issues.append({
            ...
            "message": (
                ...
                f"in {where}, run /polisade:migrate --apply to canonicalize"
            ),
        })
...
def _parse_manifest_artifacts(text):
    ...
    Uses the same light parser shape as polisade_doctor to stay stdlib-only.
    """
    out = []
...
def main():
    ...
    # ── 2.4. Load/perf NFR without a TASK that references it (#34) ──
    # Гейт производительности (шаг 2h в /polisade:implement) стреляет по TASK,
    # ссылающейся на такую NFR. Если ни одна TASK на неё не ссылается, гейт не
    # выстрелит никогда — «verification: load test» останется словом в таблице.
    # Это WARN, а не error: декомпозиция могла ещё не дойти до этой NFR.
    # ГРАНИЦА: сканируются только SPEC в docs/specs/ по ISO-форме (у
    # `kind: change-spec` своя грамматика и свой линтер) и только frontmatter
    # `requirements:` в tasks/TASK-*.md — упоминание NFR в прозе таска не
    # считается ссылкой, потому что по прозе гейт не срабатывает тоже.
    if specs_dir.is_dir():
        covered_reqs = collect_task_requirement_refs(root, req_index)
        for spec_file in sorted(specs_dir.glob("SPEC-*.md")):
            try:
                content = spec_file.read_text()
            except IOError:
            ...
            if fm.get('kind') in _NON_ISO_SPEC_KINDS:
                continue
            doc_id = fm.get('id', '') or spec_file.stem.split('-')[0]
            unverified = [nfr_id for nfr_id in extract_load_nfr_ids(content)
                          if f"{doc_id}.{nfr_id}" not in covered_reqs]
            if unverified:
                issues = [{
                    "level": "warn",
                    "message": (
                        f"{nfr_id}: verification is a load/perf test but no TASK "
                        f"references it — performance verification is unplanned "
                        f"(#34)"
                    ),
                } for nfr_id in unverified]
    ...
    # Scans the same directory set as polisade_sync.scan_artifacts — any duplicate
    ...
    dup_scan_dirs = [
        ...
        # ADR handled separately below (new+legacy relocation dirs, #187).
    ]
    ...
    # ADR global-id handling with #187 prefer-new back-compat. A given ADR id
    # may legitimately exist in BOTH the new (docs/architecture/decisions) and
    # legacy (docs/adr) dir during the relocation window — that is a prefer-new
    # shadow (warn), not a hard duplicate. Two files sharing an id WITHIN one
    # dir is a real OPS-023 violation (error, via the generic reporter below).
    adr_loc = {}  # id -> {"new": [paths], "legacy": [paths]}
    for kind, rel in (("new", ADR_DIR), ("legacy", ADR_LEGACY_DIR)):
        d = root / rel
        if not d.is_dir():
            continue
        for f in sorted(d.glob("ADR-*.md")):
            ...
            if not art_id or art_id.endswith("-XXX"):
                continue
            adr_loc.setdefault(art_id, {"new": [], "legacy": []})[kind].append(
                str(f.relative_to(root)))
    for art_id, loc in sorted(adr_loc.items()):
        new_p, leg_p = loc["new"], loc["legacy"]
        if len(new_p) > 1 or len(leg_p) > 1:
            # Real within-location duplicate → feed the generic error reporter.
            id_to_paths.setdefault(art_id, []).extend(new_p + leg_p)
        elif new_p and leg_p:
            # Relocation shadow: readers prefer the new path.
            results.append({
                "artifact": art_id,
                "type": "ADR",
                "status": "warn",
                "issues": [{
                    "level": "warn",
                    "message": (
                        f"{art_id} present in both new ({new_p[0]}) and legacy "
                        f"({leg_p[0]}) ADR location — readers prefer the new path; "
                        f"remove the legacy copy once migration is complete (#187)."
                    ),
                }],
            })
            total_warnings += 1
        else:
            # Single location → register for cross-type id uniqueness.
            id_to_paths.setdefault(art_id, []).extend(new_p + leg_p)
    ...
    # ── 2b. Detect misplaced non-TASK artifacts (#162) ──────────
    for misplaced_file, canonical_dir in find_misplaced_artifact_files(root):
        rel = misplaced_file.relative_to(root)
        prefix = misplaced_file.name.split("-", 1)[0]
        issue = {
            "level": "error",
            "message": (
                f"{prefix} file in wrong location: {rel} "
                f"— must be in {canonical_dir}/. "
                f"Fix: mkdir -p {canonical_dir} && mv {rel} {canonical_dir}/ "
                f"&& python3 scripts/polisade_sync.py ."
            ),
        }
        results.append({
            "artifact": misplaced_file.name,
    ...
    # ── 3. Lint ADRs (new + legacy relocation dirs, prefer-new — #187) ──
    ...
    # ── 4b. Living architecture corpus rules (#187, scoped) ────
    # Only active when the corpus model dir exists (living mode). Silo projects
    # have no docs/architecture/model/, so this is a no-op for them. Deep
    # enforcement (dangling $ref, FR-coverage fold) is the weak-model-
    # orchestrator spec (#133/#184/#135); here we guard the one cheap, decisive
    # corpus invariant: one entity, defined once (the per-SPEC-silo drift bug).
    model_entities = root / "docs" / "architecture" / "model" / "entities"
    if model_entities.is_dir():
        entity_homes = {}  # entity name (stem) → [paths]
        for f in sorted(model_entities.glob("*.yaml")):
            entity_homes.setdefault(f.stem, []).append(str(f.relative_to(root)))
        for name, paths in sorted(entity_homes.items()):
            if len(paths) > 1:
                results.append({
                    "artifact": f"model/entities/{name}",
                    "type": "CORPUS",
                    "status": "fail",
                    "issues": [{
                        "level": "error",
                        "message": (
                            f"entity {name!r} defined in >1 corpus file "
                            f"({', '.join(paths)}) — the corpus must have one "
                            f"home per entity (#187). Merge into a single file."
                        ),
                    }],
                })
                total_errors += 1
