...
Что примитив ДАЁТ (механика, проверяется тестами):
  ...
  * **хэш-контракт цели** (полоса V3-S3.33) — вызывающий может объявить, каким
    он ВИДЕЛ существующий файл (`--expect-sha256`) или что его не было
    (`--expect-absent`), и примитив сверяет это САМ, вплотную к записи, под уже
    взятой блокировкой операции. Ключ сверки берётся из ПРОЧИТАННЫХ байтов
    цели (не из mtime, размера или пути), поэтому «файл тот же, но другой»
    невозможно. Расхождение — отказ, а не перезапись: чужая правка, попавшая в
    окно между планом вызывающего и записью, больше не исчезает молча.

ХЭШ-КОНТРАКТ ПОДРОБНО (что именно обещано):

  * `--expect-absent` на POSIX публикует файл через `linkat` — «создать, если
    нет» исполняет ЯДРО, и окна нет вовсе: цель, появившаяся за микросекунду
    до публикации, даёт `EEXIST`, а не тихую перезапись;
  * `--expect-sha256` читает цель ЧЕРЕЗ ТОТ ЖЕ descriptor каталога, считает
    sha256 прочитанных байтов (чтение обрамлено `fstat` с обеих сторон, иначе
    свежий ключ тождества привязался бы к устаревшему дайджесту) и,
    непосредственно перед `replace`, повторно сверяет тождество файла
    (`dev`/`ino`/`size`/`mtime_ns`/`ctime_ns`).

    **Остаточное окно здесь ЕСТЬ, и оно не «пренебрежимое»: между последней
    сверкой и `rename` нет ни одной синхронизации.** Правка, легшая ровно в это
    окно, будет затёрта молча — `rename` в POSIX безусловен, условного обмена
    в stdlib нет. То есть `--expect-sha256` — это НАДЁЖНЫЙ ДЕТЕКТОР чужой
    правки, сделанной до финальной сверки, и НЕ барьер против писателя,
    работающего одновременно с публикацией. Первая редакция этого модуля
    описывала окно как «нужно вернуть все пять полей `stat`» — это было
    преуменьшением, снято по ревью полосы. Атомарен без оговорок только
    `--expect-absent` на POSIX (`linkat`). Названо вслух, потому что «почти
    закрыто» и «закрыто» — разные вещи;
  * контракт покрывает `write`, записи и удаления `promote`. **У удаления та
    же природа, что у `--expect-sha256`, и та же граница:** сверка идёт
    вплотную к `unlink`, но условного удаления в POSIX нет, поэтому файл,
    подменённый между сверкой и `unlink`, будет снят. Это детектор чужой
    правки, а не барьер против одновременного писателя — сказано здесь, чтобы
    фраза «контракт покрывает удаления» не читалась сильнее, чем есть.
    `restore` контракта НЕ принимает намеренно: его смысл — вернуть корпус к
    состоянию backup КАКИМ БЫ корпус ни стал, и ожидание конкретных байтов
    цели противоречило бы самой операции.
...
Exit codes:
    ...
    1 — предметный отказ: блокировка занята / операция уже идёт / оборванная
        ...
        на пути / побег пути / нечего восстанавливать / **разошёлся
        хэш-контракт цели** (status: требуется внимание). Корпус при отказе
        НЕ тронут.
    2 — usage: нет обязательного аргумента, нет staging/backup, не директория,
        неразборный `--expect-sha256` / `--expect-from`
"""
...
#: Значение хэш-контракта, означающее «цели не должно существовать».
EXPECT_ABSENT = "absent"

#: Можно ли опубликовать новый файл через `linkat`. Это единственный способ
#: закрыть окно «проверили, что цели нет» → «записали» ПОЛНОСТЬЮ: решение
#: принимает ядро (`EEXIST`), а не наша проверка секундой раньше. Где этого
#: нет — контракт `absent` проверяется нами и честно помечается как более
#: слабый режим (`expectMode: absent-checked`).
_LINK_DIR_FD_OK = _DIR_FD_OK and os.link in os.supports_dir_fd
...
def atomic_write_bytes(target: Path, data: bytes, *, mode: int = 0o644,
                       expect=None, what: str = "запись") -> None:
    ...
    `expect` — хэш-контракт цели. В этом режиме окно уже, чем в dir-fd пути
    (сверка идёт по имени, а не по дескриптору), и это честно печатается как
    `pathMode: path-fallback`.
    """
    ...
    try:
        ...
        st = None
    still_key = None
    if expect == EXPECT_ABSENT:
        if st is not None:
            raise _expect_absent_error(target.name, what=what, raced=False)
    elif expect is not None:
        if st is None:
            raise _expect_mismatch_error(target.name, expect, None, what=what)
        # По ДЕСКРИПТОРУ, а не по имени: `_sha256(path)` + отдельный `lstat`
        # позволяли подменить inode между чтением и снятием ключа, и ключ
        # СВЕЖЕГО файла оказывался привязан к дайджесту СТАРОГО (находка ревью
        # круга 2). Тождество берётся у того самого дескриптора, что читали.
        try:
            fd_t = os.open(str(target), os.O_RDONLY)
        except OSError:
            raise _expect_mismatch_error(target.name, expect, None, what=what)
        try:
            st_before = os.fstat(fd_t)
            h = hashlib.sha256()
            while True:
                chunk = os.read(fd_t, 65536)
                if not chunk:
                    break
                h.update(chunk)
            st_after = os.fstat(fd_t)
        finally:
            os.close(fd_t)
        if _stat_key(st_before) != _stat_key(st_after):
            raise CorpusError(
                "E-expect-raced",
                "%s: цель %s менялась ПРЯМО ВО ВРЕМЯ чтения" % (what, target.name),
                hint="дайджест такого чтения ничего не значит — прогон остановлен",
                details={"path": target.name, "expected": expect},
            )
        actual = h.hexdigest()
        if actual != expect:
            raise _expect_mismatch_error(target.name, expect, actual, what=what)
        still_key = _stat_key(st_after)
    ...
    try:
        ...
        # Поздняя сверка и здесь: в первой редакции fallback-ветка шла
        # «дайджест → replace» вообще без повторной проверки (ревью круга 1).
        if expect == EXPECT_ABSENT:
            if target.exists() or target.is_symlink():
                raise _expect_absent_error(target.name, what=what, raced=True)
        elif still_key is not None:
            try:
                now_key = _stat_key(target.lstat())
            except OSError:
                now_key = None
            if now_key != still_key:
                raise CorpusError(
                    "E-expect-raced",
                    "%s: цель %s изменилась в момент записи" % (what, target.name),
                    hint="публикация отменена — байты вызывающего не затёрли "
                         "чужую правку",
                    details={"path": target.name, "expected": expect},
                )
...
# ---------------------------------------------------------------------------
# Хэш-контракт цели: «я видел файл вот таким» / «файла не было»
# ---------------------------------------------------------------------------


def normalise_expect(value, *, what: str):
    """`None` | `EXPECT_ABSENT` | 64-символьный hex. Мусор — UsageError.

    Молча трактовать неразборное ожидание как «без ожидания» нельзя: вызывающий
    думал бы, что защищён, а защиты не было. Поэтому любая непонятная строка —
    ошибка вызова (exit 2), а не тихое ослабление контракта.
    """
    if value is None:
        return None
    raw = str(value).strip()
    if not raw:
        raise UsageError("%s: пустое ожидание — убери аргумент или дай sha256"
                         % what)
    low = raw.lower()
    if low == EXPECT_ABSENT:
        return EXPECT_ABSENT
    if len(low) == 64 and all(c in "0123456789abcdef" for c in low):
        return low
    raise UsageError(
        "%s: ожидание должно быть sha256 (64 hex) или `%s`, получено %r"
        % (what, EXPECT_ABSENT, raw))


def _expect_absent_error(rel: str, *, what: str, raced: bool) -> CorpusError:
    return CorpusError(
        "E-expect-raced" if raced else "E-expect-present",
        "%s: цель %s существует, а вызывающий объявил её отсутствующей%s"
        % (what, rel, " (появилась в момент записи)" if raced else ""),
        hint="файл создан кем-то другим между планом и записью. Перезапись "
             "стёрла бы чужую работу — прогон остановлен. Разберись, чей это "
             "файл, и повтори осознанно (без --expect-absent или с "
             "--expect-sha256 его байтов)",
        details={"path": rel, "expected": EXPECT_ABSENT},
    )


def _expect_mismatch_error(rel: str, expected: str, actual, *,
                           what: str) -> CorpusError:
    if actual is None:
        return CorpusError(
            "E-expect-missing",
            "%s: цель %s исчезла — вызывающий ожидал файл с sha256 %s"
            % (what, rel, expected[:12]),
            hint="файл удалили между планом и записью. Записать поверх пустоты "
                 "значит воскресить документ, который кто-то снял намеренно — "
                 "прогон остановлен",
            details={"path": rel, "expected": expected, "actual": None},
        )
    return CorpusError(
        "E-expect-mismatch",
        "%s: цель %s изменилась между планом и записью (ожидалось sha256 %s, "
        "на диске %s)" % (what, rel, expected[:12], actual[:12]),
        hint="кто-то правил этот файл в корпусе. Перезапись потеряла бы его "
             "правку молча — прогон остановлен. Перечитай цель, реши, что "
             "делать с расхождением, и повтори с актуальным --expect-sha256",
        details={"path": rel, "expected": expected, "actual": actual},
    )


def _stat_key(st):
    """Тождество файла по пяти полям — то, что можно сравнить у fd и у имени."""
    return (st.st_dev, st.st_ino, st.st_size, st.st_mtime_ns, st.st_ctime_ns)


def _read_target_at(fd: int, name: str, *, what: str, rel: str, expected: str):
    """Прочитать цель ЧЕРЕЗ дескриптор каталога и сверить sha256 её байтов.

    Возвращает `(stat_key, mode)` прочитанного файла. Ключ сверки — из
    ПРОЧИТАННЫХ байтов: ни размер, ни mtime, ни путь ключом не являются,
    поэтому «файл того же размера, но другой» контракт не проходит.
    """
    try:
        tfd = os.open(name, os.O_RDONLY | os.O_NOFOLLOW, dir_fd=fd)
    except FileNotFoundError:
        raise _expect_mismatch_error(rel, expected, None, what=what)
        ...
        raise
    try:
        # Снимок ДО и ПОСЛЕ чтения. Один только `fstat` после чтения привязывал
        # бы СВЕЖИЙ ключ тождества к УСТАРЕВШЕМУ дайджесту: правка, легшая
        # между последним `read()` и `fstat`, выглядела бы как «файл не
        # менялся» (находка ревью круга 1).
        st_before = os.fstat(tfd)
        h = hashlib.sha256()
        while True:
            chunk = os.read(tfd, 65536)
            if not chunk:
                break
            h.update(chunk)
        st = os.fstat(tfd)
        ...
        os.close(tfd)
    if _stat_key(st_before) != _stat_key(st):
        raise CorpusError(
            "E-expect-raced",
            "%s: цель %s менялась ПРЯМО ВО ВРЕМЯ чтения" % (what, rel),
            hint="дайджест такого чтения ничего не значит — прогон остановлен",
            details={"path": rel, "expected": expected},
        )
    actual = h.hexdigest()
    if actual != expected:
        raise _expect_mismatch_error(rel, expected, actual, what=what)
    return _stat_key(st), stat.S_IMODE(st.st_mode)


def _assert_still(fd: int, name: str, key, *, what: str, rel: str,
                  expected: str) -> None:
    """Последняя сверка тождества цели ВПЛОТНУЮ к публикации.

    Между чтением байтов и `replace` цель могли подменить. Пять полей
    (`dev`/`ino`/`size`/`mtime_ns`/`ctime_ns`) ловят и перезапись на месте, и
    подмену переименованием. Писатель, вернувший все пять полей, не пойман —
    это названо в границе модуля, а не выдано за герметичность.
    """
    try:
        ...
        raise CorpusError(
            "E-expect-raced",
            "%s: цель %s исчезла в момент записи" % (what, rel),
            hint="кто-то удалил файл между сверкой и публикацией — прогон "
                 "остановлен, чтобы не воскресить снятый документ",
            details={"path": rel, "expected": expected},
        )
    if _stat_key(st) != key:
        raise CorpusError(
            "E-expect-raced",
            "%s: цель %s изменилась В МОМЕНТ записи" % (what, rel),
            hint="файл правил кто-то ещё, пока шла операция. Публикация "
                 "отменена — байты вызывающего не затёрли чужую правку",
            details={"path": rel, "expected": expected},
        )
...
def _atomic_write_rel(root: Path, rel: str, data: bytes, *, mode: int,
                      what: str, expect=None) -> str:
    """Атомарная запись `root/rel` без окна на подмену компонента пути.

    `expect` — хэш-контракт цели (`None` | `EXPECT_ABSENT` | sha256-hex).
    Возвращает режим контракта, применённый ФАКТИЧЕСКИ (`none` /
    `absent-atomic-link` / `absent-checked` / `sha256`), чтобы отчёт печатал
    силу гарантии, а не намерение вызывающего.
    """
    if not _DIR_FD_OK:
        target = _resolve_under(root, rel, what=what)
        atomic_write_bytes(target, data, mode=mode, expect=expect, what=what)
        return ("none" if expect is None
                else ("absent-checked" if expect == EXPECT_ABSENT else "sha256"))
    ...
    with _DirChain(root, dirs, what=what, create=True) as chain:
        ...
        # Хэш-контракт: сверка идёт ЗДЕСЬ — под уже взятой блокировкой
        # операции и по тому же дескриптору каталога, что и запись. Всё, что
        # вызывающий проверял раньше, к этому моменту могло устареть; именно
        # это окно контракт и закрывает.
        expect_mode = "none"
        still_key = None
        if expect == EXPECT_ABSENT:
            if st is not None:
                raise _expect_absent_error(rel, what=what, raced=False)
            expect_mode = ("absent-atomic-link" if _LINK_DIR_FD_OK
                           else "absent-checked")
        elif expect is not None:
            if st is None:
                raise _expect_mismatch_error(rel, expect, None, what=what)
            still_key, mode = _read_target_at(fd, name, what=what, rel=rel,
                                              expected=expect)
            expect_mode = "sha256"
        ...
        try:
            ...
            if expect_mode == "absent-atomic-link":
                # Решение «цели нет» принимает ЯДРО, а не наша проверка выше:
                # `linkat` на занятое имя даёт EEXIST. Окна нет вовсе.
                try:
                    os.link(tmp_name, name, src_dir_fd=fd, dst_dir_fd=fd)
                except FileExistsError:
                    raise _expect_absent_error(rel, what=what, raced=True)
                ...
                if expect == EXPECT_ABSENT:
                    ...
                    _assert_still(fd, name, still_key, what=what, rel=rel,
                                  expected=expect)
...
def _delete_rel(root: Path, rel: str, *, what: str, expect=None) -> bool:
    """Снять `root/rel`, если это обычный файл. Директорию не трогаем.

    `expect` — sha256 байтов, которые вызывающий рассчитывает удалить.
    Удаление тоже необратимо, поэтому «файл под этим именем стал другим» здесь
    останавливает прогон ровно так же, как и при записи. `EXPECT_ABSENT` для
    удаления бессмыслен и отвергается вызывающим (см. `_split_expect_map`).
    """
    if not _DIR_FD_OK:
        target = _resolve_under(root, rel, what=what)
        if target.is_symlink() or target.is_file():
            if expect is not None:
                if target.is_symlink():
                    raise CorpusError(
                        "E-symlink", "%s: цель удаления — симлинк (%s)"
                        % (what, rel))
                actual = _sha256(target)
                if actual != expect:
                    raise _expect_mismatch_error(rel, expect, actual, what=what)
            target.unlink()
    ...
    with chain:
        ...
        if expect is not None:
            still_key, _mode = _read_target_at(fd, name, what=what, rel=rel,
                                               expected=expect)
            _assert_still(fd, name, still_key, what=what, rel=rel,
                          expected=expect)
...
def _guard_previous_state(root: Path, *, force: bool, staging_hint):
    ...
    if state.get("state") in ("done", "aborted"):
        # `aborted` — прогон, отказавший ДО единственной записи в корпус.
        # Терминален так же, как `done`: следующему писателю разбирать нечего.
        return {"previousState": state.get("state")}
    if force:
...
def op_write(root: Path, rel: str, data: bytes, run_id: str, *,
             stale_after: int, force: bool = False, expect=None) -> dict:
    """Атомарно записать ОДИН файл инкремента под блокировкой.

    `expect` — хэш-контракт цели: `None` (без контракта, поведение как до
    V3-S3.33), `EXPECT_ABSENT` («файла не было») или sha256 существующего
    файла. Проверка идёт ВНУТРИ операции, а не до неё, — в этом весь смысл.
    """
    ...
    with _LockScope(root, run_id, stale_after, "write") as lock_info:
        ...
        expect_mode = _atomic_write_rel(root, norm, data, mode=0o644,
                                        what="запись", expect=expect)
    payload = {
        "ok": True,
        "op": "write",
        "path": norm,
...
def _split_expect_map(expect_map, write_rels, delete_rels, *, strict: bool):
    """Разложить карту ожиданий по фазам плана и отвергнуть бессмыслицу.

    Ключ, которого нет в плане, — это не «лишняя строчка»: вызывающий думает,
    что защитил путь, а путь вообще не пишется. Молча проглотить такой ключ
    значит выдать несуществующую защиту за существующую, поэтому — отказ.
    """
    if not expect_map:
        if strict and (write_rels or delete_rels):
            raise CorpusError(
                "E-expect-incomplete",
                "--expect-strict требует ожидание для каждого пути плана, а "
                "карта пуста",
                hint="дай --expect-from с ожиданиями (%d записей, %d удалений) "
                     "или убери --expect-strict"
                     % (len(write_rels), len(delete_rels)),
            )
        return {}, {}
    writes, dels, unplanned = {}, {}, []
    for raw_key, raw_value in expect_map.items():
        key = _normalise_rel(raw_key, what="ожидание")
        value = normalise_expect(raw_value, what="ожидание %s" % key)
        if key in write_rels:
            writes[key] = value
        elif key in delete_rels:
            if value == EXPECT_ABSENT:
                raise CorpusError(
                    "E-expect-bad-delete",
                    "ожидание `%s` для удаляемого пути %s бессмысленно"
                    % (EXPECT_ABSENT, key),
                    hint="удалять нечего, если файла нет; для удаления давай "
                         "sha256 байтов, которые рассчитываешь снять",
                )
            dels[key] = value
        else:
    ...
    if unplanned:
        raise CorpusError(
            "E-expect-unplanned",
            "ожидания заданы для путей, которых нет в плане: %s"
            % ", ".join(sorted(unplanned)[:10]),
            hint="такое ожидание НИЧЕГО не защищает — путь не пишется и не "
                 "удаляется. Поправь карту или план",
            details={"unplanned": sorted(unplanned)},
        )
    if strict:
        missing = sorted((write_rels - set(writes))
                         | (delete_rels - set(dels)))
        if missing:
            raise CorpusError(
                "E-expect-incomplete",
                "--expect-strict: без ожидания остались %d путь(ей): %s"
                % (len(missing), ", ".join(missing[:10])),
                hint="strict-режим обещает, что КАЖДЫЙ путь плана сверен; "
                     "допиши ожидания или сними --expect-strict",
                details={"missing": missing},
            )
    return writes, dels


def _precheck_expectations(root: Path, corpus_rel: str, expect_writes,
                           expect_deletes) -> None:
    """Сверить хэш-контракт ДО backup и ДО открытия журнала.

    Без этой проверки заведомо устаревшая карта ожиданий валила промоцию уже
    ПОСЛЕ того, как журнал переведён в `in-progress`: корпус оставался чистым,
    а проект — помеченным «оборванная промоция», и следующий прогон требовал
    `--force`. То есть корректный отказ контракта производил ложный диагноз
    повреждения. Здесь отказ приходит раньше первой служебной записи.

    Эта сверка НЕ заменяет ту, что идёт внутри записи: между ней и записью
    остаётся backup, и авторитетна именно поздняя проверка (по дескриптору,
    вплотную к публикации). Ранняя — про «не портить состояние зря».
    """
    for rel, expected in sorted(expect_writes.items()):
        target = _resolve_under(root, "%s/%s" % (corpus_rel, rel),
                                what="ожидание")
        exists = target.is_file()
        if expected == EXPECT_ABSENT:
            if os.path.lexists(str(target)):
                raise _expect_absent_error(rel, what="промоция", raced=False)
            continue
        if not exists:
            raise _expect_mismatch_error(rel, expected, None, what="промоция")
        actual = _sha256(target)
        if actual != expected:
            raise _expect_mismatch_error(rel, expected, actual, what="промоция")
    for rel, expected in sorted(expect_deletes.items()):
        target = _resolve_under(root, "%s/%s" % (corpus_rel, rel),
                                what="ожидание")
        if not target.is_file():
            raise _expect_mismatch_error(rel, expected, None, what="удаление")
        actual = _sha256(target)
        if actual != expected:
            raise _expect_mismatch_error(rel, expected, actual, what="удаление")


def op_promote(root: Path, staging: Path, run_id: str, *, corpus_rel: str,
               deletes, backup_dir, no_backup: bool, dry_run: bool, force: bool,
               stale_after: int, expect_map=None, expect_strict: bool = False) -> dict:
    ...
    delete_rels = {rel for rel, _ in plan_deletes}
    expect_writes, expect_deletes = _split_expect_map(
        expect_map, write_rels, delete_rels, strict=expect_strict)
    ...
    report = {
        ...
        "pathMode": PATH_MODE,
        # Сколько путей плана реально прикрыто хэш-контрактом, а сколько нет.
        # Без этих двух чисел «мы задали ожидания» читалось бы как «всё
        # сверено», хотя карта могла покрывать один путь из тридцати.
        "expectChecked": len(expect_writes) + len(expect_deletes),
        "expectUnchecked": (len(plan_writes) - len(expect_writes)
                            + len(plan_deletes) - len(expect_deletes)),
        "expectStrict": bool(expect_strict),
    }
    ...
    with _LockScope(root, run_id, stale_after, "promote") as lock_info:
        ...
        # Раньше backup и раньше журнала: расхождение контракта — это отказ
        # прогона, а не повреждение состояния проекта.
        _precheck_expectations(root, corpus_rel, expect_writes, expect_deletes)
        ...
        try:
            ...
            # Фаза backup: в корпус не записано НИ БАЙТА, значит «оборванная
            # промоция» тут — ложный диагноз.
            state["state"] = "aborted"
            state["phase"] = "aborted"
            state["promoted"] = 0
            state["deleted"] = 0
        ...
        try:
            for rel, _target in plan_writes:
                src = staging / rel
                expect_modes[rel] = _atomic_write_rel(
                    root, "%s/%s" % (corpus_rel, rel), src.read_bytes(),
                    mode=0o644, what="промоция",
                    expect=expect_writes.get(rel))
                promoted += 1
            state["phase"] = "delete"
            _atomic_write_json(_state_path(root), state)
            for rel, _target in plan_deletes:
                if _delete_rel(root, "%s/%s" % (corpus_rel, rel), what="удаление",
                               expect=expect_deletes.get(rel)):
                    deleted += 1
        except BaseException as exc:
            state["error"] = "%s: %s" % (type(exc).__name__, exc)
            state["interruptedAfterWrites"] = promoted
            state["interruptedAfterDeletes"] = deleted
            if promoted == 0 and deleted == 0:
                # Отказ ДО первой мутации (например, хэш-контракт первой же
                # цели). Корпус этой промоцией не менялся — метить его
                # смешанным значит требовать отката там, откатывать нечего.
                state["state"] = "aborted"
                state["phase"] = "aborted"
            try:
                _atomic_write_json(_state_path(root), state)
    ...
    # Режим печатается ФАКТИЧЕСКИЙ (что исполнило ядро), а не заказанный:
    # `absent-atomic-link` и `absent-checked` — разной силы гарантии, и
    # подменять вторую первой в отчёте значит врать о защите.
    report["expectModes"] = sorted({m for m in expect_modes.values()
                                    if m != "none"})
...
def op_status(root: Path, *, corpus_rel: str, staging_hint, stale_after: int,
    ...
    try:
        ...
        elif state.get("state") in ("done", "aborted"):
            payload["promote"] = {
...
_BOUNDARY = (
    ...
    "блокировка кооперативная; хэш-контракт цели работает ТОЛЬКО там, где "
    "вызывающий его задал (expectChecked/expectUnchecked в отчёте)"
)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    exp = p_write.add_mutually_exclusive_group()
    exp.add_argument("--expect-sha256", dest="expect_sha256", default=None,
                     help="хэш-контракт: цель обязана содержать ИМЕННО эти "
                          "байты в момент записи, иначе отказ")
    exp.add_argument("--expect-absent", dest="expect_absent",
                     action="store_true",
                     help="хэш-контракт: цели не должно существовать; "
                          "публикация идёт атомарным `linkat` (EEXIST → отказ)")
    ...
    p_prom.add_argument("--expect-from", dest="expect_from", default=None,
                        help="JSON-объект {путь-в-корпусе: sha256|absent} — "
                             "хэш-контракт целей; проверяется в момент записи")
    p_prom.add_argument("--expect-strict", dest="expect_strict",
                        action="store_true",
                        help="требовать ожидание для КАЖДОГО пути плана "
                             "(иначе непокрытые пути просто считаются)")
    ...
    try:
        ...
        if args.cmd == "write":
            if args.stdin:
                ...
                data = src.read_bytes()
            expect = (EXPECT_ABSENT if args.expect_absent
                      else normalise_expect(args.expect_sha256,
                                            what="--expect-sha256"))
        ...
        if args.cmd == "promote":
            ...
            expect_map = None
            if args.expect_from:
                exp_path = Path(args.expect_from)
                if not exp_path.is_file():
                    raise UsageError("--expect-from %s не файл" % exp_path)
                try:
                    expect_map = json.loads(exp_path.read_text(encoding="utf-8"))
                except (OSError, ValueError) as exc:
                    raise UsageError("--expect-from %s нечитаем (%s)"
                                     % (exp_path, exc))
                if not isinstance(expect_map, dict):
                    raise UsageError(
                        "--expect-from %s: ожидался JSON-объект "
                        "{путь: sha256|absent}" % exp_path)
            payload = op_promote(root, Path(args.staging).resolve(), args.run_id,
                                 ...
                                 expect_strict=args.expect_strict)
            _emit(payload, as_json)
            return 0
