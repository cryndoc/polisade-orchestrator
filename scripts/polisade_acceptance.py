...
Usage:
    ...
    python3 scripts/polisade_acceptance.py status  [--root DIR] [--json]
...
_INLINE_COMMENT_RE = re.compile(r"<!--.*?-->", re.S)
...
def parse_acceptance(text: str) -> ParseResult:
    ...
    for idx, raw in enumerate(lines, start=1):
        # ФЕНС СИЛЬНЕЕ РАЗМЕТКИ (#295). Внутри ```-блока лежит тело чека, и
        # `<!--` там — это символы команды (grep по комментариям в вёрстке,
        # sed по шаблону), а не начало markdown-комментария. Раньше состояние
        # «внутри комментария» проверялось РАНЬШЕ фенса, поэтому такая строка
        # уводила разбор до ближайшего `-->` и съедала не только свою пару, но
        # и все следующие. Тот же приоритет уже был признан верным ниже для
        # заголовков (`## ...` внутри фенса — строка чека, а не новая секция);
        # здесь он просто не был применён к комментариям.
        if in_comment:
            # Режим комментария сильнее всего: `-->` его закрывает, и ничего
            # другого внутри не разбирается — фенс внутри комментария не
            # открывается, иначе скрытая пара стала бы исполняемой.
            if "-->" in raw:
                in_comment = False
            continue
        if _FENCE_RE.match(raw):
            ...
            continue
        if not in_fence:
            # Открытие комментария — только СНАРУЖИ блока кода. Внутри `<!--`
            # это символы команды (#295). Форма «```bash <!--» тоже разобрана
            # выше как фенс: строка ОТКРЫВАЕТ блок, а `<!--` в ней — хвост
            # info-строки; раньше она уводила разбор в комментарий до конца
            # файла, и терялись все пары (находка собственного прогона).
            ...
            if "<!--" in raw and "-->" in raw:
                # ЗАКРЫТЫЙ комментарий вырезается из строки, а не отменяет её:
                # `## AC-001 — интент <!-- заметка -->` markdown показывает как
                # обычный заголовок, и пропуская строку целиком, парсер терял
                # пару, которую человек видит (находка ревью). Если после
                # выреза не осталось ничего — строка и была комментарием.
                raw = _INLINE_COMMENT_RE.sub("", raw)
                if not raw.strip():
                    continue
...
def _rel(root: Path, path: Path) -> str:
    # ОБЕ стороны разрешаются. Раньше разрешался только путь, и вызывающий,
    # передавший неразрешённый корень, получал АБСОЛЮТНЫЙ путь в выводе: на
    # macOS `/var` это симлинк на `/private/var`, поэтому `relative_to`
    # промахивался и срабатывал фолбэк. Через CLI корень приходил уже
    # разрешённым, так что дефект был латентным — его поднял `status` (#343),
    # который зовут из doctor с сырым корнем. Цена: абсолютный путь несёт имя
    # пользователя и уезжает в отчёт и в описание PR.
    try:
...
# ── issue #343 — приёмка становится видимой конвейеру ──────────────────────
# ОДИН расчёт, четыре читателя: `/polisade:doctor`, `/polisade:state`, секция
# описания PR и строка в `/polisade:review-pr`. Четыре собственных разбора
# одного файла разошлись бы молча — это тот же класс, которым открывается
# правило №1 CLAUDE.md.
#
# Чеки здесь НЕ исполняются. Проверка — произвольная команда из репозитория;
# запускать её в `doctor` или при сборке описания PR значит запускать чужой
# код там, где человек его не просил. Отсюда граница: `status` отвечает на
# «что известно о приёмке», а не «зелена ли она прямо сейчас», и последнее
# известное состояние берётся из отчёта, который написал `run`.
def acceptance_status(root: Path, path: Path = None):
    """Состояние приёмки БЕЗ исполнения чеков. Возвращает dict."""
    path = path or (root / DEFAULT_REL_FILE)
    rel = _rel(root, path)
    out = {
        "schema_version": REPORT_SCHEMA_VERSION,
        "source": rel,
        ...
        "baseline": {"state": "missing", "changed_checks": [], "changed_instruments": []},
        "last_run": None,
        "honest_note": HONEST_NOTE,
    }

    if not path.exists():
        out["summary"] = "приёмка не заведена"
        return out
    try:
        text = path.read_text(encoding="utf-8")
    except (OSError, UnicodeDecodeError) as exc:
        out["state"] = "unreadable"
        out["summary"] = "файл приёмки нечитаем (%s)" % exc.__class__.__name__
        return out

    parsed = parse_acceptance(text)
    l_err, l_warn = lint_pairs(parsed.pairs, rel, root)
    errors = parsed.errors + l_err
    warnings = parsed.warnings + l_warn
    out["pairs"] = len(parsed.pairs)
    out["lint"] = {"errors": len(errors), "warnings": len(warnings)}
    out["state"] = "lint_red" if errors else "ok"

    base_state, baseline = load_baseline(root)
    out["baseline"]["state"] = base_state
    if base_state == "valid" and baseline:
        base_pairs = baseline.get("pairs", {}) or {}
        now_pairs = {p.id: p.digest for p in parsed.pairs}
        out["baseline"]["changed_checks"] = sorted(
            [k for k, v in now_pairs.items() if k in base_pairs and base_pairs[k] != v]
            + sorted(set(now_pairs) ^ set(base_pairs)))
        # Второй способ купить зелень — поправить не чек, а ПРИБОР: тест-файл,
        # который чек запускает. База помнит и его sha, и без этой половины
        # «база совпала» означало бы меньше, чем читается.
        base_files = baseline.get("referenced_files", {}) or {}
        now_files = referenced_files_map(root, parsed.pairs)
        out["baseline"]["changed_instruments"] = sorted(
            relpath for relpath in set(base_files) | set(now_files)
            if base_files.get(relpath) != now_files.get(relpath))
    ...
    if report_path.exists():
        ...
        if isinstance(report, dict):
            ...
            # 🚨 УСТАРЕВШИЙ ОТЧЁТ — НЕ ФАКТ О ЭТОМ КОДЕ. «Приёмка зелёная»,
            # снятая три коммита назад или на другом наборе пар, читается
            # ревьюером как утверждение о том, что он смотрит. Отсюда два
            # признака, и оба печатаются.
            out["last_run"] = {
                ...
                "stale": bool(
                    (head and report.get("head_commit") and head != report.get("head_commit"))
                    or report.get("set_digest") != set_digest(parsed.pairs)),
            }

    out["summary"] = _acceptance_summary_line(out)
    return out


def _acceptance_summary_line(status):
    """Одна строка для человека. Порядок ветвей — порядок важности."""
    if status["state"] == "absent":
        return "приёмка не заведена"
    if status["state"] == "unreadable":
        return "файл приёмки нечитаем"
    if status["state"] == "lint_red":
        return "линт формы красный: %d ошибок в %s" % (
            status["lint"]["errors"], status["source"])
    bits = ["пар: %d" % status["pairs"]]
    run = status["last_run"]
    if not run:
        bits.append("прогонов не было")
    else:
        s = run["summary"]
        bits.append("последний прогон: %d/%d зелёных" % (
            s.get("green", 0), s.get("total", 0)))
        if run["stale"]:
            # Не «устарел — значит неважно», а наоборот: это утверждение о
            # ДРУГОМ коде, и подавать его как состояние текущего нельзя.
            bits.append("⚠️ отчёт снят на другом коде (stale)")
    base = status["baseline"]
    if base["state"] != "valid":
        bits.append("база не зафиксирована")
    elif base["changed_checks"] or base["changed_instruments"]:
        bits.append("⚠️ база разошлась: чеков %d, приборов %d" % (
...
def cmd_status(args):
    root, path = resolve_paths(args)
    status = acceptance_status(root, path)
    if args.json:
        print(json.dumps(status, ensure_ascii=False, indent=2, sort_keys=True))
        return 0
    print("═══════════════════════════════════════════")
    print("ПРИЁМКА — СОСТОЯНИЕ (%s)" % status["source"])
    print("═══════════════════════════════════════════")
...
def build_parser():
    ...
    sp = common(sub.add_parser(
        "status", help="состояние приёмки БЕЗ исполнения чеков (#343)"))
    sp.add_argument("--json", action="store_true")
