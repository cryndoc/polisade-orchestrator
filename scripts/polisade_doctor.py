...
def _settings_permission_findings(allow):
    """Return findings for polluted permission entries (#121)."""
    findings = []
    for entry in allow:
        if not isinstance(entry, str):
            continue
        m = re.match(r"^Bash\((\w+)\b", entry)
        if m and m.group(1) in _HYGIENE_BASH_KEYWORDS:
            findings.append(f"shell-keyword rule: {entry}")
            continue
        if re.match(r"^(?:Read|Bash|Write|Edit)\(//", entry):
            findings.append(f"double-slash path: {entry}")
            continue
        if _HYGIENE_NUMERIC_UID_RE.search(entry):
            findings.append(f"numeric user-id path: {entry}")
    return findings
...
def check_settings_hygiene(root):
    ...
    plus malformed absolute paths carrying the corp numeric user-id. The root
    cause is upstream, but we flag the pollution so the PM removes it before the
    file is force-committed and leaks the id. Remediation is manual today; an
    ...
    for sub, name in _HYGIENE_SETTINGS_FILES:
        ...
        if not isinstance(allow, list):
            continue
        findings = _settings_permission_findings(allow)
        if findings:
            polluted[f"{sub}/{name}"] = findings

    if not polluted:
        return {"name": "settings_hygiene", "status": "pass",
                "message": "no polluted permission entries"}

    total = sum(len(v) for v in polluted.values())
    parts = [f"{fname}: {len(f)} (e.g. {f[0]})" for fname, f in polluted.items()]
    ...
    return {
        ...
        "message": (
            ...
            + ". Remove these allow-rules manually; do not force-commit this file."
        ),
    }
