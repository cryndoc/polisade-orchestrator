...
def check_corpus_review(root):
    ...
    for path in sorted(corpus.rglob("*.md")):
        ...
        if re.search(r"^review:\s*REQUIRED\s*$", head, re.MULTILINE):
            pending.append(rel)
        elif (re.search(r"^reviewed_by:\s*\S", head, re.MULTILINE)
              and re.search(r"^reviewed_at:\s*\S", head, re.MULTILINE)):
