...
import polisade_id  # noqa: E402  (seed identity — one rule, one module)
...
def _leading_frontmatter(path):
    """The `---`-delimited block at the START of the file, or ''.

    Scanning «the first N bytes» instead let a line in the BODY answer a
    question that is only meaningful in frontmatter.
    """
...
def _yaml_field(head, name):
    """The VALUE of a frontmatter field, read the way the format is written.

    Issue #322: the stamp was matched with `^review:\\s*REQUIRED\\s*$`, which
    requires the line to END after the word — and the canonical example in
    `skills/design-corpus/SKILL.md`, the one a person copies, is
    `review: REQUIRED                # снять может ТОЛЬКО человек — см. ниже`.
    The shipped format and the shipped reader disagreed about the same line, so
    a correctly stamped file was reported as carrying NO stamp at all: `fail`,
    with a message telling the author the file has neither mark.

    The same read fixes a quieter form of it in the other direction: `\\S` after
    the colon is satisfied by a `#`, so `reviewed_by: # nobody` counted as
    reviewed. A comment is not a value.

    Returns "" when the field is absent or carries nothing but a comment.
    """
    for line in head.splitlines():
        key, sep, value = line.partition(":")
        if not sep or key.strip() != name or key[:1] in (" ", "\t"):
            continue
        return value.split("#")[0].strip().strip('"').strip("'")
    return ""


def check_corpus_review(root):
    """Corpus files a person has not read yet (#300).

    The living corpus is generated BEST-EFFORT: provenance is `INFERRED` or
    `GAP` and never `CONFIRMED`, there are no deterministic integrity gates, and
    the statements in it are a model's grounded guesses. That is written in
    every file's footer — but a footer is a thing you stop seeing.

    So each emitted file carries `review: REQUIRED`, cleared by replacing that
    line with BOTH `reviewed_by:` and `reviewed_at:` in the leading frontmatter.

    🚨 The honest strength of this, because the first draft of the surrounding
    prose overstated it: a generator WRITES the file, so nothing here can stop
    it from writing `reviewed_by:` too. This is a convention, not a barrier.
    What makes it worth having is that clearing the stamp is then a deliberate,
    visible edit — it shows up in the diff of a generated file, under a name,
    with a date — and that the check below counts what is still unread instead
    of letting a footer nobody reads carry the whole warning. A mechanism that
    could not be forged would need something outside this tool to attest; that
    does not exist here, and claiming it would be the lie.

    Regeneration re-stamps the file, because what was read was the previous
    text.

    This check COUNTS, it does not block: `settings.experimental.designCorpus`
    is off by default, and a gate that fires in a flow nobody runs teaches
    people to skip gates. What it must never do is stay silent while unreviewed
    generated architecture sits in `docs/`.
    """
    corpus = root / "docs" / "architecture"
    if not corpus.is_dir():
        return {"name": "corpus_review", "status": "pass",
                "message": "no living corpus in this project"}

    pending, reviewed, unstamped = [], 0, []
    for path in sorted(corpus.rglob("*.md")):
        if not path.is_file() or path.is_symlink():
            continue
        try:
            head = _leading_frontmatter(path)
        except (OSError, UnicodeDecodeError):
            continue
        rel = path.relative_to(root).as_posix()
        if _yaml_field(head, "review") == "REQUIRED":
            pending.append(rel)
        elif _yaml_field(head, "reviewed_by") and _yaml_field(head, "reviewed_at"):
            # BOTH, and only inside the leading frontmatter. The first draft
            # scanned the first 4 KiB and accepted `reviewed_by:` alone, so a
            # line anywhere in the BODY cleared the stamp — including one the
            # generator wrote itself.
            reviewed += 1
        elif re.search(r"^generated_by:\s*\S", head, re.MULTILINE):
            # generated, but carrying neither stamp: the file cannot say
            # whether anyone read it, which is worse than saying «nobody did».
            unstamped.append(rel)

    if unstamped:
        return {"name": "corpus_review", "status": "fail",
                "message": ("%d generated corpus file(s) carry neither `review: "
                            "REQUIRED` nor `reviewed_by:` — nothing can tell whether "
                            "a person has read them: %s"
                            % (len(unstamped), ", ".join(unstamped[:4])))}
    if pending:
        return {"name": "corpus_review", "status": "warn",
                "message": ("%d corpus file(s) await a human read (%d reviewed). "
                            "Replace `review: REQUIRED` with `reviewed_by:` / "
                            "`reviewed_at:` once you have read one: %s"
                            % (len(pending), reviewed, ", ".join(pending[:4])))}
    if reviewed:
        return {"name": "corpus_review", "status": "pass",
                "message": "all %d generated corpus file(s) carry a human review "
                           "stamp" % reviewed}
    return {"name": "corpus_review", "status": "pass",
            "message": "no generated corpus files in docs/architecture"}


def check_seed_identity(root):
    """Artefacts still waiting for a number, and seeds that name two of them.

    Two separate facts, one report, because both are about the same field:

    * an artefact whose id is still a SEED has not landed on the trunk yet.
      That is a normal, expected state on a feature branch — it is only worth
      SAYING, so that a person who wonders why their task is `TASK-k7m2q4xz`
      reads the answer instead of guessing that something broke.
    * a seed carried by TWO artefacts makes `polisade_id.py resolve` ambiguous,
      and the resolver is what keeps a reference made before numbering — a
      commit message, a PR title — readable afterwards. Copying an artefact
      file is the ordinary way this happens, and nothing else would notice.
    """
    try:
        by_seed, _ = polisade_id.index(root)
    except OSError as exc:
        return {"name": "seed_identity", "status": "warn",
                "message": f"artefacts could not be read: {exc}"}

    unnumbered = sorted(
        (r for group in by_seed.values() for r in group if polisade_id.is_seed_id(r["id"])),
        key=lambda r: r["path"])
    # One list, three ways a seed stops doing its job — and all three are
    # equally silent: the artefact still LOOKS fine, and the reference made
    # before numbering is what stops resolving later.
    problems = polisade_id.seed_problems(root)
    if problems:
        detail = "; ".join(f"{r['path']}: {r['problem']}" for r in problems[:6])
        more = f" (+{len(problems) - 6})" if len(problems) > 6 else ""
        return {
            "name": "seed_identity",
            "status": "fail",
            "message": (
                f"{len(problems)} artefact(s) carry a seed that cannot resolve a "
                f"pre-numbering reference, and sync refuses to number them until "
                f"it can. Mint a fresh one with `polisade_id.py new-seed` or make "
                f"`seed:` equal the id's tail: {detail}{more}"
            ),
        }
    if unnumbered:
        names = ", ".join(r["id"] for r in unnumbered[:5])
        more = f" (+{len(unnumbered) - 5})" if len(unnumbered) > 5 else ""
        state = polisade_id.trunk_state(root)
        where = ("run /polisade:sync --apply here to number them"
                 if state["on_trunk"] else
                 f"numbers are handed out on the trunk only — {state['why']}")
        return {
            "name": "seed_identity",
            "status": "pass",
            "message": (
                f"{len(unnumbered)} artefact(s) still carry a seed id "
                f"({names}{more}) — {where}."
            ),
        }
    return {"name": "seed_identity", "status": "pass",
            "message": "every artefact carries a number; no seed names two of them"}
...
def check_counter_shared(root):
    """Does the number high-water mark travel, or is it still one machine's?

    `.state/counters.json` is the ONLY record that a number was ever handed out
    — every other fact is reconstructible from the files on disk, and a deleted
    artefact leaves no file. While git ignores it, the author and a colleague's
    fresh clone answer «what is the next number» differently from the SAME
    commit: measured `TASK-005` against `TASK-004` (#317).

    3.7.21 re-includes the file, and `/polisade:migrate` adds that line to an
    existing project. But a project that has not been migrated keeps the old
    behaviour — and keeps it SILENTLY, which is the half this check exists for.
    The question is asked of GIT, not of the `.gitignore` text: a negation below
    its exclusion is a line git never applies (#292), so reading the file would
    answer about the intention rather than the effect.
    """
    counters = root / ".state" / "counters.json"
    if not counters.exists():
        return {"name": "counter_shared", "status": "pass",
                "message": "no .state/counters.json yet — nothing has been numbered"}
    try:
        probe = subprocess.run(
            ["git", "-C", str(root), "rev-parse", "--is-inside-work-tree"],
            capture_output=True, text=True, timeout=5)
    except (OSError, subprocess.TimeoutExpired) as exc:
        return {"name": "counter_shared", "status": "warn",
                "message": "cannot ask git whether the counter travels: %s" % exc}
    if probe.returncode != 0 or probe.stdout.strip() != "true":
        return {"name": "counter_shared", "status": "pass",
                "message": "not a git work tree — there is one copy of the counter"}
    try:
        ignored = subprocess.run(
            ["git", "-C", str(root), "check-ignore", "-q", "--", ".state/counters.json"],
            capture_output=True, text=True, timeout=5)
    except (OSError, subprocess.TimeoutExpired) as exc:
        return {"name": "counter_shared", "status": "warn",
                "message": "cannot ask git whether the counter travels: %s" % exc}
    if ignored.returncode != 0:
        return {"name": "counter_shared", "status": "pass",
                "message": ".state/counters.json travels with the repository — "
                           "a deleted number is not handed out twice"}
    return {
        "name": "counter_shared", "status": "warn",
        "message": (".state/counters.json is ignored by git, so the mark above "
                    "DELETED numbers is per-machine: you and a fresh clone can "
                    "hand out the same number for different work. Run "
                    "`/polisade:migrate --apply` — it adds `!.state/counters.json` "
                    "to .gitignore (issue #317)"),
    }


def check_session_log(root):
    """The audit trail exists — as the directory, or as the legacy single file.

    Since 3.7.19 the log is `.state/session-log/`, one file per artefact-session
    named by that artefact's permanent seed. The single growing file could not
    be committed: every branch that logged anything conflicted on it, so the one
    durable record of what happened lived only on the machine that made it.

    The legacy `session-log.md` still counts as an audit trail — a project that
    has one is not broken, and this check does not nag a reader into deleting
    history. It says which shape it found, because a reader who sees only «pass»
    cannot tell whether their log is the one that travels.
    """
    directory = root / ".state" / "session-log"
    legacy = root / ".state" / "session-log.md"
    legacy_note = " (a legacy session-log.md is also present)" if legacy.exists() else ""

    if directory.is_dir():
        try:
            names = sorted(p.name for p in directory.iterdir())
        except OSError as exc:
            # `glob` SWALLOWS this and returns [], which the first draft then
            # reported as «empty». A directory nobody can read is not an empty
            # one: the audit trail may be there and unreachable, and a green
            # line over it is the worst of the four answers.
            return {"name": "session_log", "status": "warn",
                    "message": "%s — could not be read (%s); the audit trail may "
                               "be there and unreachable%s"
                               % (directory, exc.strerror or type(exc).__name__,
                                  legacy_note)}
        entries = [n for n in names if n.endswith(".md")]
        links = []
        for name in entries:
            try:
                ...
                pass
        if links:
            # git stores a symlink's TARGET PATH, not the file it points at, so
            # committing one puts an absolute path from somebody's machine into
            # the repository. The `.gitignore` cannot catch this: a name carries
            # no file mode, and these are named `*.md` like every other entry.
            return {"name": "session_log", "status": "fail",
                    "message": "%s — %d entr%s %s a SYMLINK; git would commit its "
                               "target PATH (an absolute path from a machine), not "
                               "the log: %s%s"
                               % (directory, len(links), "y" if len(links) == 1 else "ies",
                                  "is" if len(links) == 1 else "are",
                                  ", ".join(links[:4]), legacy_note)}
        if entries:
            stray = [n for n in names if not n.endswith(".md")]
            note = (" — %d file(s) that are not entries stay ignored: %s"
                    % (len(stray), ", ".join(stray[:3]))) if stray else ""
            return {"name": "session_log", "status": "pass",
                    "message": "%s — %d entr%s%s%s"
                               % (directory, len(entries),
                                  "y" if len(entries) == 1 else "ies", note, legacy_note)}
        if names:
            # Not «empty»: something is in there, and none of it is an entry —
            # a reader who logged and sees «empty» will look in the wrong place.
            return {"name": "session_log", "status": "warn",
                    "message": "%s — holds %d file(s) but no `*.md` entry: %s%s"
                               % (directory, len(names), ", ".join(names[:3]), legacy_note)}
        return {"name": "session_log", "status": "pass",
                "message": "%s — empty (nothing logged yet)%s" % (directory, legacy_note)}

    if legacy.exists():
        return {"name": "session_log", "status": "pass",
                "message": "%s — legacy single file; new entries go to "
                           ".state/session-log/ (one per artefact-session)" % legacy}
    return {"name": "session_log", "status": "warn",
            "message": ".state/session-log/ not found (audit trail)"}
...
def parse_scripts_manifest(text):
    ...
    for lineno, raw in enumerate(text.splitlines(), start=1):
        ...
        # 🚨 NOT `.strip()`. The installer copies the manifest's names
        # LOSSLESSLY, so a row reading `guide.md ` installs a file whose name
        # ends in a space — and a doctor that stripped the name then looked for
        # `guide.md`, did not find it, and called the copy incomplete. Two
        # readers of one manifest disagreeing is the failure class; the fix is
        # not to pick a normalisation but to REFUSE the ambiguity, which the
        # build never emits anyway.
        if rel != rel.strip():
            errors.append(
                f"строка {lineno}: имя {rel!r} обрамлено пробелами — установщик "
                f"перенесёт его дословно, а эта проверка искала бы другое имя. "
                f"Манифест повреждён.")
            continue
...
# Where each CLI actually keeps the permissions this check is about. Issue
# #337: `.claude/settings.json` is a CLAUDE CODE artefact — the GigaCode build
# strips every mention of it through the claude-only markers (0
# occurrences in the published `commands/polisade/migrate.md`), and GigaCode
# reads `~/.gigacode/settings.json` instead. The neighbouring
# `check_settings_hygiene` in this same file already knew that; this check did
# not, so one doctor report said two different things about where settings live.
_PERMISSIONS_HOME = {
...
def check_acceptance(root):
    """Issue #343 — приёмка видна конвейеру, а не живёт рядом с ним.

    Чеки НЕ исполняются, ровно по тому же основанию, что и в
    `check_project_gates`: проверка — это произвольная команда из
    репозитория, и запускать её внутри диагностики значит выполнять чужой код
    там, где человек его не просил. Состояние берётся из
    `polisade_acceptance.acceptance_status` — того же расчёта, который читают
    `/polisade:state`, описание PR и `/polisade:review-pr`. Четыре
    собственных разбора одного файла разошлись бы молча.

    Отсутствие приёмки — законное состояние (`pass`), а не проблема: это
    opt-in практика. `fail` остаётся за тем, что объективно сломано и
    чинится: файл нечитаем или его форма красная. Всё остальное —
    предупреждение: вечный красный, который не может погаснуть, обесценивает
    соседние (урок #337).
    """
    try:
        import polisade_acceptance
    except ImportError as e:      # вендоренный набор без этого модуля
        return {"name": "acceptance", "status": "warn",
                "message": f"acceptance engine unavailable: {e}"}
    try:
        status = polisade_acceptance.acceptance_status(Path(root))
    except Exception as e:        # noqa: BLE001 — диагностика не должна падать
        return {"name": "acceptance", "status": "warn",
                "message": f"acceptance status unavailable: {e.__class__.__name__}: {e}"}

    state = status["state"]
    if state == "absent":
        return {"name": "acceptance", "status": "pass",
                "message": ("приёмка не заведена (opt-in практика; завести — "
                            "`/polisade:acceptance author`)")}
    if state == "unreadable":
        return {"name": "acceptance", "status": "fail",
                "message": f"{status['source']} нечитаем — приёмка не может быть прогнана"}
    if state == "lint_red":
        return {"name": "acceptance", "status": "fail",
                "message": ("форма приёмки красная: %d ошибок в %s "
                            "(`polisade_acceptance.py lint`)"
                            % (status["lint"]["errors"], status["source"]))}

    problems = []
    run = status["last_run"]
    if run is None:
        problems.append("прогонов не было")
    else:
        red = (run["summary"].get("red", 0) + run["summary"].get("timeout", 0)
               + run["summary"].get("error", 0))
        if red:
            problems.append("последний прогон: %d не зелёных из %d"
                            % (red, run["summary"].get("total", 0)))
        if run["stale"]:
            # Устаревший отчёт — утверждение о ДРУГОМ коде. Молчать о нём
            # хуже, чем не иметь отчёта вовсе: «зелено» прочитают как факт
            # о текущем дереве.
            problems.append("отчёт снят на другом коде (stale)")
    base = status["baseline"]
    if base["state"] == "missing":
        problems.append("база не зафиксирована — `--fail-on-changed` не с чем сравнивать")
    elif base["state"] == "invalid":
        problems.append("база повреждена")
    elif base["changed_checks"] or base["changed_instruments"]:
        problems.append("база разошлась: чеков %d, приборов %d"
                        % (len(base["changed_checks"]), len(base["changed_instruments"])))

    if problems:
        return {"name": "acceptance", "status": "warn",
                "message": "пар %d; %s" % (status["pairs"], "; ".join(problems))}
    return {"name": "acceptance", "status": "pass",
            "message": ("пар %d, последний прогон зелёный, база совпадает "
                        "(best-effort практика, не гарантия)" % status["pairs"])}
...
def check_settings_permissions(root):
    ...
    if not path.exists():
        # The contour comes from `detect_current_cli()` — the ONE place that
        # answers "which CLI is this", already used by this file elsewhere. A
        # second copy of that logic here is the divergence this check is being
        # fixed for, one layer up.
        try:
            from polisade_cli_caps import detect_current_cli
            cli = detect_current_cli()
        except Exception:
            cli = ""
        elsewhere = _PERMISSIONS_HOME.get(cli)
        if elsewhere:
            # NOT a fail: in this contour the file is absent BY DESIGN, and a
            # red signal that can never be cleared teaches people to stop
            # reading red signals — that corp report carried three fails, one
            # of them permanent. What this check does NOT do is claim the
            # permissions are fine: it says it did not look. Reading the CLI's
            # own settings file for equivalents would need to know what those
            # equivalents ARE, which is unmeasured here, and a check built on a
            # guess is worse than one that admits its limit.
            return {"name": "settings_permissions", "status": "pass",
                    "message": ("not applicable under %s — `.claude/settings.json` "
                                "is a Claude Code artefact; this CLI keeps its "
                                "permissions in %s, which `settings_hygiene` "
                                "reads. Permissions NOT checked here (#337)."
                                % (cli, elsewhere))}
        # Unknown contour → the old verdict stands. Fail-closed: the check is
        # softened only on POSITIVE evidence of a contour that does not use the
        # file, never on the absence of evidence.
...
def main():
    ...
    checks.append(check_counter_shared(root))
    ...
    checks.append(check_seed_identity(root))
    checks.append(check_corpus_review(root))
    ...
    # Issue #343 — приёмка: заведена ли, красная ли форма, разошлась ли база.
    # Чеки не исполняются (см. docstring) — это диагностика, а не прогон.
    checks.append(check_acceptance(root))
