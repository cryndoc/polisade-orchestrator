...
def _stale_hint(version, dest=VENDOR_BIN_REL):
    ...
    return (f"копия устарела или искажена — обновите каталог из архива "
            ...
            f"(`mkdir -p {parent} && rm -rf {dest} && "
            f"cp -R <распакованный архив>/scripts {dest}`)")
...
def verify_vendored_scripts(root):
    ...
    install_hint = (
        f"установите из ОБЫЧНОГО терминала: `mkdir -p {parent} && "
        f"cp -R <распакованный архив расширения>/scripts {dest}` "
...
_HYGIENE_NUMERIC_UID_RE = re.compile(r"/Users/\d+/")
...
_HYGIENE_PROSE_RE = re.compile(r"[—–]|[.,;:!?]\s|[.!?]$")
...
# 🚨 Deliberate call on the ambiguous tokens (#279). Each of these is BOTH a
# Python name and a real binary: `import` is ImageMagick's screen grabber,
# `open` is macOS's launcher, `as` is the GNU assembler, `pass` is
# password-store, `next` is the Next.js CLI. Two rules apply to them:
#
#   1. an ARGUMENT PATTERN clears the token outright. A real permission rule for
#      a real command almost always carries one — you write `Bash(import *)`
#      because you intend to pass it a file — whereas a transcribed source line
#      is the bare token. So `Bash(import *)` and `Bash(as *)` are never
#      findings, at the cost of under-reporting `Bash(print *)`-style junk. That
#      is the right trade: one false positive on `Bash(import *)` teaches the PM
#      to ignore the whole check.
#   2. the bare form is reported only for tokens actually OBSERVED as junk in
#      the corp file — `open` and `print`. Second opinion, round 1: reporting
#      every bare ambiguous token would flag `Bash(pass)` (password-store) and
#      `Bash(next)` on nothing but a guess. Evidence, not symmetry, decides.
_HYGIENE_AMBIGUOUS_TOKENS = {
    "import", "open", "print", "pass", "input", "next", "iter", "str",
    "len", "float", "int", "bool", "property", "match", "self", "as", "del",
    "id", "type", "dir", "set", "hash", "zip", "sum", "time", "test", "help",
    "compile", "exec", "eval", "format", "sorted", "tuple",
}
_HYGIENE_AMBIGUOUS_BARE_REPORTED = {"open", "print"}
...
def _hygiene_is_py_string(inner):
    ...
    * the payload is EXACTLY one closed quoted string (nothing trailing it, which
    ...
    if not m:
        return False
    content = m.group(2)
    # A fully parenthesised remark is prose too — no path is `(…)`.
    prose = bool(_HYGIENE_PROSE_RE.search(content)) or (
        content.startswith("(") and content.endswith(")"))
    return prose and len(content.split()) >= 3
...
def _python_fragment_finding(entry):
    ...
    if token in _HYGIENE_PY_KEYWORDS or token in _HYGIENE_PY_BUILTINS:
        if token in _HYGIENE_AMBIGUOUS_TOKENS and (
                has_arg_pattern or token not in _HYGIENE_AMBIGUOUS_BARE_REPORTED):
            return None
...
def _hygiene_redact(text, home=None):
    ...
    return _HYGIENE_NUMERIC_UID_RE.sub("/Users/<uid>/", text)
...
def _settings_permission_findings(allow, home=None):
    ...
    for raw in allow:
        ...
        if re.match(r"^(?:Read|Bash|Write|Edit)\(//", raw):
            ...
            continue
        if _HYGIENE_NUMERIC_UID_RE.search(raw):
...
def check_settings_hygiene(root):
    ...
    for label, path, scope in scopes:
        ...
        try:
            ...
            continue
        # A malformed file must not crash an advisory check: `{"permissions":
        # null}` and a JSON array root both used to raise AttributeError and
        # take the whole doctor run down with them (second opinion, round 1).
    ...
    return {
        ...
        "status": "warn",
        "message": (
            f"{total} polluted permission entr{plural} — " + "; ".join(parts)
            + ". " + " ".join(advice)
        ),
    }
