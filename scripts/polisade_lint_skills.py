...
_CORPUS_DIRECT = {
    ...
    "scripts/polisade_id.py": {
        "scope": "docs/architecture/DESIGN-*/{README.md,manifest.yaml} — "
                 "только поля идентичности (`id:` и машинные ссылки) и "
                 "переименование каталога пакета, и только при выдаче номера",
        "reason":
            "Это не содержимое корпуса, а ИМЯ артефакта. Номер пакета живёт в "
            "имени каталога, поэтому выдача номера переименовывает каталог "
            "целиком — `git mv`, чтобы переезд остался переездом в истории; "
            "провести это через примитив, адресуемый путями и хэшами, значило "
            "бы переписать пакет заново и потерять её. Правятся ровно два "
            "поля: `id:` в README и `id:`/`parent:`/`supersedes:` в манифесте, "
            "который схема пакета обязывает совпадать с именем каталога. "
            "ДО 3.7.21 файл значился ЧИТАТЕЛЕМ с утверждением «ни одного файла "
            "корпуса не пишет» — неверным уже в 3.7.17, где `_rewrite_id` "
            "правит README пакета: ошибка классификации, а не устройства "
            "(issue #314/#315).",
    },
...
def _init_bundle_docs_targets(root):
    """`{имя: целевой путь}` для плоских `docs/*.md` из `_INIT_INLINE_BUNDLE`.

    Читается из СКАНИРУЕМОГО дерева через `ast` — как `_dev_only_scripts`:
    импорт `tools/convert.py` затащил бы модуль своего репозитория, и фикстура
    линта проверялась бы против чужой раскладки.
    """
    conv_path = root / "tools" / "convert.py"
    if not conv_path.is_file():
        return {}
    try:
        tree = ast.parse(conv_path.read_text(encoding="utf-8"))
    except (OSError, SyntaxError, UnicodeDecodeError):
        return {}
    for node in ast.walk(tree):
        ...
        if not any(isinstance(t, ast.Name) and t.id == "_INIT_INLINE_BUNDLE"
                   for t in targets):
            continue
        try:
        ...
        out = {}
        for row in rows:
            src, target = str(row[0]), str(row[1])
            if not src.startswith("docs/") or "/" in src[len("docs/"):]:
                continue
            out[src[len("docs/"):]] = target
        return out
    return {}


def _migrate_not_flat_templates(root):
    """`DOCS_TEMPLATES_NOT_FLAT` из сканируемого `scripts/polisade_migrate.py`."""
    mig = root / "scripts" / "polisade_migrate.py"
    if not mig.is_file():
        return None
    try:
    ...
    for node in ast.walk(tree):
        if not isinstance(node, ast.Assign):
            continue
        if not any(isinstance(t, ast.Name) and t.id == "DOCS_TEMPLATES_NOT_FLAT"
                   for t in node.targets):
            continue
        # `frozenset({...})` — вызов, а не литерал, поэтому literal_eval берёт
        # аргумент, а не всё выражение.
        value = node.value
...
def check_docs_templates_targets(root):
    """Issue #341 — раскладка шаблонов артефактов объявлена ОДИН раз.

    `/polisade:init` объявляет, куда едет каждый шаблон
    (`tools/convert.py::_INIT_INLINE_BUNDLE`). `/polisade:migrate` доносит
    подмножество — те, чей дом `docs/templates/<то же имя>` — и держит
    ДОПОЛНЕНИЕ этого набора константой `DOCS_TEMPLATES_NOT_FLAT`. Два
    утверждения об одной раскладке расходятся молча и всегда в худшую сторону:
    новый шаблон, забытый в дополнении, поедет мигратором не в тот дом, а
    лишнее имя в дополнении навсегда исключит шаблон из доставки.

    Здесь набор ВЫВОДИТСЯ из bundle и сравнивается с тем, что записано в
    миграторе — ни одна из сторон не переписана в линт.
    """
    issues = []
    canon_dir = root / "skills" / "init" / "templates" / "docs"
    if not canon_dir.is_dir():
        return issues
    bundle = _init_bundle_docs_targets(root)
    if not bundle:
        return issues
    declared = _migrate_not_flat_templates(root)
    if declared is None:
        return issues

    canon = {p.name for p in canon_dir.glob("*.md") if p.is_file()}
    flat = {name for name, target in bundle.items()
            if target == "docs/templates/" + name}
    expected_not_flat = canon - flat

    if declared != expected_not_flat:
        issues.append({
            "level": "error",
            "message": (
                "Issue #341: `DOCS_TEMPLATES_NOT_FLAT` в "
                "scripts/polisade_migrate.py разошёлся с раскладкой "
                "`_INIT_INLINE_BUNDLE`: в миграторе %s, а из bundle следует %s. "
                "Мигратор доставит шаблон не в тот дом либо не доставит вовсе."
                % (sorted(declared) or "пусто", sorted(expected_not_flat) or "пусто")
            ),
        })

    missing = sorted(name for name in bundle if name not in canon)
    if missing:
        issues.append({
            "level": "error",
            "message": (
                "Issue #341: `_INIT_INLINE_BUNDLE` называет шаблоны, которых "
                "нет в skills/init/templates/docs/: %s. init инлайнил бы "
                "пустоту, а мигратор не доставил бы ничего." % ", ".join(missing)
            ),
        })
    return issues
...
def main():
    ...
    docs_templates_issues = check_docs_templates_targets(root)
    ...
    # Issue #341 — the artefact-template layout is declared once: the migrator's
    # complement set is derived from `_INIT_INLINE_BUNDLE`, not restated.
    if docs_templates_issues:
        t_errors = [i for i in docs_templates_issues if i["level"] == "error"]
        t_warnings = [i for i in docs_templates_issues if i["level"] == "warn"]
        total_errors += len(t_errors)
        total_warnings += len(t_warnings)
        status = "fail" if t_errors else ("warn" if t_warnings else "pass")
        results.append({
            "skill": "_docs_templates_targets",
            "dir": "tools/convert.py + scripts/polisade_migrate.py",
            "status": status,
            "issues": docs_templates_issues,
        })
