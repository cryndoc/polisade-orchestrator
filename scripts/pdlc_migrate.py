...
# directory (`~/.gigacode/extensions/pdlc/...`), so any runtime read against
...
def compute_migrations(state, root):
    ...
    if "pdlcVersion" not in state:
        ...
        migrations.append(Migration(
            f"Add pdlcVersion: {CURRENT_PDLC_VERSION}",
            add_pdlc_version,
            [],
        ))
    ...
    if needs_entry:
        def append_pdlc_tmp(s, path=gitignore_dst):
            ...
            if path.exists():
                ...
                path.write_text(block.lstrip("\n"), encoding="utf-8")
        migrations.append(Migration(
            "Append `.pdlc/tmp/` to .gitignore (issue #57)",
            append_pdlc_tmp,
            [gitignore_dst],
        ))
...
def main():
    ...
    # may have been freshly written by this very run (vcs bootstrap, pdlc/tmp
