...
Subcommands:
    pr-create   --title T (--body B | --body-file F | --body-stdin) [--head BR] [--base main]
                ...
                branch is returned as-is instead of opening a second one. The
                payload carries `existing: true` in that case, `false` when the
                PR was just created; the rest of the shape is identical, so a
                caller that ignores the field keeps working.
...
def gh_find_open_pr(head: str, project_root: Path) -> tuple[dict | None, bool]:
    """Look for the OPEN PR whose head is `head`. Return (pr_or_None, ok).
    ...
    The second element is False when the lookup could not answer (`gh` failed,
    unparseable output). The caller then still creates — a transient provider
    hiccup must not block the autonomous loop — but says so in the payload
    rather than implying the branch was checked.
    """
    if not head:
        return None, False
    out = _gh(
        ["pr", "list", "--head", head, "--state", "open", "--limit", "50",
         "--json", "number,url,title,state,headRefName,isCrossRepository"],
    ...
    for row in rows:
        ...
        # accepted on the strength of the flag alone.
        if row.get("headRefName") != head:
            continue
        ...
        # have read as "same repo". A gh too old to know the field fails the
        # whole `--json` call instead, which lands on lookup_ok=False → create.
...
def gh_pr_create(args, project_root: Path) -> dict:
    ...
    found, lookup_ok = gh_find_open_pr(head, project_root)
    ...
    return {"number": number, "url": url, "state": "OPEN", "title": args.title,
...
def bb_find_open_pr(inst, project_key: str, slug: str, auth_mode,
                    head: str) -> tuple[dict | None, bool]:
    """Look for the OPEN Bitbucket PR whose fromRef is `head`. (pr, ok) — #158.
    ...
    for pr in data.get("values", []):
        ...
        if not isinstance(project, dict) or project.get("key") != project_key:
            continue
        return pr, True
    return None, True
...
def bb_pr_create(args, project_root: Path) -> dict:
    ...
    found, lookup_ok = bb_find_open_pr(inst, project_key, slug, auth_mode, head)
    if found is not None:
        return {
            "number": found["id"],
            ...
            "headRefName": (found.get("fromRef") or {}).get("displayId") or head,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    c = sub.add_parser("pr-create", parents=[parent, env_parent], allow_abbrev=False)
    c.add_argument("--title", required=True)
    c.add_argument("--head", default=None)
    c.add_argument("--base", default=None)
    add_body_flags(c)

    c = sub.add_parser("pr-view", parents=[parent, env_parent], allow_abbrev=False)
    c.add_argument("id", type=int)
    c.add_argument("--fields", default=None,
                   help="Comma-separated subset of: title,body,files,state,headRefName,mergeable,url")

    c = sub.add_parser("pr-list", parents=[parent, env_parent], allow_abbrev=False)
    c.add_argument("--head", default=None)
    c.add_argument("--state", default="OPEN", choices=["OPEN", "MERGED", "ALL"])

    c = sub.add_parser("pr-diff", parents=[parent, env_parent], allow_abbrev=False)
    c.add_argument("id", type=int)

    c = sub.add_parser("pr-merge", parents=[parent, env_parent], allow_abbrev=False)
    c.add_argument("id", type=int)
    c.add_argument("--squash", action="store_true")
    c.add_argument("--delete-branch", action="store_true")

    c = sub.add_parser("pr-comment", parents=[parent, env_parent], allow_abbrev=False)
    c.add_argument("id", type=int)
    add_body_flags(c)

    c = sub.add_parser("pr-close", parents=[parent, env_parent], allow_abbrev=False)
    c.add_argument("id", type=int)

    sub.add_parser("whoami", parents=[parent, env_parent], allow_abbrev=False)

    c = sub.add_parser("git-push", parents=[parent, env_parent], allow_abbrev=False)
