...
def build_parser() -> argparse.ArgumentParser:
    ...
    c = sub.add_parser("pr-create", parents=[parent], allow_abbrev=False)
    c.add_argument("--title", required=True)
    c.add_argument("--head", default=None)
    c.add_argument("--base", default=None)
    add_body_flags(c)

    c = sub.add_parser("pr-view", parents=[parent], allow_abbrev=False)
    c.add_argument("id", type=int)
    c.add_argument("--fields", default=None,
                   help="Comma-separated subset of: title,body,files,state,headRefName,mergeable,url")

    c = sub.add_parser("pr-list", parents=[parent], allow_abbrev=False)
    c.add_argument("--head", default=None)
    c.add_argument("--state", default="OPEN", choices=["OPEN", "MERGED", "ALL"])

    c = sub.add_parser("pr-diff", parents=[parent], allow_abbrev=False)
    c.add_argument("id", type=int)

    c = sub.add_parser("pr-merge", parents=[parent], allow_abbrev=False)
    c.add_argument("id", type=int)
    c.add_argument("--squash", action="store_true")
    c.add_argument("--delete-branch", action="store_true")

    c = sub.add_parser("pr-comment", parents=[parent], allow_abbrev=False)
    c.add_argument("id", type=int)
    add_body_flags(c)

    c = sub.add_parser("pr-close", parents=[parent], allow_abbrev=False)
    c.add_argument("id", type=int)

    sub.add_parser("whoami", parents=[parent], allow_abbrev=False)

    c = sub.add_parser("git-push", parents=[parent], allow_abbrev=False)
