...
Single source of truth used by polisade_lint_artifacts.py, polisade_doctor.py, and
polisade_migrate.py. Stdlib-only per invariant #6.

Composite format: ``{DOC_ID}.{REQ_ID}``, e.g. ``PRD-001.FR-007``,
``FEAT-002.FR-007``, ``SPEC-002.NFR-003``.

  - ``DOC_ID`` ∈ {PRD-NNN, SPEC-NNN, FEAT-NNN}: 3-digit zero-padded — or the
    same document's SEED (``SPEC-a1b2c3d4``) before the trunk has numbered it.
...
# A requirement document is named by a NUMBER on the trunk and by a SEED
# everywhere else (3.7.16). Both spellings are the SAME document at different
# moments of its life, and every consumer of a requirement id has to accept
# both — issue #315: creation was moved to seeds, the consumers were not, so
# `requirements: [SPEC-a1b2c3d4.FR-001]`, the shape `/polisade:spec` now
# produces, was an `invalid requirements ID format` error until sync ran on the
# trunk. Seed grammar is `polisade_id.SEED_RE` — a letter first, then seven
# `[a-z0-9]` — and it is restated (not imported) because this module is the
# shared, dependency-free bottom of the requirement stack; the regression suite
# pins the two against each other.
_DOC_TAIL = r'(?:\d{3}|[a-z][a-z0-9]{7})'

COMPOSITE_REQ_RE = re.compile(
    r'^(?:(?P<scope>(?:PRD|SPEC|FEAT)-%s)\.)?(?P<req>(?:FR|NFR)-\d{2,3})$' % _DOC_TAIL
)

REQ_REF_SCAN_RE = re.compile(
    r'\b(?:(?:PRD|SPEC|FEAT)-%s\.)?(?:FR|NFR)-\d{2,3}\b' % _DOC_TAIL
)
...
DOC_ID_RE = re.compile(r'^(?:PRD|SPEC|FEAT)-%s$' % _DOC_TAIL)
    ...
    for _kind, rel in DOC_KIND_DIRS:
        ...
        for md_file in sorted(dir_path.glob("*.md")):
            ...
            if not doc_id or doc_id.endswith("-XXX"):
                stem_m = re.match(r'^((?:PRD|SPEC|FEAT)-%s)' % _DOC_TAIL, md_file.stem)
                if not stem_m:
                    continue
