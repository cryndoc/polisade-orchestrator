...
# Portable timeout wrapper: run cmd with stdin from /dev/null, pipe through
# redact, append to $OUT. If cmd does not finish in $1 seconds, send SIGTERM
# then SIGKILL. macOS has no GNU `timeout`, so we roll our own with a
# background killer. stdin redirection is critical — it tells gigacode
# "not a tty", preventing it from booting the interactive TUI.
run_bounded() {
    local to=$1; shift
    ( "$@" </dev/null 2>&1 | redact ) >> "$OUT" &
    local job=$!
    ( sleep "$to" && kill -TERM "$job" 2>/dev/null \
        && sleep 2 && kill -KILL "$job" 2>/dev/null ) >/dev/null 2>&1 &
    local killer=$!
    wait "$job" 2>/dev/null
    local rc=$?
...
# Longer timeout for live model calls (capability probe).
run_live() {
    printf '\n$ %s\n' "$*" >> "$OUT"
    run_bounded 90 "$@"
}
...
# the top-level help (600+ lines of noise).
for sub in mcp extensions; do
    printf '\n--- gigacode %s --help ---\n' "$sub" >> "$OUT"
    run gigacode "$sub" --help
done

section "LIVE LISTS (mcp list)"
# `gigacode -l` / `--list-extensions` is BROKEN in 0.10.0 — instead of
# listing and exiting, it launches the full interactive TUI (hangs for
# minutes until user manually quits). Skip it; the same data is
# available from the `~/.gigacode/extensions/` listing in CONFIG DIRS.
...
if [ "$LIVE" -eq 1 ]; then
    ...
    run_live gigacode --allowed-tools=run_shell_command -p "$PROBE_PROMPT"

    section "CAPABILITY PROBE (with -p only)"
    run_live gigacode -p "$PROBE_PROMPT"
else
