...
def apply_plan(root: Path, corpus_rel: str, plans, *, run_id: str, policy: str,
    ...
    # …и ровно эта сверка теперь ЕДЕТ В ПРИМИТИВ хэш-контрактом. Собственный
    # docstring `_verify_unmoved` говорил: окно между сверкой и записью можно
    # закрыть только ожидаемым хэшем ВНУТРИ примитива — «это его контракт, а не
    # мигратора». Контракт появился в V3-S3.33, но мигратор им не пользовался,
    # и окно оставалось открытым (находка обоих ревью, круг 1).
    # `--expect-strict`: ожидание обязано быть у КАЖДОГО пути плана, иначе
    # «сверено» относилось бы к части файлов, а читалось бы как про все.
    expect_map = {}
    for it in selected:
        if it["status"] == "create":
            expect_map[it["target"]] = "absent"
        else:
            digest = (it.get("conflict", {}).get("targetSha256")
                      or it.get("_target_sha"))
            if not digest:
                raise MigrateError(
                    "E-expect-unknown",
                    "для цели %s/%s не известен ожидаемый sha256 — под "
                    "хэш-контрактом писать вслепую нельзя"
                    % (corpus_rel, it["target"]),
                    hint="перестрой план: без ожидания примитив не отличит "
                         "чужую правку от нашей",
                )
            expect_map[it["target"]] = digest
    expect_file = staging.parent / ("%s.expect.json" % staging.name)
    expect_file.write_text(
        json.dumps(expect_map, ensure_ascii=False, indent=2, sort_keys=True)
        + "\n", encoding="utf-8")
    argv += ["--expect-from", str(expect_file), "--expect-strict"]
