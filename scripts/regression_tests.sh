...
canon = {"id": "session-1a2b3c4d", "messages": [
    {"role": "user", "content":
...
PY
    ...
    # Session-log analyser assertions depend on
    # scripts/ops027_check_session_log.py, which is not shipped in every
    # distribution. Skip gracefully when absent (other test_ops_027
    # assertions above cover the lint side independently).
    if [ ! -f scripts/ops027_check_session_log.py ]; then
        _p "OPS-027 analyser checks skipped (analyser script not in this tree)"
        return 0
    fi
...
assert 'session-1a2b3c4d' in ids, f'canonical issue string not caught: {ids}'
...
" && _p "OPS-027 analyser catches canonical Windows-path" \
