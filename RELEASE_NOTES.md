...
Fix: `tools/convert.py` now wraps skill-asset and `skills/init/templates/*`
rewrites in the same `${PDLC_PLUGIN_ROOT:-<abs>}` expansion that
`{plugin_root}` already uses. Closes a silent OPS-021 violation (#92):
on Qwen/GigaCode extension builds, `skills/<n>/<asset>` and
`skills/init/templates/<file>` references were baked as **bare** absolute
build-tree paths (on release artefacts — the GitHub Actions runner path
`/home/runner/work/...`), so `/pdlc:init` on an end-user machine read
from missing files and the LLM silently reconstructed templates from
memory. Claude Code target was unaffected (no conversion step).

### Converter

- **`tools/convert.py`** — the global `skills/<n>/<asset>` replacement
  (line 697) and the per-skill bare `<asset>/` form (line 703) now emit
  `${PDLC_PLUGIN_ROOT:-<abs>}/assets/<skill>/<asset>` and
  `${PDLC_PLUGIN_ROOT:-<abs>}/templates/init/<file>` — matching the
  treatment `{plugin_root}` already receives. Both forms resolve on the
  install machine via `PDLC_PLUGIN_ROOT=~/.qwen/extensions/pdlc` and
  fall back to the build-time path when the env var is unset.

### Guards

- **`tools/validate.py`** — new `bare_plugin_root_path` check scans every
  converted command body for literal occurrences of the extension root
  that are **not** inside a `${PDLC_PLUGIN_ROOT:-…}` wrapper. Runs as a
  hard-fail gate in the Qwen build and release workflows. Deterministic
  (literal match against `str(ext.resolve())`), not a heuristic regex.
- **`scripts/regression_tests.sh` — `test_ops_021`** — new assert with
  the same semantics: `bash scripts/regression_tests.sh --ops=021`
  catches a regression of this converter contract.
