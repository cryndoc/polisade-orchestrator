...
footgun observed in a GigaCode 0.16.0 session where the agent parsed
«закоммить всё, КРОМЕ `.gigacode/`» as «force-add `.gigacode/`» and
executed `git add -f .gigacode/`. Text-only fix (prompt guards +
linter + regression + harness); no new runtime code.
...
- **Reminder bullet** in `skills/pr/SKILL.md` `## Важно` covering the
  third canonical guard surface per issue #74 acceptance #2.
...
- `test_ops_027` in `scripts/regression_tests.sh` with 15 checks
  (positive live repo, positive grep × 3 canonical locations,
  negative fixtures × 3, post-convert bullet+marker verification,
  gitignore coverage, init append-block audit, PROJECT_STATE schema,
  session-log analyser on canonical Windows-path from issue #74 +
  backslash variant + clean-session sanity).

### Reproduction + post-hoc tooling

- `scripts/ops027_repro_harness.sh` — portable bash harness for
  corp-contour live reproduction. Flags: `--cli={claude-code,qwen,
  gigacode,manual}`, `--surface={claudemd,implement,both}`,
  `--build-local`, `--bin=<path>`. Preflight guard-check via shared
  lint helpers, isolated `pdlc-ops027-test` extension for `--build-local`,
  `_with_timeout` fallback for hosts without GNU coreutils.
- `scripts/ops027_check_session_log.py` — stdlib-only post-hoc
  analyser for exported CLI session JSON. Regex handles Windows-style
  paths (`c:/...`, `c:\...`) and basename-match covers the canonical
  issue-#74 pattern («exclude absolute path» vs «force relative»).

Closes #74. Related: #53, #62, #68, #75.
