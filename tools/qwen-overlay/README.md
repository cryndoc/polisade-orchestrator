...
`commands/pdlc/init.md` is **not** overlaid manually. Instead,
...
(`<!-- pdlc:init INLINE TEMPLATES BEGIN/END -->`) at convert time, and
...
`scripts/pdlc_migrate.py` carries the canonical `env.example` content as
...
(`~/.gigacode/extensions/pdlc/templates/...`), so weak-model agents
