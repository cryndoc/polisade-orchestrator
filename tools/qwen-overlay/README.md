...
| Override | Why |
|---|---|
| `commands/polisade/review.md` | The source skill shells out to `codex exec --full-auto -m gpt-5.3-codex ...` to get an independent reviewer. In the Qwen target there's no external reviewer CLI dependency — instead a clean-context Qwen subagent does the review directly via the Task tool. The `self` flag is accepted for CLI compatibility but is effectively a no-op (Qwen always uses subagent). The prompt and response format are identical to the source; only the execution mechanism changes. |
| `commands/polisade/review-pr.md` | Same idea for the PR-level review. The subagent now does the `gh pr diff` / `gh pr view` pre-fetch and the `gh pr comment` publish itself, instead of wrapping an external reviewer call. The `self` flag is accepted for compatibility. |
...
## Why some skills are NOT overlaid (issue #119)

`commands/polisade/init.md` is **not** overlaid manually. Instead,
`tools/convert.py:_inline_init_templates()` auto-embeds the canonical bytes
of every `skills/init/templates/*` resource between sentinel markers
(`<!-- polisade:init INLINE TEMPLATES BEGIN/END -->`) at convert time, and
`_rewrite_init_step_4()` replaces the Read+Write paragraph with a
Write-only imperative pointed at the inline section. The shipped
`scripts/polisade_migrate.py` carries the canonical `env.example` content as
a module-level `_CANONICAL_ENV_EXAMPLE` literal regenerated via
`scripts/_regen_canonical_env_example.py`. Reason: GigaCode CLI 0.10.0
Filesystem Guard read-protects the plugin install dir
(`~/.gigacode/extensions/polisade/templates/...`), so weak-model agents
cannot Read template files at runtime. A static overlay would freeze the
template content and silently drift; the auto-embed path keeps source =
target by construction. Strict post-build gate
(`_strict_post_build_checks`) and lint check
(`check_init_inline_markers` / `check_migrate_canonical_env_example`)
together enforce that drift cannot ship. See issue #119,
`docs/gigacode-cli-notes.md`, and CLAUDE.md invariant #12.
