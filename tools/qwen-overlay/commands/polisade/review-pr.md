...
/polisade:review-pr 42 self        # То же (self — default для Qwen)
...
gh pr list --head <ветка из предыдущей команды> --json number --jq '.[0].number'
```

Две команды, а не одна с `$(...)`: корп-шелл режет подстановку команд
(issue #331). Имя ветки подставляется в следующий вызов как текст.
...
Выполни (вывод — в файлы, не в переменные: корп-шелл режет `$(...)`, issue #331):
  mkdir -p ${POLISADE_WORK_DIR:-.}/.polisade/tmp
  gh pr diff ${PR_NUM} > ${POLISADE_WORK_DIR:-.}/.polisade/tmp/pr-diff.txt
  gh pr view ${PR_NUM} --json title,body,files --jq '{title,body,files}' > ${POLISADE_WORK_DIR:-.}/.polisade/tmp/pr-desc.json

Прочитай оба файла и держи их содержимое в контексте для следующего шага
как `${PR_DIFF}` и `${PR_DESC}`.
...
<!-- polisade:exec-denied CAPSULE END -->
  ...
  ${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_diff_smells.py --diff-file ${POLISADE_WORK_DIR:-.}/.polisade/tmp/pr-diff.txt --project-root "${POLISADE_WORK_DIR:-.}" > ${POLISADE_WORK_DIR:-.}/.polisade/tmp/smells.txt

Прочитай `${POLISADE_WORK_DIR:-.}/.polisade/tmp/smells.txt` и подставь содержимое вместо `${SMELLS}`. Дифф берётся
из файла, записанного на шаге 1, — ни конвейера из переменной, ни `$(...)`.
...
- Приёмка (issue #343): секция ВСЕГДА, включая «не заведена» — отсутствие
  приёмки это факт о PR, а не пустое место. Данные берутся детерминированно,
  из того же расчёта, что читают `/polisade:doctor`, `/polisade:state` и
  описание PR; чеки при этом НЕ исполняются:
...
<!-- polisade:exec-denied CAPSULE END -->
  ...
  Печатай поле `summary` как есть, своими словами не переписывай. Три вещи,
  которые ревьюер обязан увидеть, если они там есть: красные пары, **stale**
  (отчёт снят на ДРУГОМ коде — это утверждение не об этом PR) и **разошедшаяся
  база** (изменён чек либо ПРИБОР — тест-файл, который чек запускает; второй
  способ купить зелень).
  ⛔ **Не отработавшая команда — это «не удалось определить» с дословным
  отказом, а НЕ «приёмка не заведена»** (issue #356): второе — ответ
  инструмента, а не заполнитель пустоты. Строку `summary` печатай дословно —
  её наличие и есть свидетельство, что команда отработала.
  ⛔ **Балла не ставит и мерж не блокирует.** Приёмка здесь best-effort: файл
  проверок лежит в репозитории, виден модели и доступен ей на запись. Это
  знание для решения ревьюера, а не гарантия — подавать её как гарантию
  запрещено (класс F1).
...
- Приёмка: {summary из `polisade_acceptance.py status` — как есть} — БЕЗ балла: best-effort практика, не гарантия (issue #343)
...
Запиши текст ниже в `${POLISADE_WORK_DIR:-.}/.polisade/tmp/review.md` и опубликуй его файлом — `--body-file`,
не `--body "$(...)"`: тот же приём, которым рецепт #108 передаёт тело PR
(issue #331).

  gh pr comment ${PR_NUM} --body-file ${POLISADE_WORK_DIR:-.}/.polisade/tmp/review.md
