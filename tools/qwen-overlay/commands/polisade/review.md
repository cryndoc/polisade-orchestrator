...
${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_diff_smells.py --base <base> --family test-smells --project-root "${POLISADE_WORK_DIR:-.}" > ${POLISADE_WORK_DIR:-.}/.polisade/tmp/smells.txt
```

Затем прочитай `${POLISADE_WORK_DIR:-.}/.polisade/tmp/smells.txt` и подставь содержимое вместо `${SMELLS}`.
Присваивание через `$(...)` здесь НЕ используется: корп-шелл (GigaCode CLI)
режет подстановку команд с сообщением «Command substitution using $(), ``,
<(), or >() is not allowed for security reasons» — и капсула выше объяснила бы
этот отказ Filesystem Guard'ом, то есть неверной причиной (issue #331).
Перенаправление в файл корп-шелл пропускает — тем же приёмом рецепт #108
готовит тело PR.
