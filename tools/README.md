...
python3 scripts/polisade_lint_skills.py .

# 2. Convert to Qwen extension, applying the overlay
python3 tools/convert.py . \
    --out build/qwen-ext/polisade \
    --overlay tools/qwen-overlay

# 3. Validate the converted extension
python3 tools/validate.py build/qwen-ext/polisade
```
...
- **Adding a new Qwen-only override:** drop the file under `qwen-overlay/<rel-path-inside-extension>` matching the layout the converter produces (e.g. `qwen-overlay/commands/polisade/<name>.md`). On the next conversion, your file replaces the auto-generated one. Document the override in `qwen-overlay/README.md` so future maintainers know why it exists.
