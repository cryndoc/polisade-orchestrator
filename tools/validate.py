...
def validate(ext: Path) -> int:
    ...
    # OPS-021: no bare absolute build-root paths in command bodies.
    # Every absolute path pointing into the extension root must be wrapped
    # in a ${POLISADE_PLUGIN_ROOT:-…} expansion, or it won't resolve on a
    # machine where the install path differs from the build path (e.g.
    # release artefacts built on a GitHub Actions runner). Deterministic:
    # literal match against the concrete ext root, not a regex heuristic.
    ext_abs = str(ext.resolve())
    wrapper_re = re.compile(r'\$\{POLISADE_PLUGIN_ROOT:-[^}]+\}')
    bare_leaks: list[str] = []
    for md in sorted((ext / "commands").rglob("*.md")):
        text = md.read_text(encoding="utf-8", errors="ignore")
        for lineno, line in enumerate(text.split("\n"), start=1):
            if ext_abs not in line:
                continue
            if ext_abs in wrapper_re.sub("", line):
                bare_leaks.append(
                    f"{md.relative_to(ext)}:{lineno}: {line.strip()[:140]}"
                )
    if bare_leaks:
        failures += 1
        print(f"FAIL bare_plugin_root_path ({len(bare_leaks)} hits):")
        for hit in bare_leaks[:5]:
            print(f"    {hit}")
        if len(bare_leaks) > 5:
            print(f"    ... and {len(bare_leaks) - 5} more")
    else:
        passed += 1
        print("PASS bare_plugin_root_path")
