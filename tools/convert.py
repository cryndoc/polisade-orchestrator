...
# ---------------------------------------------------------------------------
_TASKS_INLINE_BEGIN = "<!-- pdlc:tasks INLINE REFERENCES BEGIN -->"
_TASKS_INLINE_END = "<!-- pdlc:tasks INLINE REFERENCES END -->"
_TASKS_INLINE_LABEL = "pdlc:tasks INLINE REFERENCES"

# Reference files inlined into the converted /pdlc:tasks command, in appendix
# order. All seven references the command reads at runtime — the six
# prompt/format templates plus the compute-next-id protocol.
...
def _inline_skill_references(
    ...
    blocks: list[str] = [
        ...
        "",
        "<!-- AUTO-GENERATED by tools/convert.py — do not edit by hand. -->",
        "<!-- Bundle: pdlc tasks reference payload. Issue #139. -->",
        "",
...
def _rewrite_tasks_reference_reads(content: str) -> str:
    ...
    /pdlc:tasks command body to point at the inline-references appendix
    ...
    `${PDLC_PLUGIN_ROOT:-...}/assets/tasks/references/...` install-dir read.
    ...
    if not skills_dir.is_dir():
        ...
        for skill_dir in sorted(skills_dir.iterdir()):
            ...
            for asset in skill_dir.iterdir():
                ...
                #
                # Issue #139: for /pdlc:tasks the references/ are inlined into
                # the command body (see _inline_skill_references below), so the
                # bare-`references/` → install-dir rewrite must NOT fire — it
                # would re-introduce the very Guard-denied read path the inline
                # embed exists to remove. The directive-rewrite + inline run in
                # the conversion loop instead.
                if (
                    asset.is_dir()
                    and asset.name not in GENERIC_ASSET_NAMES
                    and not (skill_dir.name == "tasks" and asset.name == "references")
                ):
        ...
        for skill_dir in skill_dirs:
            ...
            # Issue #139: neutralise the /pdlc:tasks "Прочитай references/..."
            # read directives BEFORE rewrite_paths so the global asset-map
            # rewrite cannot turn them into install-dir read paths.
            if skill_dir.name == "tasks":
            ...
            # Issue #139: for /pdlc:tasks, fold the verbatim references/ bytes
...
def _env_write_ban_violations(body: str, where: str) -> list[str]:
    ...
    and the emitted `pdlc-init` Agent Skill must NOT:
...
def _tasks_inline_violations(body: str, where: str) -> list[str]:
    """Issue #139 contract for a shipped /pdlc:tasks artefact (the command and
    ...
    errs: list[str] = []
    if re.search(r"\$\{PDLC_PLUGIN_ROOT:-[^}]+\}/assets/tasks/references/", body):
        errs.append(
            f"strict[#139]: {where} references "
            "`${PDLC_PLUGIN_ROOT:-...}/assets/tasks/references/` — tasks "
...
def _strict_post_build_checks(
    ...
    if not init_md.exists():
        ...
        errors.extend(_env_write_ban_violations(body, "commands/pdlc/init.md"))

    # (3c) issue #128 finding #1: the emitted Agent Skill `pdlc-init/SKILL.md`
    # is what GigaCode actually loads for `/pdlc:init` intent. Assert it
    ...
    if not init_skill.exists():
        errors.append(
            ...
            "so GigaCode can route `/pdlc:init` intent to the inline content"
        )
    else:
        sbody = init_skill.read_text(encoding="utf-8")
        if re.search(r"\$\{PDLC_PLUGIN_ROOT:-[^}]+\}/templates/", sbody):
            errors.append(
                f"strict[#128]: {init_skill.relative_to(out_dir)} references "
                "`${PDLC_PLUGIN_ROOT:-...}/templates/` — emitted skill must "
    ...
    # (6) issue #139: the /pdlc:tasks references are inline-embedded for the
    # Qwen/GigaCode bundle. Assert both the command and the emitted Agent Skill
    # (GigaCode routes `/pdlc:tasks` intent to the latter) carry the inline
    ...
    if not tasks_skill.exists():
        errors.append(
            ...
            "route `/pdlc:tasks` intent to the inline content"
        )
    else:
        errors.extend(
            _tasks_inline_violations(
