...
def init_flat_docs_templates() -> frozenset:
    """Names under `templates/init/docs/` whose home is `docs/templates/<same>`.

    Read off `_INIT_INLINE_BUNDLE` — the place where `/polisade:init` already
    declares where each template lands — so that "which files are artefact
    templates" is stated once. `scripts/polisade_migrate.py` carries the
    complement of this set (the names it does NOT own) and
    `polisade_lint_skills.py::check_docs_templates_targets` asserts the two
    agree.
    """
    flat = set()
    for src, target, _lang, _step in _INIT_INLINE_BUNDLE:
        if not src.startswith("docs/") or "/" in src[len("docs/"):]:
            continue
        name = src[len("docs/"):]
        if target == "docs/templates/" + name:
            flat.add(name)
    return frozenset(flat)


def vendor_init_docs_templates(out_dir: Path) -> int:
    """Move `templates/init/docs/*.md` into `scripts/templates/docs/` (#341).

    GigaCode only, same reasoning and same shape as `vendor_reference_assets`.
    `/polisade:migrate` now delivers the artefact templates a project never got
    (a project that came from 2.x has no `change-spec-template.md`, and
    `/polisade:spec` reads that file FROM THE PROJECT). The delivering step is
    `polisade_migrate.py`, which under GigaCode runs out of the vendored set —
    and the install dir it would otherwise read is Guard-denied (#127). So the
    canonical bytes travel WITH the scripts, get hashed by the same manifest
    and copied by the same installer: one vendored set, one verification.

    MOVED, not copied: nothing in the GigaCode build reads `templates/init/docs`
    at runtime — `/polisade:init` carries its own inline canonical bytes
    (#119) — so a second copy would ship the same 70 KB twice and leave a path
    in the bundle that no command asks for. `templates/init/` itself stays: its
    GIGACODE.md sibling is asserted by the release job and is a different file.

    Subdirectories are deliberately left alone. `templates/init/docs/conventions/`
    is the slot for the team's own rules — `/polisade:init` writes it only when
    absent, and a migration has no business delivering it at all. So is
    `contracts-readme-template.md`, whose home is `docs/contracts/README.md`
    and not `docs/templates/`: the vendored set is exactly what the migration
    delivers, and the WHICH is read off `_INIT_INLINE_BUNDLE` rather than
    restated here — `/polisade:init` already declares where every template
    lands, and a second list of the same fact is how the two diverge.
    """
    src = out_dir / "templates" / "init" / "docs"
    scripts_dir = out_dir / "scripts"
    if not src.is_dir() or not scripts_dir.is_dir():
        return 0
    dest = scripts_dir / "templates" / "docs"
    moved = 0
    for path in sorted(src.glob("*.md")):
        if path.name not in init_flat_docs_templates():
            continue
        if path.is_symlink():
            # Refused, not skipped — the same escape `shutil.copytree` walked
            # into in #300: a link vendors whatever it points at, from outside
            # the bundle, under a canonical name.
            raise SystemExit(
                f"template `{path.name}` is a symlink; a vendored canon must be "
                f"real files, or the migration writes bytes from outside the "
                f"delivery into a project"
            )
        target = dest / path.name
        if target.exists():
            raise SystemExit(
                f"cannot vendor template `{path.name}`: {target} already exists "
                f"in the bundle. Moving onto it would destroy whatever shipped "
                f"there — rename the collision, do not let the build decide"
            )
    ...
    if plugin_scripts.is_dir():
        ...
        for script in (out_dir / "scripts").rglob("*.py"):
            ...
            if ".claude/settings.json" in content:
                # Говорим то, что ЗНАЕМ. Вердикт «код мёртв» конвертеру
                # недоступен: он грепает имя файла и не знает, есть ли в
                # функции ветка под этот контур. После #337 такая ветка есть —
                # `check_settings_permissions` определяет контур и печатает,
                # где у этого CLI лежат разрешения, — и прежний совет «удали
                # функцию вручную» ломал бы фикс. Строка живёт в файле, который
                # модель грузит каждый ход, а у репозитория есть история того,
                # как слабая модель правит скрипты (#182): совет удалять там —
                # приглашение (#360).
                summary["warnings"].append(
                    f"scripts/{script.name} mentions .claude/settings.json, "
                    f"which does not exist in the converted {target} build — "
                    f"the script is expected to handle its absence"
                )
...
# ── Кому адресовано предупреждение сборки ──────────────────────────────────
# Раздел «Conversion warnings» едет в контекстный файл, который модель грузит
# КАЖДЫЙ ХОД. Часть строк там — факты о сборке, с которыми модель реально
# столкнётся («файла разрешений в этой сборке нет»), и им там место. Часть —
# диагностика для СБОРЩИКА: догадки («likely a meta-skill»), отчёт о
# пропущенных оверлеем файлах. Модели они не адресованы, место в постоянном
# контексте занимают, а одна такая строка ещё и советовала удалить живой код
# (#360). Строки остаются в выводе сборки и в CI — просто не едут в промпт.
_BUILDER_ONLY_WARNING_MARKS = (
    "likely a meta-skill",
    "overlay: skipped",
)


def _context_warnings(warnings):
    """Подмножество предупреждений, адресованных МОДЕЛИ, а не сборщику."""
    return [w for w in warnings
            if not any(m in w for m in _BUILDER_ONLY_WARNING_MARKS)]
...
def build_qwen_md(
    ...
    lines += [
        ...
        "",
        "## Project state is written by scripts, not by hand",
        "",
        "NEVER edit `.state/*.json` with the Write/Edit tools — not to flip a "
        "setting, not to add a field, not 'just one value'. Those files are "
        "written by the plugin's own scripts, which write atomically, report "
        "what changed, stage the result for the commit and say what the "
        "change still needs. A hand edit does none of that, and the PM gets "
        "no plan, no `stage_paths` and no warning that the mode it enabled "
        "needs a template that is not there yet.",
        "",
        "To switch an experimental mode on or off, run "
        "`/polisade:migrate --enable=<flag>` / `--disable=<flag>` (dry-run "
        "first, then `--apply`). Measured: asked to 'enable changeSpec' in "
        "plain words, the model edited the state file directly — which is "
        "exactly what that command exists to replace.",
        "",
        "## Commands",
        "",
    ]
    ...
    _ctx_warnings = _context_warnings(summary["warnings"])
    if _ctx_warnings:
        lines += ["## Conversion warnings", ""]
        for w in _ctx_warnings:
            lines.append(f"- {w}")
        lines.append("")
...
def main() -> None:
    ...
    if summary.get("_is_gigacode"):
        ...
        # #341 — the artefact templates join the same vendored set, for the
        # same reason: the step that delivers them runs from the project, and
        # the install dir it would otherwise read is Guard-denied.
        summary["vendored_templates"] = vendor_init_docs_templates(out_dir.resolve())
