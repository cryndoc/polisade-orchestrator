...
def write_scripts_manifest(out_dir: Path, plugin_version: str, target: str) -> int:
    ...
    lines = [
        ...
        "# project (issue #127). Install/upgrade from a normal terminal:",
        "#   mkdir -p .polisade && rm -rf .polisade/bin",
        "#   cp -R <unpacked extension>/scripts .polisade/bin",
        "# then commit it. /polisade:doctor and /polisade:init-verify verify",
        "# this manifest against the copy.",
    ]
   ...
   b) Если проверка не прошла — **STOP**. Выведи PM ровно это и жди; шаги 1–7
      ...
      ⛔ Рантайм-скрипты не установлены в проект (или копия не совпадает с
         манифестом). Выполни в ОБЫЧНОМ терминале (не в CLI), в корне проекта:
           ...
           mkdir -p .polisade
           rm -rf {GIGACODE_SCRIPTS_ROOT_DEFAULT}
           cp -R <распакованный архив расширения>/scripts {GIGACODE_SCRIPTS_ROOT_DEFAULT}
           git add {GIGACODE_SCRIPTS_ROOT_DEFAULT} && git commit -m "chore: vendor polisade runtime scripts"

         `mkdir -p .polisade` обязателен: в свежем проекте родительского
         каталога ещё нет, и `cp -R` без него не создаст его сам. `rm -rf`
         перед копированием — тоже: иначе от прошлой версии останутся файлы,
         которых нет в новом манифесте.

         Если в окружении выставлен `POLISADE_SCRIPTS_ROOT` — копируй в ЭТОТ
         каталог, а не в `{GIGACODE_SCRIPTS_ROOT_DEFAULT}`: проверка смотрит
         именно туда и точный путь называет в своём сообщении.
...
def build_qwen_md(
    ...
    if has_scripts and is_gigacode:
        ...
        lines.append(
            ...
            f"(`mkdir -p .polisade && cp -R <unpacked bundle>/scripts "
            f"{GIGACODE_SCRIPTS_ROOT_DEFAULT}`) "
            f"and verified against `{GIGACODE_SCRIPTS_ROOT_DEFAULT}/"
