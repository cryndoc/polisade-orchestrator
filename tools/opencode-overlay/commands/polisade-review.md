...
Команда уже исполняется изолированным субагентом (`subtask: true`). Субагент сам навигирует проект через Read/Glob/Grep, наружу запускает только advisory-сканер из шага 1.5. Выполни ревью по следующему промпту:
