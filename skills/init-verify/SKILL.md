---
name: init-verify
description: 'Verify that /pdlc:init produced a canonical project structure (valid .state JSON, real pdlcVersion/schemaVersion, context file present) rather than content reconstructed from memory. Use when PM mentions "verify init", "check init", "validate project structure", "проверь init", "проверь инициализацию", or right after /pdlc:init to confirm the generated files are byte-correct before reporting success.'
---

# /pdlc:init-verify — Verify Polisade Orchestrator init output
...
`{"pdlcVersion": "...", "schemaVersion": 5}`, or an `.env.example` full of
...
1. Run the deterministic check below via `run_shell_command`. It reads only
   ...
   # match is enforced by pdlc_lint_skills.py::check_version_consistency
   # (invariant #1, 5th source) so this literal cannot silently drift.
   EXPECTED_PDLC_VERSION = "2.24.2"
   ...
   if isinstance(state, dict):
       ...
       if "version" in state:
           fails.append(
               ...
               "(canonical schema uses pdlcVersion + schemaVersion)"
           )
       # pdlcVersion must EQUAL the current release — not just look like a
       # semver. A stale `2.24.1` in a `2.24.2` install means the file was
       # reconstructed (or a version-lockstep break) and must fail loud.
       pv = state.get("pdlcVersion")
       if pv != EXPECTED_PDLC_VERSION:
           fails.append(
               f"{ps_path} pdlcVersion {pv!r} != expected "
               f"{EXPECTED_PDLC_VERSION!r} (stale or reconstructed)"
           )
       if state.get("schemaVersion") != 5:
           fails.append(
               f"{ps_path} schemaVersion != 5 (got {state.get('schemaVersion')!r})"
           )
   ...
   if fails:
       ...
       print(
           ...
           "canonical bytes. Re-run /pdlc:init — do NOT invent or paraphrase "
...
2. Report the result:
   ...
   - Exit 1 / one or more `FAIL:` lines → report **FAIL** and surface every
     `FAIL:` line verbatim. The caller (`/pdlc:init` step 6.8) must STOP and not
     report success.
