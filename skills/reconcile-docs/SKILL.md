...
```bash
${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_reconcile.py anchors --root . --json
```
...
```bash
${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_reconcile.py record --root . --stdin < findings.json
```
...
### Шаг 3.5 — Сверка с ВНЕШНИМ эталоном (только если PM его передал)

Отдельный режим и **отдельная секция отчёта**. Включается, когда PM передал
reference-спеку — файл или папку со схемами (JSON Schema / OpenAPI / AsyncAPI):
«сверь `async-api.md` со схемами, которые я прислал». Не передал — режима нет,
и в отчёте про него ничего не пишется.

⚠️ Это ДРУГАЯ сверка, и путать её с основной нельзя:

| | Шаги 1–3 (основная) | Шаг 3.5 (этот) |
|---|---|---|
| С чем сверяем | корпус ↔ **код** | артефакт ↔ **документ, который дал пользователь** |
| Кто сверяет | модель, best-effort | скрипт, механически |
| Что на выходе | мнение с уверенностью | таблица расхождений по 4 измерениям |
| Чем НЕ является | вердиктом о корпусе | вердиктом о корпусе **и** сверкой с кодом |

Совпадение полей с эталоном **не** говорит ничего о коде и **не** повышает
провенанс корпуса: `INFERRED`/`GAP` до и после, `CONFIRMED` не ставится.
...
Эталоном может быть файл или каталог; каталог скрипт обходит сам и печатает
список источников со статусом каждого — вручную сводить папку не надо.

- exit **0** — «расхождений по сверенным измерениям нет» (имя, тип,
  `format`/`pattern`, `required`). ⛔ Не переписывай это как «артефакт
  соответствует эталону»: несравниваемые измерения перечислены в `--rules`.
- exit **1** — таблица расхождений идёт в секцию отчёта целиком.
- exit **2** — не разобрано. Процитируй `файл:строка` и скажи, что сверка не
  выполнена. Неразбор — не «чисто».

⛔ Результат этого шага **не попадает** в `findings.json` шага 3. Это не
расхождение «корпус ↔ код», у него нет `kind` из закрытого словаря и нет
`code_ref`; попытка положить его туда — либо `E-RC-MISSING-FIELD`, либо (если
подписать «вердикт») `E-RC-VERDICT-CLAIM`. Секция живёт в отчёте PM рядом с
основной, отдельным блоком:

```
──────── СВЕРКА С ВНЕШНИМ ЭТАЛОНОМ (не с кодом) ────────
Эталон:   <путь, переданный PM>
Артефакт: <файл корпуса/силоса>
Рамка:    сверка ДВУХ ДОКУМЕНТОВ по имени, типу, format/pattern, required.
          Это НЕ сверка с кодом и НЕ вердикт о корпусе; провенанс не меняется.
Итог:     <таблица | «расхождений по сверенным измерениям нет» | «не разобрано: файл:строка»>
────────────────────────────────────────────────────────
```

⛔ Расхождение эталона и артефакта не объявляется «опечаткой» и не приводится
к одному имени/формату без PM (дисциплина #88) — оно выносится как открытый
вопрос **DECISION NEEDED** и до ответа PM не чинится:
...
Правка артефакта, если PM её подтвердил, идёт обычным путём шага 5 (через
примитив записи), а не «заодно».
...
`${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_corpus_io.py` (пофайловая атомарность,
...
3. Собери правки в staging `.polisade/tmp/design-corpus/<run-id>/staging/`
   (тот же формат корпуса — раскладку задаёт `/polisade:design-corpus`,
   и её описание доступно там, где доступен он сам: скилл помечен
   `claude_only`, поэтому в сборках Qwen/GigaCode/opencode ни его, ни его
   справочников нет, и ссылаться на них оттуда — значит послать читателя по
   несуществующему пути. Провенанс правленого файла остаётся `INFERRED`/`GAP`
   — **`CONFIRMED` не ставится никогда**).
