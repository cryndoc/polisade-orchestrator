...
# /polisade:reconcile-docs [SPEC-XXX | DESIGN-XXX] — Advisory drift detection
...
> `architecture.corpus.mode == "living"` в `.state/PROJECT_STATE.json`, дизайн
> живёт в **едином корпусе** `docs/architecture/`, а не в per-SPEC пакете
> `DESIGN-NNN-*/`. Тогда:
> - системный слой (C4 L1/L2, glossary, data-model) — **system-wide, без
>   single SPEC-parent**: реконсиль его против корпуса целиком, а не против
>   одного пакета;
> - per-SPEC дельта читается из `docs/specs/SPEC-NNN/changeset.yaml` +
>   `docs/architecture/trace.json` (FR → element), не из package-manifest;
> - отсутствие per-SPEC `DESIGN-PKG` — НЕ повод останавливаться (ниже): в
>   corpus-режиме это норма.
...
4. Прочитай `manifest.yaml` — перечень design-артефактов и `realizes_requirements` (composite `{DOC}.FR-NNN` — см. «Requirement ID Scoping» в CLAUDE.md; drift между manifest и sub-artifact frontmatter lint ловит отдельно)
