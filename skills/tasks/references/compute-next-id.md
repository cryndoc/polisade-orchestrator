...
## 1a. Артефакт с СЕМЕНЕМ (идентичность до номера)

Номер нельзя выдать безопасно на ноутбуке: `.state/counters.json` — файл клона,
а протокол требует держать его монотонным «даже если артефакты удалены», то
есть он несёт отметку над УДАЛЁННЫМИ артефактами, которой нет ни в одном
закоммиченном файле. На свежем клоне следующий номер поэтому другой — замерено
на репозитории, где `TASK-004` создали и удалили: у автора `005`, у коллеги
`004`. Двое создают `TASK-005` дважды, и ни одна машина об этом не узнает.

Поэтому у артефакта две фазы:

* **семя** чеканится при создании, на любой машине и любой ветке:
  `polisade_id.py new-seed` → восемь символов `[a-z][a-z0-9]{7}`. Живёт в
  `id: TASK-<семя>`, в имени файла и в поле `seed:`;
* **номер** выдаёт `/polisade:sync`, запущенный **на транке**, и только там.
  Одно место не может столкнуться само с собой. Файл переименовывается,
  `id:` переписывается, **`seed:` остаётся навсегда**.

Семя остаётся, потому что ссылку, сделанную ДО нумерации — сообщение коммита,
заголовок PR, строку в переписке — переписать нельзя. `polisade_id.py resolve
<семя>` отвечает на неё и через год. Прозу внутри артефактов sync не трогает:
это чужой текст, и он остаётся верным ровно потому, что семя не менялось.

🚨 **Извлекателей ниже это НЕ КАСАЕТСЯ, и не по договорённости.** Первый символ
семени — буква, поэтому `stem.split("-")[1].isdigit()` на нём ложь, а
`^SPEC-(\d+)` не матчится. Ненумерованный артефакт невидим для `max()` **по
построению**: ни одна формула в таблице §3 не меняется. Ровно это и позволило
ввести схему, не трогая ни одного извлекателя.

Отсюда же требование к формату: семя, которое могло бы оказаться числом,
попало бы в `max()` как номер.
...
**`/polisade:tasks`** — batch создание TASK внутри одной сессии. После первого
...
**`/polisade:design`** — читает счётчики `DESIGN` и `ADR` (ADR создаётся
subagent'ом внутри `/polisade:design`). Guard идёт на оба типа отдельно.
