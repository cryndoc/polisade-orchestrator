---
name: tasks
description: Create TASKs from PLAN/SPEC/FEAT/BUG
argument-hint: "[PLAN-XXX | SPEC-XXX | FEAT-XXX | BUG-XXX]"
cli_requires: "task_tool"
---

# /pdlc:tasks [PLAN-XXX | SPEC-XXX | FEAT-XXX | BUG-XXX] — Создание задач через субагент

Создание атомарных задач из плана, спецификации, Feature Brief или Bug Report через изолированный субагент.
...
### Для SPEC/FEAT/BUG (один субагент)
...
┌─────────────────────────────────────────────────────────────┐
│  ОСНОВНОЙ АГЕНТ                                             │
│  1. Валидация: SPEC/FEAT/BUG со статусом ready              │
...
│  Input: SPEC/FEAT/BUG + project context                     │
│                                                             │
│  Делает:                                                    │
...
2. Найди PLAN, SPEC, FEAT или BUG со статусом `ready`
3. Если указан ID — проверь что он `ready`
4. Если не указан:
   - Покажи список всех ready артефактов (PLAN, SPEC, FEAT, BUG)
...
Прочитай и собери:
1. **Исходный документ** (PLAN, SPEC, FEAT или BUG)
...
4. **Out of scope и Constraints** (из source SPEC, если source — SPEC или PLAN→SPEC):
   ...
   - Если source — FEAT/BUG без SPEC в chain: "N/A"
5. **System boundary** (из SPEC frontmatter, если source — SPEC или PLAN→SPEC):
   ...
   - Если source — FEAT/BUG без SPEC: "N/A"
...
5b. ПОКРЫТИЕ ТРЕБОВАНИЙ (ОБЯЗАТЕЛЬНО)
   ...
   - Извлеки ВСЕ FR-NNN и NFR-NNN из секций 5 (Functional Requirements) и 6
     (Non-Functional Requirements) parent SPEC
...
6. SELF-REVIEW (ОБЯЗАТЕЛЬНО — выполни для КАЖДОЙ задачи)
   ...
   □ REQUIREMENTS_FRONTMATTER: Все TASKs имеют requirements: [FR-NNN, NFR-NNN]
     во frontmatter, ссылающиеся на конкретные FR/NFR из parent SPEC?
     Если parent — FEAT/BUG/CHORE без SPEC, requirements: [] допустимо.
...
3. Frontmatter ОБЯЗАТЕЛЬНО содержит:
   ...
   **design_refs при наличии DESIGN-PKG** (mapping уже проверен основным агентом в шаге 2.6):
   ...
   Если parent — FEAT/BUG/CHORE без SPEC → requirements: [], design_waiver: false
...
```
Ты — senior developer, создающий задачу на исправление бага.

═══════════════════════════════════════════
SYSTEM ROLE: Bug Fix Task Planner
═══════════════════════════════════════════

Твоя задача — преобразовать bug report в одну (реже 2-3) атомарную задачу,
которую агент сможет реализовать автономно, без уточняющих вопросов.

ПРИНЦИПЫ РАБОТЫ:

1. ВЕРИФИКАЦИЯ ЧЕРЕЗ КОД (КРИТИЧНО)
   - Прочитай ВСЕ файлы, упомянутые в баге ("Связанные файлы")
   - Найди ВСЕ места, где проявляется описанная проблема
   - Проверь, нет ли аналогичной проблемы в ДРУГИХ файлах (часто баги дублируются в нескольких местах)
   - Составь полный список мест для исправления

2. SCOPE
   - Для большинства багов — 1 TASK достаточно
   - Создавай 2-3 TASK только если баг требует изменений в разных подсистемах с разными зависимостями
   - Фикс + тесты = один TASK (не разделяй)

3. КОНКРЕТНОСТЬ
   - Укажи файлы, функции/классы (НЕ номера строк — они fragile)
   - Опиши expected vs actual поведение
   - Приведи конкретный алгоритм исправления
...
4. SELF-REVIEW — выполни общий чеклист (принцип 6 выше), включая REQUIREMENTS_FRONTMATTER,
   FR_COVERAGE, DESIGN_REFS, NFR_VERIFIABILITY. Для BUG без parent SPEC:
   - REQUIREMENTS_FRONTMATTER → requirements: [] допустимо (graceful)
   - FR_COVERAGE → N/A (нет parent SPEC с FR)
   - DESIGN_REFS → обычно [] (BUG редко имеет DESIGN-PKG)
   - NFR_VERIFIABILITY → применимо, если баг связан с NFR (regression по perf/security)

   Дополнительно для BUG:
   □ Покрывает ли задача ВСЕ найденные в коде места с проблемой?

═══════════════════════════════════════════
BUG REPORT
═══════════════════════════════════════════

{полное содержимое BUG файла}
...
2. Для TASK заполни:
   - Frontmatter (id, title, status: ready, parent: BUG-XXX, priority, depends_on: [])
   - Контекст проблемы (из bug report + что нашёл в коде)
   - Цель фикса (expected behavior)
...
Task tool:
  subagent_type: "general-purpose"
  description: "Create TASKs from {SPEC-XXX/FEAT-XXX/BUG-XXX}"
...
5. Обнови `.state/PROJECT_STATE.json`:
   ...
   - Если SPEC/FEAT/BUG → parent статус остаётся `ready`
...
requirements: [FR-001]   # из parent SPEC секций 5/6 (если есть SPEC в chain)
