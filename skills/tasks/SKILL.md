...
│  5. Номера выдаст /polisade:sync на транке                  │
└─────────────────────────────────────────────────────────────┘
```
...
#### Аддендум «БД и нагрузка» (issues #36 / #34) — дословно в промпт:

Условие: исходный документ меняет схему БД (новая таблица/колонка/тип/индекс,
правка ER-модели) **или** содержит NFR, у которого нагрузочна колонка проверки
— `Verification` в §6 ISO-спеки, `measure` в §5.2 NFR-QAS-Δ change-spec'а
(`load`, `perf`, `нагруз`, `latency`). Тогда допиши к базовому
промпту субагента блок ниже **дословно** — правило, оставленное только в теле
скилла, до генерации тасков не доезжает (урок F-O, issue #230):

```
ОТДЕЛЬНЫЕ ТАСКИ (обязательно):
- Изменение схемы БД → отдельный TASK «миграция для <что меняется>».
  ## Приёмка содержит три проверяемых пункта: (1) forward-миграция
  применяется на fixture-датасете; (2) rollback на один шаг работает;
  (3) после forward → rollback → forward данные совпадают с baseline.
  Файл миграции создаётся ТОЛЬКО средствами migration tool проекта —
  прямой DDL по живой БД не является выполнением этого TASK.
- NFR с нагрузочной верификацией → отдельный TASK «perf-верификация
  NFR-XXX», где NFR-XXX стоит в `requirements:`. ## Приёмка: тест
  запускается командой из `testing.performanceCommand`, а пороги (p99,
  error rate, throughput) заданы ЧИСЛАМИ ИЗ ФОРМУЛИРОВКИ NFR внутри
  самого теста — не в конфиге плагина.
```
...
1. **Возьми семя для TASK** — номер при создании не вычисляется:

    ```bash
    ${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_id.py new-seed
    ```

    Имя файла — `TASK-<семя>-<slug>.md`, фронтматтер — `id: TASK-<семя>` И
    `seed: <семя>` (оба обязательны). Батч — это N независимых семян (`--count N`), а не последовательность: задачи пачки могут ссылаться друг на друга через `depends_on: [TASK-<семя соседа>]` сразу. Счётчик не читается и не пишется, и
...
2. **Batch-режим.** Возьми `--count N` семян разом и раздай по задаче.
   Последовательности нет: каждое семя самостоятельно, порядок в пачке
   ничего не значит, и повторное чтение диска не нужно — читать нечего.
3. **Write-guard на каждый файл.** Перед `Write tasks/TASK-<семя>-slug.md`
   проверь, что файла нет. Проверка одна вместо трёх: сравнивать будущий
   номер с индексом больше не с чем — номера у создаваемой задачи ещё нет.
   При коллизии — новое семя (`polisade_id.py new-seed`) и повтор; если
   часть пачки уже записана, сообщи, сколько файлов создано.
4. **Счётчик не трогай.** `.state/counters.json` пишет только
...
8. **Линт тасков — дефектный таск не выпускается + предупреждения.**
   ...
   **(а) Ошибки — на каждом таске** (в цикле):
   ```bash
   ${POLISADE_PYTHON:-python3} scripts/polisade_spec_lint.py --root . --json tasks/TASK-{k}-{slug}.md
   ```
   ...
   **риге** (автономный исполнительный контур, где нет человека дописать секцию)
   ...
   ```bash
   ${POLISADE_PYTHON:-python3} scripts/polisade_spec_lint.py --root . --strict-acceptance --json tasks/TASK-{k}-{slug}.md
   ```
   ...
   **(б) Пересечение координат — на всём наборе тасков этой спеки сразу**
   ...
   ```bash
   ${POLISADE_PYTHON:-python3} scripts/polisade_spec_lint.py --root . --json tasks/TASK-*.md
   ```
