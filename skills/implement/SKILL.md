...
def setup_worktree(project_root, branch_name):
    ...
    # Журнал НЕ копируется: с 3.7.19 это закоммиченный каталог
    # `.state/session-log/`, и он уже пришёл в worktree вместе с веткой.
    # Копия корневой версии поверх ветки затёрла бы записи ветки.
...
6. НЕ создавай новые артефакты (TASK/FEAT/ADR). Если нужен новый артефакт →
   верни waiting_pm. Причина — ОБЪЁМ, а не техника: субагент исполняет
   выданную задачу, а решение завести новую работу принимает PM. (До 3.7.17
   здесь стояло «counters.json недоступен»; с семенами создать артефакт в
   worktree технически можно, и правило, объяснённое неверной причиной,
   приглашает его нарушить, как только читатель это заметит.)
...
case "$MODE" in
  true)
    ...
    # `git -C` вместо `CURRENT=$(cd … && …)`: корп-шелл режет подстановку
    # команд (issue #331), а подоболочка здесь была нужна только ради cwd.
    git -C "$WORK" rev-parse --abbrev-ref HEAD > "${POLISADE_WORK_DIR:-.}/.polisade/tmp/branch.txt"
    CURRENT=<содержимое .polisade/tmp/branch.txt>
...
9. **Приёмка (best-effort, если заведена)**. Регрессия отвечает на вопрос «не
   сломали ли соседнее», приёмка — «получил ли заказчик то, что просил»; одно
   другое не заменяет.
...
<!-- polisade:exec-denied CAPSULE END -->
   ...
   Состояние бери ОДНОЙ командой, а не собственным разбором файла (issue #343):
   `${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_acceptance.py status --json`.
   Чеки она НЕ исполняет. Её же читают `/polisade:doctor`, `/polisade:state` и
   раздел `## Приёмка` в описании PR — один расчёт, четыре читателя.

   Режим — `knowledge.json :: testing.acceptanceMode` (`off` | `advise` |
   `block`, дефолт `advise`):
   - `off` — шаг пропущен, скажи об этом одной строкой;
   - `advise` (дефолт, **сегодняшнее поведение**) — после зелёной регрессии
     предложи прогон `/polisade:acceptance run` (по красным —
     `/polisade:acceptance repair`); цикл не блокируется ничем;
   - `block` — прогони сам; красная приёмка ДЕРЖИТ TASK: статус `waiting_pm` с
     цитатой красных пар, в `review` задача не уходит. Ремонт — существующий
     контур (3 раунда, дальше честная остановка).

   ⛔ В режиме `block` скажи ГРОМКО, в том же выводе (класс F1): это
   **остановка, а не гарантия** — файл проверок лежит в репозитории, виден
   модели и доступен ей на запись; запрет держится промптом, а не барьером.
   Приёмка с гарантией — свойство платного продукта.

   Файла нет — одна строка («приёмка не заведена — `/polisade:acceptance
   author`»), цикл не блокируется ни в одном режиме: завести образ результата
...
""")
    ...
    #   приёмка (#343)        → `## Приёмка` ВСЕГДА, включая «не заведена»:
    #                            отсутствие приёмки это факт о PR, а не пустое
    #                            место. Текст — поле `summary` из
    #                            `polisade_acceptance.py status --json`, как
    #                            есть; своими словами не переписывать. Отчёт,
    #                            снятый на другом коде, помечен там же (stale)
    #                            — это утверждение о ДРУГОМ дереве, и подавать
    #                            его как состояние этого PR нельзя.
    #                            Команда НЕ отработала → `не удалось
    #                            определить` + дословный отказ. «Приёмка не
    #                            заведена» — это ОТВЕТ инструмента, а не
    #                            заполнитель пустоты (issue #356).
...
<!-- polisade:exec-denied CAPSULE END -->
    ...
    cmd = (
        ...
        f'--head "{BRANCH}" '
        f'--base "{BASE or "main"}" '
        # статический fallback ".", не $(pwd): корп-шелл режет подстановку
        # команд (issue #331) — тот же приём, что в рецептах migrate/sync.
        '--project-root "${POLISADE_WORK_DIR:-.}" '
        '--format json'
    )
    PR_JSON_RC = run(cmd)
