...
1. **Возьми семя для PRD** — номер при создании не вычисляется:

    ```bash
    ${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_id.py new-seed
    ```

    Имя файла — `PRD-<семя>-<slug>.md`, фронтматтер — `id: PRD-<семя>` И
...
2. **Write-guard.** Перед `Write` проверь, что `docs/prd/PRD-<семя>-slug.md`
   не существует. Проверять `artifactIndex` нечем: номера ещё нет. При
   коллизии — АБОРТ.
