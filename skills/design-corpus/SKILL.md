...
3. Если ключа нет или он `false` — **объясни PM**, что это экспериментальный
   opt-in режим, и предложи включить его КОМАНДОЙ (issue #339):
   `/polisade:migrate --enable=designCorpus --apply`. Правку
   `.state/PROJECT_STATE.json` руками не диктуй: её нельзя ни проверить, ни
   откатить средствами плагина, и она легко делается не в том файле.
...
| Что пишем | Команда |
|---|---|
| один файл (ADR, ARCHRUN, `changeset.yaml`) | `polisade_corpus_io.py write <путь> --run-id <run-id> --from <файл> [--expect-absent \| --expect-sha256 <hex>]` |
| staging → корпус (+backup, +DELETE) | `polisade_corpus_io.py promote --staging <dir> --run-id <run-id> --backup <dir> --delete <путь> --expect-from <json> --expect-strict --json` |
...
```
${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_corpus_io.py status --json
${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_corpus_io.py acquire --run-id <run-id> --json
```
...
/polisade:design-corpus --adopt               # перевести АРХИТЕКТУРУ ПРОЕКТА на корпус (Узел A)
```
...
Если в `$ARGUMENTS` есть `--adopt` — это НЕ инкремент по SPEC, а перевод
проекта; SPEC не требуется, иди на **Узел A**.

Если PM просит миграцию силосов старыми подкомандами (`--migrate-report` /
`--migrate`) — их больше нет: скажи это и предложи `--adopt` (issue #340).

### Узел A — перевод проекта на живой корпус (`--adopt`, issue #340)

Одна фраза PM («переведи архитектуру на корпус») — одна команда. Раньше это
была ручная последовательность из трёх разнородных шагов, два из которых PM
запускал скриптами из терминала, а о третьем — том, где начинается настоящая
работа, — надо было догадаться.

⛔ **Dry-run по умолчанию.** Ни одного байта в корпус до явного подтверждения
PM. Границы не двигаются: провенанс остаётся `INFERRED`/`GAP`, транзакции
набора нет, конфликт останавливает прогон и разрешается человеком.
...
**A1. Карта и список работы — ОДНОЙ командой.** Имя файла выбираешь ты, PM его
не называет:

```bash
mkdir -p .polisade/tmp/design-corpus
${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_migrate_silo.py \
  --all --root . --json \
  --worklist .polisade/tmp/design-corpus/adopt-worklist.json
```

ЗАМЕРЕНО: `--worklist` пишется и в dry-run — второй прогон ради него не нужен.
Путь обязан лежать ВНЕ корпуса (скрипт это проверяет сам: `E-worklist-in-corpus`).

**A2. Силосов нет — скажи это прямо и заверши.** `silos: []` в JSON:

- `architecture.corpus.mode == "living"` → проект уже на корпусе, переводить
  нечего;
- иначе → силосов нет и корпус пуст; обычный путь наполнения —
  `/polisade:design-corpus SPEC-NNN`.

⛔ Не делай вид, что что-то сделал: «перевёл» при нуле силосов — класс F1.

**A3. Покажи ПЛАН, по числам из JSON, четырьмя классами:**

| класс | что с ним будет |
|---|---|
| `carry` | переносится байт-в-байт скриптом |
| `derive` | сворачивает МОДЕЛЬ — это и есть настоящая работа |
| `unmapped` | файла нет в каталоге `/polisade:design` — решает человек |
| `index` | индекс самого пакета, не переносится |

⛔ **`carry` сегодня ПУСТ, и это результат, а не заглушка** (`CARRY = {}`): по
действующей раскладке (`references/corpus-layout.md`) ни у одного артефакта
силоса нет дома-одиночки — C4 L1 и L2 едут в пару «типизированный источник +
рендер», остальное один-ко-многим. Скажи это вслух, иначе «перенесено 0»
прочитается как сбой транспорта.

**A4. Подтверждение PM.** Нет — заверши, не записав ничего.

**A5. Транспорт** — тот же вызов с `--apply` и `--run-id <run-id>`.
`--on-conflict` НЕ трогай: дефолт `stop` означает, что конфликт (цель
существует и её байты отличаются) останавливает прогон ЦЕЛИКОМ — ни одного
байта в корпусе — и печатает вопросник. Выбор `skip`/`overwrite` делает
человек, не ты.

**A6. Свёртка `derive`** — по одному артефакту, обычным **Узлом 2** и на общих
правилах ветки best-effort: провенанс `INFERRED`/`GAP`, `code_refs`, честный
футер, §7-чеклист. Halt — существующий: `ARCHRUN` → `/polisade:unblock` →
`/polisade:design-corpus --resume=<run-id>`.
⛔ Пока файл не свёрнут, его **единственная копия — в силосе**: читать оттуда,
с громкой пометкой переходного чтения. Считать его устаревшим из-за того, что
карта назвала ему целевой дом, — потерять факт.

**A7. `architecture.corpus.mode` → `"living"`** — по существующему правилу из
раздела «## Состояние»: только когда `promote` отчитался `planned == promoted`.
Ненулевой exit примитива — не успех, и `mode` в таком прогоне не трогаем.

**A8. Честная сводка:** что уехало в корпус, что осталось в силосе, какие файлы
ждут `reviewed_by:`. И отдельной строкой: **из силоса не удалено ничего** — это
осознанный шаг человека, а не побочный эффект перевода; у силоса есть и другие
детерминированные потребители (drift-gate читает `DESIGN-*/api.md` и
`DESIGN-*/data-model.md`, issue #205).
...
1. Синтезируй **тонкий** `docs/specs/SPEC-NNN/changeset.yaml` (created/modified/
   ...
   любой другой файл инкремента, — не Write-инструментом. Под хэш-контрактом:
   новый changeset — `--expect-absent`, обновление существующего —
   `--expect-sha256` от байтов, которые ты прочитал перед синтезом.
...
3. **provenance-labeler** — каждый эмитируемый файл несёт во frontmatter
   ...
   🚨 **`review: REQUIRED` ставится на КАЖДЫЙ эмитируемый файл, всегда.**
   Скилл не имеет права его не поставить и не имеет права поставить что-то
   другое: это утверждение не о качестве вывода, а о том, что вывод **ещё не
   читал человек**. Снять отметку можно единственным способом — человек правит
   файл руками и заменяет строку на

   ```yaml
   reviewed_by: <кто>              # человек, а не команда
   reviewed_at: YYYY-MM-DD
   ```

   Перегенерация файла ставит `review: REQUIRED` заново — и это не досадная
   мелочь, а суть: прочитанным был ПРЕЖНИЙ текст, а не новый. Сколько файлов
   ждут человека, докладывает `/polisade:doctor` (проверка `corpus_review`).
...
4. **honest-footer** — в `INDEX.md` и заметной строкой в теле каждого файла:
   ...
   > **Каждый файл несёт `review: REQUIRED`, пока его не прочитал человек**;
   > `/polisade:doctor` считает непрочитанные.
5. **Без детерминированных гейтов**, но выход **обязан** проходить
   ...
   дублей id) **и** `polisade_lint_mermaid.py` (рендеримость ` ```mermaid `-блоков,
   #188). Нерешённый вопрос → **halt to PM** (ARCHRUN), как в штатном флоу.
6. **Запись — упорядоченная, пофайлово атомарная, но не транзакция на прогон.**
   ...
   2. прогони формат-линт **на staging**; красный линт или нерешённый вопрос →
      ...
      **без частичной промоции и без флипа `mode`**, корпус байт-цел.
      Формат-линт staging'а — это ДВА инструмента, оба обязательны:
      `polisade_lint_artifacts.py` (структура артефактов) и
      `polisade_lint_mermaid.py` (рендеримость диаграмм, issue #188).
      Второй ловит класс, невидимый первому: блок с `;` в подписи стрелки или
      с незакрытым `subgraph` пройдёт любую структурную проверку и упадёт
      `Parse error` уже в корпусе, откуда его достаёт только новый прогон с
      backup'ом. Дешевле не промотировать:
...
<!-- polisade:exec-denied CAPSULE END -->
      ...
      ${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_lint_mermaid.py . \
          --paths '.polisade/tmp/design-corpus/<run-id>/staging/**/*.md' --json
      ```

      exit 1 → **не промотируй**: покажи PM `файл:строка` и код (`MM-01`…),
      почини в staging и прогони заново. exit 2 → сломан вызов, а не зелёный
      результат. exit 0 → «известных граблей нет»; это не обещание рендера
      (слепые пятна — в `--help`), и в отчёте формулируй именно так;
   ...
   4. промотируй **одной командой** — она запишет файлы staging в
      ...
      **Промоция идёт под хэш-контрактом.** Между `--dry-run` (шаг 4а) и
      записью проходят backup и скан корпуса; человек или другой инструмент
      мог за это время править корпус руками — блокировка кооперативная и его
      не останавливает. Поэтому сначала собери карту ожиданий
      `.polisade/tmp/design-corpus/<run-id>/expect.json` вида
      `{"<путь-в-корпусе>": "<sha256>|absent"}`: **для каждого** пути из
      `created` — `"absent"`, для каждого из `modified`/`unchanged` и для
      каждого `--delete` — sha256 байтов, которые ты ПРОЧИТАЛ при построении
      diff'а (не размер и не дату). Карта обязана покрывать план целиком —
      это и требует `--expect-strict`.
...
<!-- polisade:exec-denied CAPSULE END -->
      ...
      ```
      ${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_corpus_io.py promote \
      ...
      Отказ `E-expect-*` — **halt, а не повод повторить**: он означает, что
      корпус правили после того, как ты построил staging. Покажи PM путь,
      ожидавшийся и фактический sha256 (они в `details`), и пересобери
      инкремент на актуальном корпусе. `--force` тут не поможет и не для
      этого. В успешном отчёте сверь `expectUnchecked: 0` — ненулевое значение
      означает, что часть плана прошла БЕЗ сверки, и говорить «корпус
      промотирован под контролем» в этом случае нельзя.
...
1. Создай артефакт `docs/architecture/runs/ARCHRUN-<семя>.md`. Семя берётся
   так же, как у любого артефакта — `${POLISADE_PYTHON:-python3}
   {plugin_root}/scripts/polisade_id.py new-seed`; номер выдаст
   `/polisade:sync --apply` на транке (протокол:
   `skills/tasks/references/compute-next-id.md`). Frontmatter:
   `id`, `seed`, `type: ARCH-RUN`, `status: waiting_pm`, `parent: SPEC-NNN`,
   `created`, тело — процитированный вопрос. Это **запись в корпус**, поэтому
   она идёт через тот же примитив
   (Write-инструментом в `docs/architecture/` не пишем):
   собери файл в staging и положи его на место командой
   `${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_corpus_io.py write docs/architecture/runs/ARCHRUN-<семя>.md --run-id <run-id> --from <staging-файл> --expect-absent`.
   `--expect-absent` обязателен: ARCHRUN — НОВЫЙ файл, и если имя уже занято,
   перезапись стёрла бы чужой журнал прогона. Отказ (`E-expect-present`) —
   возьми новое семя, а не повторяй с `--force`.
   Тем же способом кладётся `docs/specs/SPEC-NNN/changeset.yaml` (Узел 3 п.1):
   новый — `--expect-absent`, правка существующего —
   `--expect-sha256 <sha256 прочитанных байтов>`.
2. Сохрани staging; запиши резюм-детали в
   ...
   ещё не начиналась).
3. `${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_sync.py . --apply` — `ARCHRUN-NNN`
...
`${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_migrate_design.py <silo-dir> --report`
(read-only, ничего не пишет).

**Перевод силоса на корпус (V3-S3.31)** — `${POLISADE_PYTHON:-python3}
...
**Точка входа для PM — `--adopt` (Узел A, issue #340).** Она делает всю
последовательность сама: инвентаризует силосы, показывает план, забирает
worklist (имя файла выбирает скилл, не PM), после подтверждения запускает
транспорт и сворачивает класс `derive` по одному артефакту. Вручную те же
шаги остаются доступны — ниже описано, что именно они делают.
