...
1. Создай артефакт `docs/architecture/runs/ARCHRUN-NNN.md` (next-id по
   ...
   `${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_corpus_io.py write docs/architecture/runs/ARCHRUN-NNN.md --run-id <run-id> --from <staging-файл> --expect-absent`.
   `--expect-absent` обязателен: ARCHRUN — НОВЫЙ файл под свежевычисленным
   номером, и если номер уже занят, значит next-id разошёлся с диском —
   перезапись стёрла бы чужой журнал прогона. Отказ (`E-expect-present`) —
   пересчитай номер, а не повторяй с `--force`.
