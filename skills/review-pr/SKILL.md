...
- **Режим**: если слово `self` присутствует в аргументах — форсировать `self`. Иначе — спросить единый OPS-011 helper:
  ...
  ```bash
  caps=$(python3 {plugin_root}/scripts/polisade_cli_caps.py detect)
...
```bash
python3 {plugin_root}/scripts/polisade_vcs.py pr-list --head "$(git branch --show-current)" --format json --project-root "${POLISADE_WORK_DIR:-.}" | jq -r '.[0].number // empty'
```
...
PR_DIFF=$(python3 {plugin_root}/scripts/polisade_vcs.py pr-diff "${PR_NUM}" --project-root "${POLISADE_WORK_DIR:-.}")
PR_DESC=$(python3 {plugin_root}/scripts/polisade_vcs.py pr-view "${PR_NUM}" --fields title,body,files --format json --project-root "${POLISADE_WORK_DIR:-.}")
```
...
```bash
python3 {plugin_root}/scripts/polisade_vcs.py pr-comment "${PR_NUM}" --body-stdin \
...
- Субагент pre-fetch'ит diff и PR description через `polisade_vcs.py` и передаёт в промпт ревьюера. Ревьюер навигирует проект для чтения TASK, parent, AGENTS.md и исходного кода.
...
5. Push изменения через verified-helper (OPS-028 — НЕ bare `git push`):
   `python3 {plugin_root}/scripts/polisade_vcs.py git-push --branch <branch> --project-root "${POLISADE_WORK_DIR:-.}"`
