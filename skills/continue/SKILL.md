...
0. Pre-check: определить reviewer CLI через единый helper (OPS-011):
   - `caps=$(python3 {plugin_root}/scripts/polisade_cli_caps.py detect)` → JSON с `reviewer.mode`
...
0. `review` — resume-процедура (OPS-008):
   ...
   **Phase B — Auto-discover PR:**
   ...
   3. Если пуст → discover:
      `python3 {plugin_root}/scripts/polisade_vcs.py pr-list --head <expected_branch> --state OPEN --format json --project-root "${POLISADE_WORK_DIR:-.}"`
   ...
   **Phase C — Create PR** (если Phase B не нашла):
   1. `python3 {plugin_root}/scripts/polisade_vcs.py git-push --branch <expected_branch> --set-upstream --project-root "$WORK_DIR"`
      ...
      - exit=2 → push verification failed: status → **`waiting_pm`**, в
        ...
        **НЕ** `branch -D`.
   2. `python3 {plugin_root}/scripts/polisade_vcs.py pr-create --title "[TASK-XXX] ..." --body-file <PR_BODY_FILE> --head <expected_branch> --project-root "${POLISADE_WORK_DIR:-.}"`
