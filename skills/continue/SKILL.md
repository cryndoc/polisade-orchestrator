...
0. Pre-check: определить reviewer CLI через единый helper (OPS-011):
   - `caps=$(${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_cli_caps.py detect)` → JSON с `reviewer.mode`
...
while iterations < 2:
    ...
    if review.score >= 8:  # PASS
        run('${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_vcs.py pr-merge N --squash --delete-branch --project-root "${POLISADE_WORK_DIR:-.}"')
        task.status = "done"
...
<!-- polisade:push-stop CAPSULE END -->
   ...
   **Phase C — Create PR** (если Phase B не нашла):
   1. `${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_vcs.py git-push --branch <expected_branch> --set-upstream --project-root "$WORK_DIR"`
   ...
   2. `${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_vcs.py pr-create --title "[TASK-XXX] ..." --body-file <PR_BODY_FILE> --head <expected_branch> --project-root "${POLISADE_WORK_DIR:-.}"`
      Вызов идемпотентен (issue #158): если по `<expected_branch>` уже открыт
