...
- С `--no-task` — только CHORE со статусом `ready`; для последующей
  декомпозиции используй `/polisade:tasks CHORE-XXX`.
...
4. **Возьми семя для CHORE** — номер при создании не вычисляется:

    ```bash
    ${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_id.py new-seed
    ```

    Имя файла — `CHORE-<семя>-<slug>.md`, фронтматтер — `id: CHORE-<семя>` И
...
5. **Write-guard.** Перед `Write` проверь, что
   `backlog/chores/CHORE-<семя>-slug.md` не существует. Если `create_task` —
   та же проверка на `tasks/TASK-<семя>-slug.md`. Проверять `artifactIndex`
   нечем: номера ещё нет. При коллизии — новое семя.
