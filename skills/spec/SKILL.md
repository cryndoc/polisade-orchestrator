...
1. **Вычисли next-id для SPEC** по протоколу из
   ...
   с рекомендацией `python3 {plugin_root}/scripts/polisade_sync.py . --apply --yes`.
2. **Write-guard.** Перед сохранением SPEC-файла, сгенерированного
   субагентом, проверь, что glob `docs/specs/SPEC-{N}-*.md` не даёт ни одного
   попадания (имя несёт переменный сегмент `story_key` — точное имя заранее
   не известно, сравнивать нужно по ID-префиксу)
...
4. **Детерминированный линт в цикле — спека не выпускается красной.** После
   ...
   **(a) Детерминированная нормализация ПЕРЕД линтом (не полагайся только на
   ...
   ```bash
   python3 scripts/polisade_spec_lint.py --root . --normalize-line --json <ФАКТИЧЕСКИЙ путь файла, который вернул субагент>
   ```
