...
argument-hint: "[PRD-XXX | FEAT-XXX] [--story=<KEY>]"
cli_requires: "task_tool"
---

# /polisade:spec [PRD-XXX | FEAT-XXX] — Техническая спецификация через субагент
...
```
/polisade:spec PRD-001    # Спека для крупной инициативы
/polisade:spec FEAT-001   # Спека для фичи (если нужна архитектура)
/polisade:spec            # Выбрать из доступных ready PRD/FEAT

/polisade:spec PRD-001 --story=ABC-1234   # → docs/specs/SPEC-NNN__ABC-1234__<slug>.md
```
...
**Не нужна SPEC (иди сразу в /polisade:tasks):**
...
│  3. Номер выдаст /polisade:sync на транке                   │
└─────────────────────────────────────────────────────────────┘
```
...
⚙️ **Выбор режима (первым делом).** Прочитай
`.state/PROJECT_STATE.json → settings.experimental.changeSpec`. Если `true` —
работаешь в **Change-Spec mode** (см. секцию «Change-Spec mode» ниже: другой
шаблон, обязательная §3-локализация из графа, линт в цикле). Если `false`/нет —
обычный ISO-29148 флоу ниже. Всё остальное (валидация, state) — общее.

⚙️ **Ключ внешней задачи (`--story`).** Распарсь `$ARGUMENTS`: позиционный
аргумент — `PRD-XXX`/`FEAT-XXX`; флаг `--story=<KEY>` (форма `--story <KEY>`
тоже принимается) — ключ задачи во внешнем трекере. Он попадает в **имя файла**
SPEC (полем `__<KEY>__`) и во frontmatter, чтобы внешние проверки могли
связать спеку с задачей трекера по пути файла в PR.

1. Нормализуй: отсутствие флага/поля, пустая строка, `null`, `~`, `none` —
   это ОТСУТСТВИЕ ключа, а не значение. Сравнение регистронезависимое.
2. Валидируй формат: `^[A-Za-z][A-Za-z0-9]*-\d+$`. Не подходит — АБОРТ с
   показом ожидаемого формата (`ABC-1234`), спеку не генерируй.
3. ⛔ Ключом внешнего трекера НЕ может быть собственный ID Polisade. Сравни
   префикс до дефиса, **регистронезависимо**, с полным списком типов
   (`KNOWN_TYPES` в `scripts/polisade_sync.py`):
   `PRD, SPEC, PLAN, TASK, FEAT, BUG, DEBT, ADR, CHORE, SPIKE, DESIGN, ARCHRUN`.
   Совпадение — АБОРТ. Причина: внешний экстрактор видит `ABC-1234` и
   `SPIKE-5` одинаково, отличить их регулярным выражением нельзя в принципе —
   значит, отказ обязан стоять здесь, на производителе имени.
4. Если флаг не передан — прочитай `story:` из frontmatter родительского
   PRD/FEAT и унаследуй значение (те же п. 1–3).
5. Если ключа нет ни там, ни там — `story_key = null`. Поведение полностью
   как раньше: сегмента `story-…` в имени файла нет.

`story_key` передаётся субагенту (шаг 3) и участвует в write-guard (шаг 5).
...
4. Если не указан:
   ...
   - Если нет ready → предложи `/polisade:prd` или `/polisade:feature`

```
Нет готовых PRD или Feature Brief для создания спецификации.

Доступные действия:
   → /polisade:feature для создания фичи
...
1. Создай файл: docs/specs/SPEC-{ID}__{story_key}__{slug}.md
   - ID — `SPEC-<семя>` (`polisade_id.py new-seed`); номер выдаст sync на транке
   - story_key = `{story_key или null}` — ключ задачи во внешнем трекере.
     Если null — поле `__{story_key}__` опускается ЦЕЛИКОМ, и имя обычное:
     `docs/specs/SPEC-{ID}-{slug}.md`.
   - ⛔ Ограничители — ДВА подчёркивания с каждой стороны, ровно как здесь.
     Они не косметика: slug всегда kebab-case (`[a-z0-9]` и одиночные дефисы),
     поэтому `__` в нём не встречается НИКОГДА — и только это делает две формы
     имени различимыми механически. Одиночный дефис вместо `__` уже неотличим
     от slug: заголовок «Postgres 15 upgrade» дал бы
     `SPEC-001-postgres-15-upgrade.md`, из которого внешняя проверка извлечёт
     несуществующую задачу `postgres-15`.
   - Регистр ключа не меняй — как передан.
   - slug — строчный kebab-case из названия, без символов `_`.
...
3. Обязательно заполни frontmatter:
   ...
   - story: {story_key или null}   # ключ задачи во внешнем трекере
   - children: []
   - requirements_count:
   ...
   - design_waiver: {existing value или false}
                             # true = PM разрешил пропуск /polisade:design.
   ...
   (re-run `/polisade:spec`) ВСЕГДА сохраняй текущее значение из исходного файла.
...
- Файл: <ТОЧНЫЙ путь созданного файла, скопируй как есть — не собирай по шаблону>
...
1. **Возьми семя для SPEC** — номер при создании не вычисляется:

    ```bash
    ${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_id.py new-seed
    ```

    Имя файла — `SPEC-<семя>-<slug>.md`, фронтматтер — `id: SPEC-<семя>` И
    `seed: <семя>` (оба обязательны). Счётчик не читается и не пишется, и
...
2. **Write-guard.** Перед сохранением SPEC-файла, сгенерированного
   субагентом, проверь, что glob `docs/specs/SPEC-<семя>[-_]*.md` не даёт ни
   одного попадания (имя несёт переменный сегмент `story_key` — точное имя
   заранее не известно, сравнивать нужно по ID-префиксу; разделитель после
   семени — `-` в обычном имени и `__` в имени с ключом трекера, поэтому класс
   `[-_]`, а не дефис). Проверять `artifactIndex` нечем: номера ещё нет. При
   коллизии — новое семя (субагент уже потратил контекст, поэтому лучше взять
   свежее семя, чем перезаписывать чужой файл).
3. **Счётчик не трогай.** `.state/counters.json` пишет только
...
- Логируй score в `.state/session-log/{today}-{seed}.md`
...
`seed:` создаваемой SPEC. Файла нет — создай, есть — допиши:
```markdown
### Quality Review: SPEC-{ID} (from {PARENT-ID})
- Date: {today}
- Iteration 1: {score}/10 → {PASS|IMPROVE}
- Iteration 2: {score}/10 → {PASS|IMPROVE} (если была)
- Command: /polisade:spec
```

## Change-Spec mode (experimental — Pipeline V2 Ф2, issue #211)

⚙️ **Гейт совместимости.** Этот режим включается ТОЛЬКО когда
`settings.experimental.changeSpec == true` в `.state/PROJECT_STATE.json`
(дефолт — `false`). Если флаг не выставлен — работает обычный ISO-29148 флоу
выше, ничего не меняется. Так новые проекты Pipeline V2 получают code-first
change-spec, а существующие `/polisade:*`-проекты не ломаются.

Change-spec — это **дельта, написанная глядя в код** (ADR-0002): ровно 6 секций,
центральная — §3 «Локализация из графа». Отличается от полной спеки тем, что
описывает **изменение**, а не систему целиком, и каждое затронутое требование
привязано к точным координатам кода (файл/символ) с provenance.

### Когда флаг включён — что меняется в алгоритме

1. **Шаблон.** Субагент использует `docs/templates/change-spec-template.md`
   (6 секций), НЕ `spec-template.md`. Frontmatter несёт `kind: change-spec`.

2. **§3 «Локализация из графа» заполняется детерминированным протоколом — ОБЯЗАТЕЛЬНО.** <!-- polisade:nav-canon POINTER: канон навигации клиента — grep-капсула LOCALIZE в skills/implement/SKILL.md §1.8 (дом канона); семантика §3 совпадает с каноном, формат строк — спека-специфичный (WPF); check_nav_canon_parity сторожит указатель. MCP-нав-протокол платного движка вырезан в V3-P2 (ADR-0004). -->
   Тот же детерминированный grep-протокол, что шаг LOCALIZE в
   `/polisade:implement`:
   - выпиши ключевые термины из каждого FR (имена сущностей, endpoint'ы, поля);
   - `grep -rn "<термин>"` → файлы-кандидаты и точные символы;
   - `grep -rn "<symbol>"` для топ-символов → точки интеграции;
   - при правке контракта — пройди по ВСЕМ попаданиям символа и перечисли
     задетые файлы (что сломается).
   **В колонке `provenance` таблицы §3 клиент пишет `grep-fallback`** — это
   значение закрытого словаря формата (см. ниже), честно называющее источник
   координаты. Остальные значения словаря — провенанс инструментов графа кода;
   словарь принадлежит общему формату (он один на клиент и исполнительный
   контур) и НЕ сужается.
   Каждая строка §3 несёт `provenance` — чем получена координата. Подменять
   `grep-fallback` на графовый провенанс, не выполняя обход графа, ЗАПРЕЩЕНО
   (класс F1: отсутствие способности как положительный факт).
   ⛔ **Спека без непустой §3 не выпускается** (линт красный, см. п.4).

   **⛔ Формат каждой строки §3 — буквальный, downstream её парсит машинно
   (узел `tasks` Orchestrator + линт-гейт исполнительного контура). Форсируй эти правила в промпте
   субагента ЯВНО, с негативными примерами — модель по привычке их нарушает
   (WP2.6, диагноз D1/D2/D4/D5):**

   - **`file` — литеральный путь от корня проекта, ЦЕЛИКОМ. БЕЗ сокращений
     `.../` и БЕЗ номеров строк `:65-67`** (координата = файл + символ; диапазон
     строк лишний и рушит exact-match).
     - ❌ Плохо: `src/main/java/.../orders/OrderTotalCalculator.java:155-164`
     - ✅ Хорошо: `src/main/java/com/example/orders/OrderTotalCalculator.java`
   - **`symbol` — сигнатура уровня метода `Class.method`** (из результата
     grep-локализации), не только имя класса.
     - ❌ Плохо: `Order`
     - ✅ Хорошо: `OrderTotalCalculator.calculate`
   - **`provenance` — несколько источников ЧЕРЕЗ ЗАПЯТУЮ**, не через ` + `.
     - ❌ Плохо: `search_symbol + find_references`
     - ✅ Хорошо: `search_symbol, find_references`
     - Словарь: `search_symbol | find_references | blast_radius | co_changed | file_outline | grep-fallback`.
   - **Заголовки FR/NFR в §2** — `### FR-NNN — …` **или** `#### FR-NNN — …`
     (сгруппированные под `### Functional Requirements` и выпущенные как H4
     допустимы — линт принимает H3 и H4).
   - **⛔ НОВЫЙ файл — объяви во frontmatter `creates_files:` (#234)**: полный
     путь в §3 **и** он же в `creates_files:`. Иначе `E-loc-file-missing`, и
     улучшатель не починит — файла физически нет (пустая §3 и `path/to/...` тоже
     красные). Это декларация замысла: линт таска сверит, что файл создан.
     - ❌ строка §3 на несуществующий `src/auth/audit.py` без декларации
     - ✅ она же + `creates_files: [src/auth/audit.py]`
     - ⚠️ только реально новые: существующий файл здесь снимет проверку,
       которая ловит опечатку в пути

3. **Стабильные FR/NFR-ID (P0-3).** `FR-NNN` / `NFR-NNN`, 3 цифры, сквозная
   нумерация; ID неизменны сквозь `SPEC → TASK → PR`; удалённые номера не
   переиспользуются. Cross-doc ссылки — composite `{SPEC_ID}.FR-NNN`
   (см. «Requirement ID Scoping» в CLAUDE.md).

4. **Детерминированный линт в цикле — спека не выпускается красной.** После
   того как субагент сохранил change-spec, основной агент:

   **(a) Детерминированная нормализация ПЕРЕД линтом (не полагайся только на
   модель).** Сначала механически срежь косметический `:line`-хвост из колонки
   `file` §3 — этот флаг переписывает файл на месте и затем линтует. ⛔ Путь
   бери ФАКТИЧЕСКИЙ (из ответа субагента / `state.artifactIndex`), не
   реконструируй его по шаблону: сегмент `story-…` опционален, и
   собранное имя разойдётся с реальным на любой спеке без ключа:

   ```bash
   ${POLISADE_PYTHON:-python3} scripts/polisade_spec_lint.py --root . --normalize-line --json <ФАКТИЧЕСКИЙ путь файла, который вернул субагент>
   ```

   `:65-67`/`:155-164` — чистая косметика (D2), безопасно снимается детерминированно,
   а не тратит итерацию субагента.

   **(b) Разбор результата:**
   - `exit 0` → спека валидна, продолжай к Quality Review Loop.
     ⚠️ На клиенте каждая строка §3 с `provenance = grep-fallback` даёт
     advisory-warning `W-loc-grep-fallback` (текст линта про предпочтительность
     графовых вызовов) — это **ожидаемый шум общего формат-линта, НЕ дефект
     спеки и НЕ повод что-то менять**: grep — штатный механизм клиента
     (V3-P2), warning вердикт не меняет (exit 0). Не «чини» его подменой
     провенанса — это класс F1.
   - `exit 1` → есть ошибки (пустая §3, дублирующийся/кривой FR-ID,
     несуществующий файл в §3, **`E-loc-path-ellipsis` — путь с `.../`**,
     неизвестный provenance). Передай JSON-отчёт обратно субагенту-улучшателю
     (тот же improvement-субагент, что в Quality Review Loop) **явным списком
     «исправь ИМЕННО это»** по каждому finding — например: `E-loc-path-ellipsis`
     → «разверни путь целиком, без `.../`»; `E-loc-provenance` на `+` → «замени
     ` + ` на запятую»; символ уровня класса → «уточни до `Class.method`».
     Он чинит ТОЛЬКО отмеченное. Повтори с шага (a). **Максимум 3 итерации**
     (WP2.6: цикла в 2 не хватило — improvement-субагент повторно вносил тот же
     формат); если после 3-й линт всё ещё красный → статус `waiting_pm` с
     процитированными findings (не выпускай красную спеку).
   - `exit 2` → ошибка использования/парсинга: почини вызов, не спеку.

   Линт-скрипт доставляется в проект `/polisade:init`'ом (`scripts/`), поэтому
   вызывается по проектному пути, а не из install-dir (под GigaCode Guard
   install-dir read-protected — issue #119/#127).

5. **Provenance-инструмент во frontmatter.** Проставь
   `localization_tool: grep-fallback` — честное значение для клиента: §3
   заполнена grep-протоколом. (Значение `mcp` принадлежит формату — его
   ставит исполнительный контур с живым графом кода; клиент его не ставит.)

Всё остальное (валидация ready-родителя, техконтекст-чекпойнт, Quality Review
Loop, обновление state/counters) — без изменений.
...
ID: SPEC-001
Название: [Название]
Файл: {фактический путь созданного файла}
...
═══════════════════════════════════════════
СЛЕДУЮЩИЙ ШАГ:
   → /polisade:design SPEC-001 — создать doc-as-code пакет (C4/ERD/OpenAPI/ADR), опционально
   → /polisade:tasks SPEC-001 — создать задачи
   → /polisade:roadmap SPEC-001 — если нужен детальный план с фазами
...
ID: SPEC-001
Файл: {фактический путь созданного файла} (draft)
На основе: FEAT-001
...
- Не создавай спеку если она не нужна — для простых фич иди сразу в `/polisade:tasks`
