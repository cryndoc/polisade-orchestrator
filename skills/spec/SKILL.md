...
2. **Write-guard.** Перед сохранением SPEC-файла, сгенерированного
   субагентом, проверь, что glob `docs/specs/SPEC-{N}[-_]*.md` не даёт ни одного
   попадания (имя несёт переменный сегмент `story_key` — точное имя заранее
   не известно, сравнивать нужно по ID-префиксу; разделитель после номера —
   `-` в обычном имени и `__` в имени с ключом трекера, поэтому класс `[-_]`,
   а не дефис. Класс обязателен и с другой стороны: голый `SPEC-{N}*.md`
   поймал бы `SPEC-1` на `SPEC-10`)
