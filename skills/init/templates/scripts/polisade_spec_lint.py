...
def _strip_inline_comment(val):
    """Drop a YAML inline `# …` comment, leaving quoted values untouched so a
    `title: "Fix #123"` keeps its hash.

    Two shapes carry a comment, and the second one used to be missed (B-301):

      key: value   # note      → the comment FOLLOWS a value (space before `#`)
      key:         # note      → the value IS the comment; the real value is the
        - real/value             block list below

    The second shape is what the shipped `docs/change-spec-template.md` itself
    teaches for `creates_files:`. The old probe searched `" #"` AFTER
    `.strip()` — and stripping deletes the very space it looks for, so the whole
    comment came back as the value: `parse_string_list_block` handed it to
    `parse_inline_list`, which split it on commas into garbage entries and never
    read the block below. In a live pilot that turned a correctly declared new
    file into an unfixable `E-loc-file-missing`.

    So: a value that is nothing but a comment is EMPTY (callers then fall
    through to the block form), and the `" #"` probe runs on the RAW value —
    before stripping — so `key:   # note` and `key: v # note` are one rule, not
    two. A hash with no whitespace in front of it (`a#b`, `http://x/y#z`) is
    still part of the value, exactly as in YAML."""
    stripped = val.strip()
    if stripped[:1] in ("'", '"'):
        return stripped
