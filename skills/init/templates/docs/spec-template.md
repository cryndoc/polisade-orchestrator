...
story: null      # ключ задачи во внешнем трекере (ABC-1234) или null. Заполняется из /polisade:spec --story=<KEY>; тот же ключ идёт в имя файла: SPEC-NNN__<KEY>__<slug>.md
...
design_waiver: false   # true = PM явно разрешил пропуск /polisade:design несмотря на архитектурные триггеры. ⛔ НЕ влияет на DESIGN CONFORMANCE и drift-gate (issue #205): waiver дрейфа — только артефакт docs/waivers/DRIFT-WAIVER-NNN.md
---
...
Заполняется агентом /polisade:spec или вручную автором.
Принципы:
...
FR — lint блокирует bare ссылку как ambiguous; /polisade:migrate --apply
...
<!--
Каждый вопрос имеет ID, владельца и срок. /polisade:spec может ставить
