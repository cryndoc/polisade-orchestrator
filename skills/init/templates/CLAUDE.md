...
├── adr/             # ADR-001-name.md
├── architecture/    # DESIGN-001-name/ (design packages from /polisade:design)
...
2. `/polisade:migrate --apply` создаст `.env.example` (reference) и `.env` (stub). `.env` добавится в `.gitignore` (некомментированной строкой).
