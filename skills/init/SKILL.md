...
4.1. Install the pre-push guard (issue #159).
   ...
   a) Resolve the destination — do **NOT** hard-code `.git/hooks`. A project
      ...
      Подставь напечатанный путь в шаги ниже вместо `$HOOKS_DIR`. Именно
      две команды, а не `HOOKS_DIR="$(…)"`: корп-шелл (GigaCode CLI) режет
      подстановку команд, и присваивание не выполнилось бы (issue #331).
...
5. Update `.gitignore` — append Polisade Orchestrator-specific entries if not already present:
   ...
   !.state/counters.json
   # База приёмки (#299): хеши ратифицированных пар И ИХ ПРИБОРОВ —
   # тест-файлов, которые запускает чек. Ни то, ни другое не выводится из
   # текущего дерева: база и есть то, С ЧЕМ сравнивают. Пока она локальна,
   # ослабленный чек видит только тот, кто последним прогонял приёмку у себя.
   !.state/acceptance-baseline.json
   # Журнал сессий (#299) — единственная долговечная запись о происходившем,
   # кроме истории git и комментариев в PR. Каталог, файл на артефакт-сессию:
   # ОДИН растущий файл конфликтовал в каждой ветке, которая в него писала.
   !.state/session-log/
   # Возврат КАТАЛОГА открывает всё внутри — вложенные каталоги и не-.md тоже
   # (замерено `git status`), поэтому две строки ниже сужают до формы записи.
   .state/session-log/**
   ...
   # Bytecode of the vendored runtime scripts (#297) — writing it is a
   # side effect of RUNNING them, and an untracked entry stops the post-apply
   ...
   Append the block **in this order** and never reorder it: git keeps the LAST
   matching rule, so a `!.state/…` line that ends up above `.state/*` negates
   nothing (issue #329). This block and `skills/init/templates/gitignore` are
   one fact — `bash scripts/regression_tests.sh --ops=initignore` pins them
   equal and runs `git status` on both.
