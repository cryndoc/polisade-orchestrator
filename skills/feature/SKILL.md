...
1. **Возьми семя для FEAT** — номер при создании не вычисляется:

    ```bash
    ${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_id.py new-seed
    ```

    Имя файла — `FEAT-<семя>-<slug>.md`, фронтматтер — `id: FEAT-<семя>` И
...
2. **Write-guard.** Перед `Write` проверь, что
   `backlog/features/FEAT-<семя>-slug.md` не существует. Проверять
   `artifactIndex` нечем: номера ещё нет. При коллизии — новое семя.
3. Создай файл `backlog/features/FEAT-<семя>-slug.md`
