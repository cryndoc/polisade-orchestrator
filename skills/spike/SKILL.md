...
1. **Возьми семя для SPIKE** — номер при создании не вычисляется:

    ```bash
    ${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_id.py new-seed
    ```

    Имя файла — `SPIKE-<семя>-<slug>.md`, фронтматтер — `id: SPIKE-<семя>` И
...
2. **Write-guard.** Перед `Write` проверь, что
   `backlog/spikes/SPIKE-<семя>-slug.md` не существует. Проверять
   `artifactIndex` нечем: номера ещё нет. При коллизии — новое семя.
3. Создай файл `backlog/spikes/SPIKE-<семя>-slug.md`
4. Спроси timebox:
   - "Сколько времени выделить на исследование? (по умолчанию 4h)"
5. **Счётчик не трогай.** `.state/counters.json` пишет только
