...
* `assigned` → `TASK-<семя>` стал `TASK-007`: файл переименован, `id:`
  переписан, `seed:` остался, машинные ссылки (`parent:` и соседи) поехали за
  номером. **Проза не тронута** — ссылку, сделанную до нумерации, читает
...
* `"status": "seed_problems"` (rc≠0) → семя не может разрешить ссылку: его нет, оно
  ...
  как ссылка станет разрешимой, нельзя.
