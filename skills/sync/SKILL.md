...
3. Распарсить JSON-ответ, показать diff пользователю.
   ...
   ⛔ **Числа берутся из JSON, а не пересчитываются** (issue #354) — то же
   правило, что в `/polisade:migrate` шаг 4, и по той же причине: замерено, как
   модель свернула список в придуманные группы и назвала PM неверные числа, по
   которым он и решает. Количество = длина соответствующего массива ответа;
   список показывай как есть. Никаких чисел, которых нет в ответе скрипта.
...
#    игноры; без -uall новый каталог схлопывается в ОДНУ запись, issue #341)
...
#    git mv уже застейджил перенос целиком (#318). Иначе СТОП и вопрос PM.
