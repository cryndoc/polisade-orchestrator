...
# /polisade:roadmap [SPEC-XXX] — План реализации через субагент
...
```
/polisade:roadmap SPEC-001   # План для конкретной спеки
/polisade:roadmap            # Выбрать из доступных ready SPEC
```
...
**Не нужен PLAN (иди сразу в /polisade:tasks):**
...
│  2. Номер выдаст /polisade:sync на транке                   │
...
4. Если не указан:
   ...
   - Если нет ready SPEC → сообщи и предложи /polisade:spec
...
1. **Определи source SPEC:**
   - Аргумент `/polisade:roadmap` — всегда SPEC-NNN → `spec_id` = аргумент
...
6. **Дождись ответа PM:**
   - PM выбирает `/polisade:design` → прервать создание плана, PM запускает design
   - PM выбирает waiver:
     a. Добавь `design_waiver: true` в SPEC frontmatter (persistent marker)
     b. Последующий `/polisade:tasks PLAN-NNN` увидит waiver на SPEC → не переспросит
...
1. Создай файл: docs/plans/PLAN-{ID}-{slug}.md
   - ID — `PLAN-<семя>` (`polisade_id.py new-seed`); номер выдаст sync на транке
...
3. Roadmap Items формат:
   Каждый item должен быть достаточно детальным для /polisade:tasks
...
1. **Возьми семя для PLAN** — номер при создании не вычисляется:

    ```bash
    ${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_id.py new-seed
    ```

    Имя файла — `PLAN-<семя>-<slug>.md`, фронтматтер — `id: PLAN-<семя>` И
...
2. **Write-guard.** Перед сохранением PLAN-файла, сгенерированного
   субагентом, проверь, что `docs/plans/PLAN-<семя>-slug.md` не существует.
   Проверять `artifactIndex` нечем: номера ещё нет. При коллизии — новое семя.
...
4. Обнови `.state/PROJECT_STATE.json`:
   ...
   - **НЕ меняй SPEC.status на `done`!** SPEC — living document по ISO/IEC/IEEE 29148:
     ...
     - Правило consistent с `/polisade:spec` и `/polisade:design` Phase 7: parent верхнеуровневый артефакт не закрывается из-за создания child
...
═══════════════════════════════════════════
СЛЕДУЮЩИЙ ШАГ:
   → /polisade:tasks PLAN-001 — создать задачи из items
...
- Каждый item декомпозируется в 2-5 TASK командой `/polisade:tasks`
