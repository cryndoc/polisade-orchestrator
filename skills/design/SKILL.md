...
1. **Возьми семя для DESIGN и ADR** — номер при создании не вычисляется:

    ```bash
    ${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_id.py new-seed
    ```

    Имя файла — `DESIGN-<семя>-<slug>.md`, фронтматтер — `id: DESIGN-<семя>` И
    `seed: <семя>` (оба обязательны). Каждому — своё семя; имя директории пакета DESIGN остаётся авторитетом для его id. Счётчик не читается и не пишется, и
...
2. **Write-guard.** Перед созданием директории пакета
   `docs/architecture/DESIGN-<семя>-slug/` проверь, что директория не
   существует. Для каждого ADR в наборе — аналогично для
   `docs/architecture/decisions/ADR-<семя>-slug.md`. Проверять `artifactIndex`
   нечем: номера ещё нет. При коллизии — новое семя, до любого IO.
   ...
   > защищает файл.
3. Имя пакета — `DESIGN-<семя>-slug`; номер выдаст `/polisade:sync` на транке
4. Если ADR в наборе: каждому — своё семя, `ADR-<семя>-slug.md`
...
2. ПОРЯДОК ГЕНЕРАЦИИ:
   ...
   i. ADRs — отдельные файлы, но пиши их в STAGING
      `.polisade/tmp/design/DESIGN-<семя>/decisions/ADR-<семя>-{slug}.md`, НЕ в
...
`seed:` создаваемого DESIGN-пакета (файл на артефакт-сессию; каталог
коммитится, конфликтов нет по построению, а имя не меняется, когда пакет
получит номер). Файла нет — создай, есть — допиши:
...
${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_corpus_io.py write \
    docs/architecture/decisions/ADR-<семя>-{slug}.md \
    --from .polisade/tmp/design/DESIGN-<семя>/decisions/ADR-<семя>-{slug}.md \
...
1. **counters.json**: не трогается — счётчик пишет `/polisade:sync --apply` на транке, там же, где выдаются номера DESIGN и ADR
...
5. **`architecture.activeADRs`** — append каждый созданный `ADR-<семя>.id` (это поле сейчас dead, оживляется новым скиллом)
...
8. **Federation glossary в knowledge.json — только если `glossary.md` создан в этом package:**
   ...
   d. Для каждого извлечённого термина:
      ...
      - **Найден И definition отличается** → CONFLICT:
        ...
        - Добавь warning в `.state/session-log/{today}-{seed}.md` (тот же файл,
          что и в Phase 7 — журнал у сессии один):
          ```markdown
   ...
   f. Логирование в `.state/session-log/{today}-{seed}.md` (тот же файл):
      ```markdown
...
- Номера DESIGN и ADR выдаёт `/polisade:sync --apply` на транке; здесь у обоих семя
