...
## Частые ошибки рендера

Эти ошибки ломают рендер **целиком**: GitHub, GitLab и VS Code показывают
`Parse error` вместо диаграммы, а в исходнике блок выглядит правдоподобно.
Детерминированный детектор — `scripts/polisade_lint_mermaid.py` (коды `MM-01`…);
он не парсер Mermaid, поэтому проще не порождать ошибку, чем ловить её.

| Не так | Так | Почему |
|---|---|---|
| `;` в подписи или метке | `,` / `—` / `#semi;` | `;` — разделитель операторов Mermaid: он рвёт строку, и блок перестаёт рендериться (инцидент 2026-06-22: 5 блоков в 4 файлах) |
| перенос строки внутри метки | `<br/>` | у метки нет многострочной формы |
| открытый блок без `end` / `{` без `}` | закрывай каждый | несбалансированный блок роняет разбор с места открытия |
| незакрытая `"` | парная `"` | всё до конца блока разбирается как строка |
| опечатка в заголовке (`sequencediagram`) | точное написание | неизвестный заголовок = нет диаграммы |

Если `;` нужен как символ — пиши HTML-сущность `#semi;`, либо заключи всю
метку в двойные кавычки: внутри `"…"` он безопасен.

Специфика C4: все подписи в `Person(...)` / `System(...)` / `Container(...)` /
`Rel(...)` заключены в двойные кавычки, поэтому `;` внутри них безопасен — но
кавычка обязана быть парной, иначе весь блок уезжает в строку.
`Container_Boundary(...) {` / `Deployment_Node(...) {` закрываются `}`.
Заголовок — точное написание `C4Context` / `C4Container` / `C4Component` /
`C4Deployment` (регистр значим).
