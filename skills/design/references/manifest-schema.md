...
- traceability tools (`/polisade:doctor`, `/polisade:review-pr`), которые проверяют покрытие FR/NFR
...
# === ADRs (живут отдельно в docs/architecture/decisions/) ===
adrs:
  - id: ADR-003                         # required
    title: "Mermaid over PlantUML for doc-as-code"  # required
    file: ../decisions/ADR-003-mermaid-over-plantuml.md  # required, путь от package dir
  ...
  - id: ADR-004
    ...
    file: ../decisions/ADR-004-sessions-in-redis.md
    status: proposed
    addresses: [NFR-001]
...
живут в `docs/architecture/decisions/`, а не внутри package dir.
...
- **`/polisade:review-pr`** — проверяет покрытие FR/NFR из source SPEC
- **`/polisade:doctor`** — строит traceability matrix
- **`/polisade:roadmap`** review — проверяет, что каждый component из manifest
  упомянут хотя бы в одном PLAN item
- Внешние AI-агенты (Codex, Qwen extension) — не должны парсить README.md tables

## Validation

`polisade_lint_skills.py` (в будущем) валидирует:
...
Поддерживаются `/polisade:doctor --architecture` для domain resolution.
