...
В `/polisade:design` мы храним OpenAPI как **YAML внутри fenced ```yaml блока** в файле `api.md`. Это даёт:
...
Если проекту нужен standalone `openapi.yaml` (для codegen) — это создаётся отдельно, не в `/polisade:design`.
...
## Сверка с эталоном: инструмент и анти-паттерны

Если PM передал **внешний эталон** контракта — папку с JSON-схемами, чужую
OpenAPI/AsyncAPI-спеку, выгрузку из системы-смежника — сверять её с артефактом
глазами нельзя. Класс ошибки задокументирован (issue #164): модель «перечитала
и вроде совпадает», а на деле разошлись регистр имени, `format` против
`pattern` и обязательность поля, причём **по-разному в разных вариантах**.

Сверка делается механически:
...
${POLISADE_PYTHON:-python3} {plugin_root}/scripts/polisade_reference_fields.py \
    diff <файл-или-каталог-эталона> <файл-артефакта>
```

Скрипт строит из обоих файлов плоскую таблицу `вариант · поле · тип ·
format/pattern · required · пример` и сравнивает её построчно. Регистр значим
(`parentRequestId` ≠ `parentrequestId`), порядок полей — нет. Exit: 0 чисто,
1 расхождения, 2 не разобрано (границы разбора — `--rules`).

Анти-паттерны, из-за которых сверка и понадобилась:

1. ⛔ **Объявить расхождение эталона «опечаткой»** и молча привести артефакт к
   удобному варианту. Эталон дал пользователь; править его толкование — не
   работа генератора.
2. ⛔ **Схлопнуть вариативность**: выбрать одно имя/формат «основным» и
   применить ко всем вариантам. Именно это и потеряло `parentrequestId` в
   `RESPONSE`.
3. ⛔ **Подать неразбор как согласие**: скрипт вышел с кодом 2 — сверки НЕ
   БЫЛО. «Расхождений не найдено» в этом случае — ложь.
4. ⛔ **Сверить «в целом»** и не привести таблицу. Расхождение без координаты
   `вариант · поле` человеком не проверяется.

Спорное расхождение фиксируется как **Decision needed** и уходит PM (#88), а
не чинится по догадке.
