...
[← README](./README.md) · [[SPEC-XXX]] §6 Нефункциональные требования
<!-- Ссылка на источник — по ID ([[SPEC-XXX]]), а НЕ относительным путём: имя
     файла спеки несёт опциональное поле ключа внешнего трекера
     (SPEC-NNN__KEY__slug.md), и собранный по шаблону путь не разрешится. -->
```
...
❌ **Не выдумывай NFR, которых нет в SPEC**. Если SPEC не упомянул security — не пиши security scenarios. Поправь SPEC через `/polisade:spec` improve, потом перезапусти design.
