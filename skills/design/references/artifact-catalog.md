...
Компактный каталог 11 типов артефактов, которые может создавать `/polisade:design`. Полные триггеры — в `conditional-triggers.md`. Полные шаблоны — в соответствующих guide-файлах.

> **⚠️ DESIGN-NNN силос deprecated для brownfield (Pipeline V2 Ф4, WP4.3 / #221).**
> На существующем корпусе intent-подмножество (adr, quality_scenarios, glossary,
> context-map/c4_context L1, deployment) переезжает в **живой корпус**
> `docs/architecture/` за детерминированными гейтами; derived (c4_container L2,
> c4_component L3, erd, state, sequence) — **регенерируется, руками не ведётся**.
> Разрез — `docs/pipeline-v2/intent-derived-split.md`. Greenfield (новый пакет с
> нуля) остаётся живым. Миграция силоса — `scripts/polisade_migrate_design.py`.
...
| `adr` | Architecture Decision Records | `docs/architecture/decisions/ADR-{N}-*.md` | нет | `adr-guide.md` |
