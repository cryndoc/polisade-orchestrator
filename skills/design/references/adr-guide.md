...
В Polisade Orchestrator есть готовый шаблон `docs/templates/adr-template.md` (Context and Problem Statement / Decision Drivers / Considered Options / Decision Outcome / Pros and Cons / Validation / More Information / Related Decisions). `/polisade:design` использует этот шаблон без изменений.
...
- AI-агент `/polisade:design` успешно генерирует C4Container диаграммы из SPEC текста
...
- File path: `docs/architecture/decisions/ADR-{N}-{slug}.md` (в `docs/architecture/decisions/`, а не внутри package dir)
