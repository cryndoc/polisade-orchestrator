...
Специфика sequence: `;` чаще всего заезжает в текст сообщения после `:`
(`A->>B: сохранил; отправил`) и в метки `loop` / `alt` / `else` / `opt` / `par` /
`critical` / `note` — это ровно те места, где он встретился в инциденте.
Каждый `loop`/`alt`/`opt`/`par`/`critical`/`rect`/`break`/`box` закрывается
своим `end`; `else`, `and`, `option` — продолжения, они `end` не закрывают.
