...
Полная таблица triggers для conditional analysis (Phase 2 алгоритма `/polisade:design`).
...
Композиция:
```
/polisade:design PRD-001 --only=c4_container,sequence,openapi --skip=sequence
...
3. Рекомендуй: `/polisade:tasks` или `/polisade:roadmap` напрямую
