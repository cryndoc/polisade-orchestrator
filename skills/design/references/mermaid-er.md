...
Специфика ER: тело сущности `ENTITY { … }` обязано закрыться `}`; кардинальность
(`||--o{`, `}o--||`) содержит фигурные скобки, но блока не открывает. Подпись
связи после `:` — тоже метка, `;` в ней ломает блок. Комментарий атрибута —
кавычки обязательны парой: `string email UK "not null, unique"`.
