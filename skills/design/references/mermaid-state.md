...
Специфика state: составное состояние `state X { … }` закрывается `}`; подпись
перехода после `:` (`Idle --> Busy : оплатил; списал`) — метка, `;` в ней
роняет блок. Заголовок пиши как `stateDiagram-v2` (дефис и `v2`, не `V2`).
