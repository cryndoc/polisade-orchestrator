...
Специфика deployment (flowchart): метка ребра живёт между вертикальными чертами
(`A -->|деплой; откат| B`) или в inline-форме (`A -- деплой; откат --> B`) — `;`
в обеих ломает блок. Каждый `subgraph` закрывается `end`. Завершающий `;` в
конце оператора (`A --> B;`) легален — это НЕ ошибка.
