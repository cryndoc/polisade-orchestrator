...
Polisade Orchestrator ships three parallel releases on every git tag — pick the one that matches the CLI you use. All three expose the same `/polisade:*` slash commands.
...
- Independent quality review (`/polisade:review-pr`) runs in an isolated Qwen/GigaCode subagent instead of shelling out to an external reviewer CLI. The `self` flag is accepted for CLI compatibility but is effectively the default in those builds.
...
| `/polisade:reconcile-docs` | Advisory drift detection: design docs vs implemented code |
...
skills/            24 /polisade:* commands (source of truth)
