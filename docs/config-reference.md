...
Plugin version covered: `2.20.11`. Schema version covered: `3`
...
|---|---|---|---|---|
| `pdlcVersion` | string | `"2.20.11"` (template) | SemVer `MAJOR.MINOR.PATCH` | Plugin version that wrote this state. Bumped on release; read by `pdlc_doctor`. |
| `schemaVersion` | integer | `1` (template), `3` (after migrate) | `1` \| `2` \| `3` | Schema format. Values `< 3` trigger migration steps in `pdlc_migrate.py`. Never edit by hand. |
| `lastUpdated` | string \| null | `null` | ISO 8601 UTC (e.g. `"2026-04-23T14:30:00Z"`) or `null` | Reserved for audit trail. Currently initialized as `null`; no skill writes it yet. Do not rely on it. |
...
| `schemaVersion: 1`, `schemaVersion: 2` | early releases | schemaVersion 3 | Migrator handles both; running `/pdlc:migrate` on an old project is idempotent. |
