...
Plugin version covered: `3.7.13`. Schema version covered: `7`
...
Only one file under `.state/` is committed: `knowledge.json`. The table says
...
| `counters.json` | 🔶 open | no | A **monotonic high-water mark**, and the one local file that carries something the committed artefacts do not. The next id is `max(counter, highest id in artifactIndex and on disk) + 1`, and the counter is never lowered — the protocol says so in as many words, "even if the artefacts on disk were deleted", explicitly so a freed id is not handed out again and references in git history and PRs keep pointing at one artefact (`skills/tasks/references/compute-next-id.md`). That protection is currently **per-machine**: measured on the same repository where `TASK-004` was created and later deleted, the author's machine hands out `TASK-005` and a colleague's fresh clone hands out `TASK-004`. The drift abort does not save the clone — it fires on a counter below disk and repairs it by lowering the mark to what is on disk, which is what erases the memory of the deleted id. Whether the mark should travel — against a conflict on every branch that creates an artefact — is issue #299. |
| `acceptance-baseline.json` | 🔶 open | no | Hashes of the ratified acceptance pairs. `--fail-on-changed` **requires** a valid baseline and refuses without one, so while the file stays local, only the person who last ran acceptance on their own machine can detect a weakened check — a colleague and CI have nothing to compare against. Whether it should be committed is issue #299. |
| `session-log.md` | 🔶 open | no | The only durable record of what happened besides git history and PR comments. Committing it gives a regulated environment something to point at, at the cost of a noisy diff. Issue #299. |
...
**Two `.state/` names that are deliberately not rows.** Both turn up if you grep
the sources, and neither is a file a target project gets.
`.state/drift-gate-report.json` appears inside a drift-gate diagnostic as an
example path: `--report` has no default, so no report is written unless the
caller names one, and the CI template shipped by `/polisade:init` writes
`drift-report.json` at the repository root instead.
`.state/migration-progress.json` is the default of `--state-file` in
`scripts/migrate_backlog_to_issues.py`, a dev-only script that ships to no
target, so that path exists only inside this repository.

**Parallel ids are not a state-locality problem.** Two people on two branches
who each create a `FEAT-002` collide because autoincrement over a distributed
history collides, not because their counters differ: both see the same ids on
`main`, and both compute the same next one. Committing `counters.json` does not
change that either — it is a floor over ids that already exist, not a lock. What
removes the collision is either handing out ranges per person or a way to
renumber an artefact with its references after the fact; neither exists today
(issue #299). Keep the two apart: committing the counter answers a different
question — whether the mark over *deleted* artefacts should survive a clone.
...
|---|---|---|---|---|
| `polisadeVersion` | string | current plugin version (template — `3.7.13` today) | SemVer `MAJOR.MINOR.PATCH` | Plugin version that wrote this state. Bumped on release; read by `polisade_doctor`. **Renamed from the legacy `pdlcVersion` in schema 6 (v3.0.0).** `polisade_doctor` accepts either key (dual-key); `polisade_migrate` renames the legacy key and drops it. |
...
#### Why `/polisade:design-corpus` is Claude Code only (issue #300)

The skill carries `claude_only: true` in `cli-capabilities.yaml`, so it is
absent from the Qwen, GigaCode and opencode bundles entirely. That is a
**deliberate refusal, and here are its two reasons** — a reader on GigaCode
should not have to infer them from a flag.

**Measured, not assumed.** The skill's own references are 21 KB — a third of
what `/polisade:tasks` already inlines into those bundles (70 KB), so they are
not the obstacle. The obstacle is that at synthesis time the corpus reuses ten
format guides from `skills/design/references/` — **132 KB measured** — and
those are cross-skill reads out of the install directory, which the GigaCode
filesystem guard refuses (the same class the `#139` inlining solved for
`/polisade:tasks`). Of that 132 KB, 52 KB is diagram-format prose whose output
is already checked deterministically by `scripts/polisade_lint_mermaid.py`, so
the figure is an upper bound on what would actually have to travel.

The second reason is not technical: the corpus flow was written for a strong
model and **has never been measured on a contour model**. Opening the mode
without that measurement would put a pilot on an unmeasured path.

**What would change the answer:** a measurement of the same SPEC increment
built by the corpus on a strong and on a contour model, judged by a person,
plus a decision about the 132 KB of guides — inline them, narrow them to what
the synthesis actually needs, or state that the node runs reduced under the
guard. Until then the honest answer to "will we have the corpus under
GigaCode?" is no, with these reasons, rather than a bare unavailable flag.
