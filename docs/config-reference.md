...
Plugin version covered: `3.8.1`. Schema version covered: `7`
...
|---|---|---|---|---|
| `polisadeVersion` | string | current plugin version (template — `3.8.1` today) | SemVer `MAJOR.MINOR.PATCH` | Plugin version that wrote this state. Bumped on release; read by `polisade_doctor`. **Renamed from the legacy `pdlcVersion` in schema 6 (v3.0.0).** `polisade_doctor` accepts either key (dual-key); `polisade_migrate` renames the legacy key and drops it. |
