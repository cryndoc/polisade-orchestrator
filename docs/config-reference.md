...
Plugin version covered: `3.2.0`. Schema version covered: `7`
...
| `settings.experimental.designCorpus` | boolean | `false` | `true` \| `false` | **EXPERIMENTAL opt-in (#187).** When `true`, `/polisade:design-corpus` (Claude Code only) applies a SPEC increment to a single living architecture corpus; when `false`/absent the skill explains the experimental mode and exits without writes. `/polisade:design` (per-SPEC silo) is unaffected either way. Added additively by `polisade_migrate.py` (schema 7). |
...
| `architecture.corpus.mode` | string | `"silo"` | `"silo"` \| `"living"` | `silo` = per-SPEC DESIGN packages (default, `/polisade:design`); `living` = one corpus (`/polisade:design-corpus`). |
| `architecture.corpus.pendingRun` | object \| null | `null` | `{ runId, archRunId, stagingDir, question, pendingPlanItems }` or `null` | Set when a `/polisade:design-corpus` run halts to PM (ARCHRUN `waiting_pm`); read by `--resume`. `null` when no run is pending. |
